...
#if DEBUG_FOR_CANNOT_GET_DBUS_CONNECTION
  if (!dbus_bus_register (connection,private,error))
#else
...
#endif
    {
      ...
      _DBUS_UNLOCK (bus);
//	  fprintf(stderr,"+++++dbus_bus_register error ,connection is NULL PID IS %d\n",getpid());
      return NULL;
    }
 ...
 */
#if DEBUG_FOR_CANNOT_GET_DBUS_CONNECTION
int get_file_info(char* file,char*date)
{
	FILE* fd;
	fd = fopen(file,"r");
	if(fd == NULL)
	...
	fgets(date,512,fd);
	fclose(fd);
/*	fprintf(stderr,"get file info OK\n");*/
	return 1;
}
...
#if DEBUG_FOR_CANNOT_GET_DBUS_CONNECTION
{
	...
	int pid = getpid();
	memset(temp,0,512);
	if(-1 == get_file_info("/dbm/local_board/slot_id",(char*)temp))
	{
		slot = -1;
	}
	sscanf((char*)temp,"%d",&slot);
	memset(temp2,0,512);
	memset(temp,0,512);
	sprintf((char*)temp2,"/proc/%d/cmdline",pid);
	if(-1 == get_file_info((char*)temp2,(char*)temp))
	{
		slot = -1;
	}
/*	fprintf(stderr,"get all info over\n");*/
  dbus_message_append_args(message,
						  DBUS_TYPE_INT32, &private,
						  DBUS_TYPE_INT32, &pid,
						  DBUS_TYPE_UINT32, &slot,
						  DBUS_TYPE_STRING, &temp3,
						  DBUS_TYPE_INVALID);
}
/*fprintf(stderr,"append message over\n");*/
...
//  fprintf(stderr,"+++++In the dbus_bus_register() reply is %x PID IS %d\n",(int)reply,getpid());
  if (reply == NULL)
    goto out;
  else if (dbus_set_error_from_message (error, reply))
  	{
//  	fprintf(stderr,"get the reply message is error,PID IS %d\n",getpid());
    goto out;
  	}
                                   ...
                                   DBUS_TYPE_INVALID))
    goto out;
//  fprintf(stderr,"get dbus bus name is %s PID IS %d\n",name,getpid());
...
DBusConnection *
dbus_bus_get_local (DBusBusType  	type,
	      	  		DBusError   *error)
{
...
//	void* data_temp;
	_dbus_return_val_if_fail (type >= 0 && type < N_BUS_TYPES, NULL);
	_dbus_return_val_if_error_is_set (error, NULL);
	if(bus_connection_addresses[type] != NULL)
	{
//		dbus_free(bus_connection_addresses[type]); /*addresses_shutdown_func has free*/
...
//	void* data_temp;
	...
	_dbus_return_val_if_fail (slot_num >= 0, NULL);
	_dbus_return_val_if_fail (type >= 0 && type < N_BUS_TYPES, NULL);
	_dbus_return_val_if_error_is_set (error, NULL);
	
//set DBUS_SYSTEM_BUS_ADDRESS env
	memset(temp,0,256);

	slot_num += 1000;
	sprintf(temp,"tipc:inst=%d",slot_num);
	
	if(bus_connection_addresses[type] != NULL)
	{
		//free(bus_connection_addresses[type]);
		addresses_shutdown_func(NULL);
	}
	bus_connection_addresses[type] = _dbus_strdup (temp);
	connection = dbus_bus_get_private (type, error);
