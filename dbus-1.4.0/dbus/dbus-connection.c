...
{
//	fprintf(stderr,"_dbus_connection_wakeup_mainloop PID is %d\n",getpid());
	
  if (connection->wakeup_main_function)
    (*connection->wakeup_main_function) (connection->wakeup_main_data);
//  fprintf(stderr,"_dbus_connection_wakeup_mainloop over PID is %d\n",getpid());
                                 ...
                                 int                 timeout_milliseconds)
{
  ...
  if (!_dbus_pending_call_set_timeout_error_unlocked (pending, message, serial))
  	{
//  	fprintf(stderr,"_dbus_pending_call_set_timeout_error_unlocked ==error PID IS %d\n",getpid());
    goto error;
  	}
...
//  	fprintf(stderr,"wo return reply %x PID IS %d\n",(int)reply,getpid());
    return reply;
}
...
/*      fprintf (stderr,"Notifying of change to dispatch status of %p now %d (%s)\n",
