...
fprintf(stderr,"Iteration block=%d depth=%d timeout_count=%d watch_count=%d PID is %d\n",
			   block, loop->depth, loop->timeout_count, loop->watch_count,getpid());
...
fprintf(stderr,"  skipping watch on fd %d as it was out of memory last time PID IS %d\n",
			   dbus_watch_get_socket (wcb->watch),getpid());
...
fprintf (stderr,"  polling watch on fd %d  %s PID IS %d\n",
			   fds[n_fds].fd, watch_flags_to_string (flags),getpid());
...
fprintf(stderr,"  skipping disabled watch on fd %d	%s PID IS %d\n",
			   dbus_watch_get_socket (wcb->watch),
			   watch_flags_to_string (dbus_watch_get_flags (wcb->watch)),getpid());
...
/*fprintf(stderr,"poll change--------------------PID IS %d\n",getpid());*/
...
fprintf (stderr,"  Invoked watch, oom = %d\n",
			   wcb->last_iteration_oom);
...
fprintf (sederr,"  moving to next iteration\n");
...
_dbus_loop_run (DBusLoop *loop)
{
  ...
  fprintf (stderr,"Running main loop, depth %d -> %d  PID is %d\n",
                 loop->depth - 1, loop->depth,getpid());
