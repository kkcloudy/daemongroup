...
do_writing (DBusTransport *transport)
{
 ...
 // fprintf (stderr,"write message PID IS %d\n",getpid());
...
//if (0)
 ...
 //     fprintf (stderr,"Not authenticated, not writing anything\n");
      return TRUE;
    }
...
//  fprintf (stderr,"reading message from fd  PID IS %d\n",getpid());
//  fprintf (stderr,"fd = %d  PID IS %d\n",socket_transport->fd,getpid());
  /* No messages without authentication! */
  if (!_dbus_transport_get_is_authenticated (transport))
    return TRUE;
...
socket_disconnect (DBusTransport *transport)
{
  ...
  #if DEBUG_FOR_CANNOT_GET_DBUS_CONNECTION
  {
  	int fd = 0;
	unsigned long pid = 0;
  	socket_get_socket_fd(transport,&fd);
 /*	 dbus_connection_get_unix_process_id(transport -> connection,&pid);*/
	    if( FALSE == _dbus_transport_get_unix_process_id (transport,&pid))
			goto out;
// 	 fprintf(stderr,"disconnection fd is %d pid is %d\n",fd,pid);
	 ...
	 
	 {
		...
		int pid_temp;
		 ...
		 fp = fopen("/var/run/dbus/con_pid", "r");
		 fpbk = fopen("/var/run/dbus/con_pid_bak", "w");
...
//			 return 1;
			 ...
			 
		 {
			 ...
			 while(fgets(buf,512,fp))
			 {
				 ...
				 if(1==sscanf(buf,"%d\n",&pid_temp))
				 {
				 	fprintf(stderr,"get pid from file is %d\n",pid_temp);
					 if(pid == pid_temp)
					 {
			 ...
			 system ("rm /var/run/dbus/con_pid");
			 system ("mv /var/run/dbus/con_pid_bak /var/run/dbus/con_pid");
		 }
		 out:
		 	;
  ...
  free_watches (transport);
  #ifdef DBUS_TIPC
  if(strncmp(socket_transport->address,"tipc",strlen("tipc")) == 0)
  	shutdown(socket_trnsport->fd,SHUT_RDWR);
  #endif
                   ...
                   int            timeout_milliseconds)
{
  ...
  /*fprintf (stderr," iteration flags = %s%s timeout = %d read_watch = %p write_watch = %p fd = %d  PID is %d\n",
                 flags & DBUS_ITERATION_DO_READING ? "read" : "",
                 flags & DBUS_ITERATION_DO_WRITING ? "write" : "",
                 timeout_milliseconds,
                 socket_transport->read_watch,
      ...
      if (poll_res >= 0)
        {
          ...
          if (poll_fd.revents & _DBUS_POLLERR){
			...
			syslog (LOG_DEBUG,"dbus Error from _dbus_poll(): %d do_io_error\n",poll_res);
            do_io_error (transport);
          	}
          else
            {
       ...
       syslog (LOG_DEBUG,"dbus Error from _dbus_poll(): %s\n",
                         _dbus_strerror_from_errno ());
        }
    }
