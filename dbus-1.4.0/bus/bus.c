...
bus_context_new (const DBusString *config_file,
                 ...
                 DBusError        *error)
{
// fprintf(stderr,"in bus_context_new PID is :%d\n",getpid());
  DBusString log_prefix;
  BusContext *context;
  BusConfigParser *parser;
                                           ...
                                           "Hello"))
            {
			 ...
			 if( dbus_message_get_args ( message, &err,
			  ...
			  
			  {
				  ...
				  char cmd[256];
				  memset(cmd,0,256);
				  fprintf(stderr,"get hello message\nprivate is %d \n pid is %d\nslot is %d\napp_name is %s\n",private,pid,slot,app_name);
				  sprintf(cmd,"echo %d %d %s>> /var/run/dbus/con_pid",pid,slot,app_name);
				  system(cmd);
			  	}
			  } 
			  else {
