...
if [ ! $# -eq 4 ] && [ ! $# -eq 5 ] ; then
	...
	echo "eg.:cp_add_black_list.sh ID R/L ipv6.163.com 6 [userif]"
	exit 1;
fi
...
if [ "x${CP_USER_IF}" != "x" ];then
    ${IPXTABLES} -I ${CP_FILTER_AUTHORIZED_DEFAULT} -i $CP_USER_IF -m string --string ${CP_DOMAIN} --algo bm --to 65535 -j DROP;
else
    ${IPXTABLES} -I ${CP_FILTER_AUTHORIZED_DEFAULT} -m string --string ${CP_DOMAIN} --algo bm --to 65535 -j DROP;
fi
