...
if [ "x${CP_USER_IF}" != "x" ];then
    iptables -I ${CP_FILTER_AUTHORIZED_DEFAULT} -i $CP_USER_IF -m string --string ${CP_DOMAIN} --algo bm --to 65535 -j DROP;
else
    iptables -I ${CP_FILTER_AUTHORIZED_DEFAULT} -m string --string ${CP_DOMAIN} --algo bm --to 65535 -j DROP;
fi
