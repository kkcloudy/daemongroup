/*
 * Generated by asn1c-0.9.23 (http://lionet.info/asn1c)
 * From ASN.1 module "RANAP-IEs"
 * 	found in "RANAP-IEs.asn"
 */

#ifndef	_GA_AltitudeAndDirection_H_
#define	_GA_AltitudeAndDirection_H_


#include <asn_application.h>

/* Including external dependencies */
#include <ENUMERATED.h>
#include <NativeInteger.h>
#include <constr_SEQUENCE.h>
#include "PDU_ARRAY.h"

#ifdef __cplusplus
extern "C" {
#endif

/* Dependencies */
typedef enum directionOfAltitude {
	directionOfAltitude_height	= 0,
	directionOfAltitude_depth	= 1
} e_directionOfAltitude;

/* GA-AltitudeAndDirection */
typedef struct GA_AltitudeAndDirection {
	ENUMERATED_t	 directionOfAltitude;
	long	 altitude;
	/*
	 * This type is extensible,
	 * possible extensions are below.
	 */
	
	/* Context for parsing across buffer boundaries */
	asn_struct_ctx_t _asn_ctx;
} GA_AltitudeAndDirection_t;

/* Implementation */
/* extern asn_TYPE_descriptor_t asn_DEF_directionOfAltitude_2;	// (Use -fall-defs-global to expose) */
extern asn_TYPE_descriptor_t asn_DEF_GA_AltitudeAndDirection;

#ifdef __cplusplus
}
#endif

#endif	/* _GA_AltitudeAndDirection_H_ */
#include <asn_internal.h>
