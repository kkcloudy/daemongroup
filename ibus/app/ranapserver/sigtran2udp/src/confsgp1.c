...
unsigned int sgp_sctp_connect_mode = 1;
	
void sgp_set_sctp_cn_mode(unsigned int opposite_connect_mode)
{	
	if(opposite_connect_mode){
		sgp_sctp_connect_mode = 0;
	}else{
		sgp_sctp_connect_mode = 1;
	}

	printf("sgp sctp connect mode is %s\n", sgp_sctp_connect_mode ? "server":"client");
	
	return ;
}


void sgp_set_global_opc(unsigned int opc)
{
	global_msc_opc = opc;
}

void sgp_set_global_msc_dpc(unsigned int dpc)
{
	global_msc_dpc = dpc;
}
...
int um3_get_cn_addr(char *argv[], udp_addr_t *paddr)
{
    char        *p;
    char        *ip;
    char        *port;

    if (0 != strncmp("cn", argv[3], 2))
        return -1;
    p = strchr(argv[3], '=');
    if (NULL == p)
        return -1;
    ...
    paddr->ipaddr = inet_addr(ip);
    paddr->port = atoi(port);
printf("at confsgp.c cn paddr->ipaddr %x, port %x \n", paddr->ipaddr, paddr->port);
    return 0;
}
...
{
    udp_addr_t        addr;
    int               asp1_ep, sgp1_ep, cn_lep = 0, cn_rep = 0;

    /* create transport layer endpoints */
    um3_get_sgp1_addr(argv, &addr);
    sgp1_ep = make_lep_sctp(addr);
    cn_lep = make_lep_udp(addr);/*udp local end point*/
    um3_get_asp1_addr(argv, &addr);
    asp1_ep = make_rep_sctp(addr);

    /* create associations between the transport layer endpoints */
    make_assoc_sctp(sgp1_ep, asp1_ep, sgp_sctp_connect_mode);
	
    um3_get_cn_addr(argv, &addr);
    cn_rep = make_rep_udp(addr);
	printf("start sigtran2udp cn_lep %d, cn_rep %d\n",cn_lep, cn_rep);
    make_assoc_udp(cn_lep, cn_rep, 0);
    return 0;
}

int um3_transport_init_arg
(
	unsigned int lip,
	...
	unsigned int cnip,
	unsigned short cnport
)
{
    udp_addr_t        laddr = {.ipaddr = lip, .port = lport}, 
					  raddr = {.ipaddr = rip, .port = rport},
					  cnaddr = {.ipaddr = cnip, .port = cnport};
    int               asp1_ep, sgp1_ep, cn_lep = 0, cn_rep = 0;

    /* create transport layer endpoints */
    sgp1_ep = make_lep_sctp(laddr);
    asp1_ep = make_rep_sctp(raddr);

    /* create associations between the transport layer endpoints */
    make_assoc_sctp(sgp1_ep, asp1_ep, 1);

    cn_lep = make_lep_udp(laddr);/*udp local end point*/
    cn_rep = make_rep_udp(cnaddr);
	printf("start sigtran2udp cn_lep %d, cn_rep %d\n",cn_lep, cn_rep);
    make_assoc_udp(cn_lep, cn_rep, 0);
    return 0;
}
...
m3_s32 um3_m3ua_nwapp(m3_u8   oprn)
{
    ...
    switch(oprn) {
        case M3_ADD: {
            ...
            api.add.info.nw_app = M3_MAX_U32;
...
//          api.add.info.rkey.nw_app = 0x01ABCDEF;
            ...
            api.add.info.rkey.rtparam[0].dpc = 0x00010101; /* 24 bit pointcode -- ANSI, change later*/
            ...
            api.add.info.rkey.rtparam[0].opc_list[1] = 0x00010304;/*change later */
...
)
{
    ...
    um3_transport_init_arg(lip, lport, rip, rport, cnip, cnport);
...
//	global_opc = 0x00010304;
//	global_msc_dpc = 0x00010101;
//	start_sccp_con = 1;
//	um3_server_start(0xc0a80456, 0x1b66b, 0xc0a80457, 0x1b669, 0xc0a80459, 0x2b66b);/*86*/
	//um3_server_start(0xc0a80459, 0x2b66b, 0xc0a80458, 0x2b669, 0xc0a80456, 0x1b66b);/*89*/
