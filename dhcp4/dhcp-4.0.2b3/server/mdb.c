...
/* Confpars.c */
void parse_host_declaration_test 
(
	char* hostname,
...
FILE *discover_file;		/* discover times file */
FILE *offer_file;			/* offer times file */
FILE *request_file;			/* requested times file */
FILE *response_file;		/* response times file */
FILE *lease_file;			/* lease file about ip, mac, starts time, ends time */

unsigned int add_range_flag = 0;
extern unsigned int dhcp_optimize_enable;	/* dhcp performance optimization. no log, stop write lease file */

extern unsigned int discover_times;
...
/**********************************************************************************
 *  set_interface_read_flag
 *
 *	DESCRIPTION:
 * 		set interface bind pool flag
 *
 *	INPUT:
 *		interface name 
 * 		bind flag 
 ...
 *		
 *		1		-> not find pool 
 *		0		-> find pool and set flag
 *		
 **********************************************************************************/

int set_interface_bind_flag(char *ifname , unsigned int unbind_flag)
{		
	struct interface_info *tmp_interface = NULL;

	for (tmp_interface = interfaces; tmp_interface; tmp_interface = tmp_interface->next){
		if(!strcmp(ifname, tmp_interface->name)){
			tmp_interface->unbind_pool_flag = unbind_flag;			
			return 0;
		}
	}
...
) 
{
	...
	discover_file = fopen("/var/run/apache2/dhcp_discover", "w+b");	
	offer_file = fopen("/var/run/apache2/dhcp_offer", "w+b");	
...
/**********************************************************************************
 *  save_vgateway_virtual_ip_in_interface
 *
 *	DESCRIPTION:
 * 		save vgateway ip in  interface 
 *
 *	INPUT:
 *		vrrp_interface_t
 *	
 ...
 *		
 **********************************************************************************/
#define IP_SAVE_LEN	4
int save_vgateway_virtual_ip_in_interface(vrrp_param_t  params)
{	
	struct interface_info *tmp_interface = NULL;
	int i = 0; 
	unsigned char ch_ip[IP_SAVE_LEN] = {0};
	int v_nu = 0; //save virtual number 
	

	for (tmp_interface = interfaces; tmp_interface; tmp_interface = tmp_interface->next) {
		
		for(i = 0; i < params.vgateway_cnt; ++i){
			if(!strcmp(tmp_interface->name, params.vgateway_interface[i].if_name)){

				if(params.vgateway_interface[i].virtual_ip){
					++v_nu;					
				}
				ch_ip[0] = (params.vgateway_interface[i].virtual_ip>>24)&0xff;		
				ch_ip[1] = (params.vgateway_interface[i].virtual_ip>>16)&0xff;
				ch_ip[2] = (params.vgateway_interface[i].virtual_ip>>8)&0xff;
				ch_ip[3] = params.vgateway_interface[i].virtual_ip&0xff;

				/*save to interface*/
				memcpy(&tmp_interface->virtual_ip, ch_ip, IP_SAVE_LEN);
				tmp_interface->vgateway_flag = 1;
				
				log_info("interface %s(vgateway) virtual ip %u.%u.%u.%u\n", tmp_interface->name, ch_ip[0], ch_ip[1], ch_ip[2], ch_ip[3]);
				memset(ch_ip, 0, IP_SAVE_LEN);					
			}
		}		

		for(i = 0; i < params.downlink_cnt; ++i){
			if(!strcmp(tmp_interface->name, params.downlink_interface[i].if_name)){

				if(params.downlink_interface[i].virtual_ip){
					++v_nu;					
				}
				ch_ip[0] = (params.downlink_interface[i].virtual_ip>>24)&0xff;		
				ch_ip[1] = (params.downlink_interface[i].virtual_ip>>16)&0xff;
				ch_ip[2] = (params.downlink_interface[i].virtual_ip>>8)&0xff;
				ch_ip[3] = params.downlink_interface[i].virtual_ip&0xff;

				/*save to interface*/
				memcpy(&tmp_interface->virtual_ip, ch_ip, IP_SAVE_LEN);
				tmp_interface->vgateway_flag = 1;
				
				log_info("interface %s(downlink) virtual ip %u.%u.%u.%u\n", tmp_interface->name, ch_ip[0], ch_ip[1], ch_ip[2], ch_ip[3]);
				memset(ch_ip, 0, IP_SAVE_LEN);					
			}
		}		
	}	
 ...
 *
 *	DESCRIPTION:
 * 		get dhcp pool status : requst times , request ok times 
 *
 *	INPUT:
 *		void
 *	
...
void
get_dhcp_pool_info(void) 
{
	FILE *fp = NULL;
	struct subnet *rv = NULL;
	struct pool *next = NULL;
	int leasenum = 0, freenum= 0;

	fp = fopen("/var/run/apache2/dhcp_leasestat", "w+b");	
	if(NULL == fp)
	{
		return;
	}
	...
	for (rv = subnets; rv; rv = rv->next_subnet) 
	{
		leasenum = 0;
		freenum = 0;
		if ((rv->shared_network) && (rv->shared_network->pools)) {
			next = rv->shared_network->pools;
			while (next ) 
			{ 
				leasenum += next->lease_count;
				freenum += next->free_leases;
				next = next->next;
			}
		}
		fprintf(fp, "%s %d %d %d %d %d %d\n",piaddr(rv->net),leasenum,freenum,
...
{
	int  whole_times = 0, segment_times = 0, count = 0;		
	struct dhcp_lease_info *lease_info;
	struct hash_table *table = lease_hw_addr_hash;
	struct subnet *rv = NULL;
	struct pool *next = NULL;
	struct lease *tmp_active = NULL;
	...
	*/
	...
	for (rv = subnets; rv; rv = rv->next_subnet) 
	{
		if ((rv->shared_network) && (rv->shared_network->pools)) 
		{
			...
			while (next ) 
			{				
				for(tmp_active = next->active; tmp_active ; tmp_active = tmp_active->next){
					lease_info->lease_ip[count].ip_addr = tmp_active->ip_addr;
					lease_info->lease_ip[count].starts = tmp_active->starts;
					lease_info->lease_ip[count].ends = tmp_active->ends;						
					memcpy(lease_info->lease_ip[count].hw_addr, &tmp_active->hardware_addr.hbuf[1], 6);
					count++;
				}
	...
	for (rv = subnets; rv; rv = rv->next_subnet) {
		if (rv->shared_network) {
			if (rv->shared_network->pools) {
				...
				while (next) {				
					for(tmp_active = next->active; tmp_active ; tmp_active = tmp_active->next) {
						lease_info->lease_ip[i].ip_addr = tmp_active->ip_addr;
						lease_info->lease_ip[i].starts = tmp_active->starts;
						lease_info->lease_ip[i].ends = tmp_active->ends;						
						memcpy(lease_info->lease_ip[i].hw_addr, &tmp_active->hardware_addr.hbuf[1], 6);
						i++;
	...
	for (rv = subnets; rv; rv = rv->next_subnet) {
		...
		if ((rv->shared_network) && (rv->shared_network->pools)) {
			...
			while (next) {
				...
				for(tmp_active = next->active; tmp_active; tmp_active = tmp_active->next){
					if (0 == memcmp(hw_addr, &tmp_active->hardware_addr.hbuf[1], 6)) {
						
						lease_ip->ip_addr = tmp_active->ip_addr;
						lease_ip->starts = tmp_active->starts;
						lease_ip->ends = tmp_active->ends; 
...
{	
	...
	*sub_count = sub_num;	
	...
	for (rv = subnets; rv; rv = rv->next_subnet) 
	{
		...
		poolnode = dhcp_dbus_find_poolnode_by_subnet(rv);
		if (poolnode) {
		...
		log_debug("sub %s %s total is %d , total free %d  total active %d discover %d offer %d request %d ack %d\n", 
...
/* zhangshu add, 2012-9-26 */
int free_lease_ip_info(struct dhcp_lease_info** lease_info)
{
    log_debug("%s\n", __func__);
    struct dhcp_lease_ip_info *tmpNode = NULL;
    struct dhcp_lease_info *head = NULL;
    if(lease_info == NULL)
        return 0;
    ...
    while(head->lease_ip->next != NULL) {
        tmpNode = head->lease_ip->next;
        head->lease_ip->next = tmpNode->next;
        tmpNode->next = NULL;
        free(tmpNode);
        tmpNode = NULL;
    }
...
)
{
	...
	while (ip_Nums) {
		...
		if (ip_lease && (FTS_ACTIVE == ip_lease->binding_state)) {
...
int lease_delete_by_subnet(struct subnet *sub, unsigned int num, struct dcli_subnet *delsub) 
{
	int i, j, count = 0;
	struct hash_bucket *bp, *next;
	struct hash_table *hw_table = lease_hw_addr_hash;
	struct hash_table *ip_table = lease_ip_addr_hash;
	unsigned int haddr, maddr;
	struct iaddr cmp_addr;
	if (!hw_table || !ip_table) {
		return 0;
	}

	haddr = host_addr (delsub->highip, sub->netmask);
	maddr = host_addr (delsub->lowip, sub->netmask);

	log_debug("=========> haddr is %d , maddr is %d <=========\n", haddr, maddr);
		...
		while (bp) {
			...
			if (!lease || (lease->ip_addr.len)) {		
				if (addr_eq(sub->net, subnet_number(lease->ip_addr, sub->netmask))) {
					/*add by sunjc for find range to delete*/
					for (j = 0; j < haddr - maddr + 1; j++) {
						cmp_addr = ip_addr (sub->net, sub->netmask, j + maddr);
						if(!memcmp(&(lease->ip_addr), &(cmp_addr), sizeof(struct iaddr))){							
							hw_hash_delete(lease);
							log_info("&& delete hw_hash lease is %s\n", piaddr(cmp_addr));
						}							
					}					
				}
			}
...
//							log_debug("&& delete ip_hash lease is %s\n", piaddr(cmp_addr));
						}							
					}					
				}
			}
...
)
{
	...
	for (ip = subnets; ip; ip = ip -> next_subnet)  {
		if (ip == subtmp) {
			log_debug("del_subnet_test ip is %s \n", piaddr(ip->net));
			if (last) {
...
lease_delete_test(const void *key, unsigned len, void *object)
{
	...
	//	if (addr_eq(sub_conf.net, subnet_number(lease->ip_addr, sub_conf.netmask))) 
		{
			lease_ip_hash_delete(lease_ip_addr_hash, lease->ip_addr.iabuf, lease->ip_addr.len, MDL);
		/*	log_debug("lease %s delete from (lease_ip_hash)\n", piaddr(lease->ip_addr)); */
		}
		/* delete the lease in the uid hash if possible. */
		if (lease->uid) {
			uid_hash_delete(lease);
		/*	log_debug("  %% uid_hash_delete\n"); */
		}

		/* delete it in the hardware address hash if possible. */
		if (lease->hardware_addr.hlen) {
			hw_hash_delete(lease);
			log_debug("  %% hw_hash_delete\n");			
		}
	}
...
{
	struct iaddr addr, low, high, cmp_addr;
	struct shared_network *shared_nw;
	struct subnet *rv = NULL, *tmp = NULL, *priv = NULL;
	struct lease *free_lease = NULL, *active = NULL, *pre_lease = NULL;
	int i = 0;
	unsigned int haddr, maddr;
	int lease_count = 0;
	
	addr.iabuf[0] = delsub->ipaddr.iabuf[0];
	addr.iabuf[1] = delsub->ipaddr.iabuf[1];
	addr.iabuf[2] = delsub->ipaddr.iabuf[2];
	addr.iabuf[3] = delsub->ipaddr.iabuf[3];
	addr.len = delsub->ipaddr.len;

	
	
	for (rv = subnets; rv; rv = rv->next_subnet) {
		haddr = host_addr (delsub->highip, rv->netmask);
		maddr = host_addr (delsub->lowip, rv->netmask);
		
		log_debug("&& haddr is %d , maddr is %d delete the number addr is %d\n",haddr, maddr,(haddr-maddr+1));

		if (addr_eq (subnet_number (addr, rv->netmask), rv->net)) {
						
			if (rv->shared_network) {
				shared_nw = rv->shared_network;
				if (shared_nw->pools && shared_nw->pools->free) {
					for (free_lease = shared_nw->pools->free; free_lease; pre_lease=free_lease, free_lease = free_lease->next) {
						/*add by sunjc for find range to delete*/
						for (i = 0; i < haddr - maddr + 1; i++) {
							cmp_addr = ip_addr (rv->net, rv->netmask, i + maddr);
							if(!memcmp(&(free_lease->ip_addr), &(cmp_addr), sizeof(struct iaddr))){	
								if(NULL == pre_lease){
									
shared_nw->pools->free = shared_nw->pools->free->next;
								}else{
									 pre_lease->next = free_lease->next;
								}
								
								dhcp_lease_destroy(free_lease, MDL);
								shared_nw->pools->lease_count--;
								shared_nw->pools->free_leases--;
								log_debug("&&delsub->ip_num is %d lease count %d delete free lease is %s\n",delsub->ip_num,shared_nw->pools->lease_count, piaddr(cmp_addr));
							}							
						}
					}
				}
				if (shared_nw->pools && shared_nw->pools->active) {
					for (active = shared_nw->pools->active; active; pre_lease=active, active = active->next) {
						/*add by sunjc for find range to delete*/
						for (i = 0; i < haddr - maddr + 1; i++) {
							cmp_addr = ip_addr (rv->net, rv->netmask, i + maddr);
							if(!memcmp(&(active->ip_addr), &(cmp_addr), sizeof(struct iaddr))){	
								if(NULL == pre_lease){
									
shared_nw->pools->active = shared_nw->pools->active->next;
								}else{
									pre_lease->next = active->next;
								}
								dhcp_lease_destroy(active, MDL);
								shared_nw->pools->lease_count--;
								log_debug("&& delsub->ip_num is %d lease count %d delete active lease is %s\n",delsub->ip_num,shared_nw->pools->lease_count, piaddr(cmp_addr));
							}
							
						}						
					}
				}				
				lease_count = shared_nw->pools->lease_count;
				if(lease_count <= 0){
					dhcp_pool_destroy(shared_nw->pools, MDL);
					dhcp_shared_network_destroy(rv->shared_network, MDL);					
				}				
			}

			//lease_ip_hash_foreach(lease_ip_addr_hash, lease_delete_test);		
			lease_delete_by_subnet(rv, delsub->ip_num, delsub);

			if(lease_count <= 0){
				dhcp_subnet_destroy(rv, MDL);
				del_subnet_test(rv);
			}			
			break;
		}
...
{
	int i = 0;
	struct subnet *rv = NULL, *prev = NULL;
	struct lease *lp = NULL;
	struct pool *del_pool = NULL;
	struct shared_network *del_share = NULL;
	...
	while (rv) {
		...
		if (addr_eq (subnet_number (iaddr, rv->netmask), rv->net)) {

			log_debug("&& delete range is %s %s\n", piaddr(iaddr), piaddr(delsub->highip));

			/* find pool & shared_network */
			find_lease_by_ip_addr(&lp, iaddr, MDL);
			if (lp && lp->pool && lp->pool->shared_network) {
				del_pool = lp->pool;
				del_share = lp->pool->shared_network;
				
			} else {

				if (lp) {
					if (lp->pool) {
						log_error("&& find lease by ip addr failed pool\n");
					} else {
			...
			/* find delete pool and remove from (shared_network pools list) */
			struct pool *tmp = del_share->pools;
			struct pool *pre = NULL;
			while (tmp) {
				if (tmp == del_pool) {
					break;
				}
			...
			for (i = 0; i < delsub->ip_num; i++) {
				lp = NULL;				
				find_lease_by_ip_addr(&lp, iaddr, MDL);
				if (lp) {

					log_debug("ip address %s, delete lease %s.\n", piaddr(iaddr), piaddr(lp->ip_addr));

					lp->pool->lease_count--;
					if (FTS_FREE == lp->binding_state) {
						lp->pool->free_leases--;
					...
					find_lease_by_ip_addr(&lp, iaddr, MDL);
					if (lp) {
						log_error("ip address %s, delete lease %s error!!\n", piaddr(iaddr), piaddr(lp->ip_addr));
					}
				}
			...
			/* delete pool */
			dhcp_pool_destroy((omapi_object_t *)del_pool, MDL);

			if(NULL == del_share->pools) {
				dhcp_shared_network_destroy((omapi_object_t *)del_share, MDL);					

				log_debug(" %% dhcp subnet destory %s\n", piaddr(rv->net));
...
)
{
	...
	for (tmp = interfaces; tmp; tmp = tmp->next) {
		if (!strcmp(tmp->name, interface_name)) {
			log_debug("delete interface for subnet interface name is %s \n", interface_name);			
			#if 1
			if (tmp->shared_network) {
				log_debug("delete shared_network %s interface %s reference\n", tmp->shared_network->name, interface_name);	
				interface_dereference (&tmp->shared_network->interface, MDL);

				if (tmp->shared_network->subnets) {
					log_debug("delete subnet %s interface %s reference\n", 
								piaddr(tmp->shared_network->subnets->net), interface_name);					
					interface_dereference (&tmp->shared_network->subnets->interface, MDL);
				}
			}
			
			dhcp_interface_remove((omapi_object_t *)tmp, NULL);
			#else
			omapi_unregister_io_object(tmp);
			dhcp_interface_destroy(tmp, MDL);			
...
)
{
	...
	pthread_mutex_lock(&DhcpHashMutex);
	lease = find_lease_by_ip_addr_from_hw(addr);
	pthread_mutex_unlock(&DhcpHashMutex);

	if (!lease) {
		log_debug("no lease here for this ip addr %s\n", piaddr(addr));
		return 1;		
	}
...
#ifndef __AX_PLATFORM__
	if (lease->pool) {		
		if ((lease->pool->free_leases) &&
			(lease->pool->pool_full)) {
			lease->pool->pool_full = 0;			
			log_debug("release lease pool lease_count is %d,  free_leases is %d, backup_leases is %d \n", 
					lease->pool->lease_count, lease->pool->free_leases, lease->pool->backup_leases);
			dhcp_notify_to_trap_event(0, lease->pool->lease_count, lease->pool->free_leases, lease->subnet);
		}
	}
#endif	
	...
	int commit;
{
	...
	if (executable_statement_foreach (hd -> group -> statements,
		...
		if(evaluate_option_cache (&hd -> client_identifier,
				       (struct packet *)0,
				       (struct lease *)0,
				       ...
				       esp -> data.option, MDL)){
			log_debug("evaluate_option_cache success!\n");
		
		}else{
			log_debug("evaluate_option_cache failed!\n");
		}
	}
...
//#if defined (DEBUG_LEASE_STATE_TRANSITIONS)
	log_debug ("make_binding_state_transition lease ip is %s: binding state %s next binding state %s",
		   piaddr (lease -> ip_addr),
		   binding_state_print (lease -> binding_state),
		   binding_state_print (lease -> next_binding_state));
//#endif
	...
	void *vpool;
{
	...
	struct lease *lease = (struct lease *)0;
	struct lease *last = (struct lease *)0;
		...
		while (lease) {
			...
			if (lease -> next_binding_state !=
				...
				if (dhcp_server_static_arp_enable) {
					/* if unbind pool, should cancel timeout process. */
					if (lease->subnet && lease->subnet->interface) {
						ifindex = if_nametoindex(lease->subnet->interface->name);
						log_debug("pool_timer out ifindex is %d\n", ifindex);
						dhcp_dbus_four_char_to_int((unsigned char *)&(lease->ip_addr.iabuf), &ipaddr);
						log_debug("pool_timer out ipaddrs is %d\n", ipaddr);
						dhcp_snp_netlink_do_ipneigh(DHCPSNP_RTNL_IPNEIGH_DEL_E, ifindex, ipaddr, &(lease->hardware_addr.hbuf[1]));
					}
				}
#endif
...
int write_leases ()
{
	...
	if (collections->classes) {
		...
		log_debug ("Wrote %d class decls to leases file.",
			  numclasseswritten);
	}
	...
	if (group_name_hash) {
	    ...
	    log_debug("Wrote %d group decls to leases file.", num_written);
	}
	...
	if (host_name_hash) {
	    ...
	    log_debug("Wrote %d deleted host decls to leases file.",
		      num_written);
	}
	...
	if (host_name_hash) {
	    ...
	    log_debug("Wrote %d new dynamic host decls to leases file.",
		      num_written);
	}
	...
	log_debug("Wrote %d leases to leases file.", num_written);
#ifdef DHCPv6
	if (!write_leases6()) {
		return 0;
	}
...
unsigned int lease_find_in_pool(struct lease *comp){

	struct lease *exist_lease = NULL;
	if(!comp){
		return 0;
	}
	for(exist_lease = comp->pool->free; exist_lease; exist_lease = exist_lease->next){
		if(!memcmp(&(exist_lease->ip_addr.iabuf), &(comp->ip_addr.iabuf), comp->ip_addr.len)){
			log_debug("lease enqueue is %s\n",piaddr(comp->ip_addr));
			return 1;
		}
	}
	for(exist_lease = comp->pool->active; exist_lease; exist_lease = exist_lease->next){
		if(!memcmp(&(exist_lease->ip_addr.iabuf), &(comp->ip_addr.iabuf), comp->ip_addr.len)){
			log_debug("lease enqueue is %s\n",piaddr(comp->ip_addr));
			return 1;
		}
	}
	for(exist_lease = comp->pool->backup; exist_lease; exist_lease = exist_lease->next){
		if(!memcmp(&(exist_lease->ip_addr.iabuf), &(comp->ip_addr.iabuf), comp->ip_addr.len)){
			log_debug("lease enqueue is %s\n",piaddr(comp->ip_addr));
			return 1;
		}
	}
	for(exist_lease = comp->pool->abandoned; exist_lease; exist_lease = exist_lease->next){
		if(!memcmp(&(exist_lease->ip_addr.iabuf), &(comp->ip_addr.iabuf), comp->ip_addr.len)){
			log_debug("lease enqueue is %s\n",piaddr(comp->ip_addr));
			return 1;
		}
	}
	for(exist_lease = comp->pool->expired; exist_lease; exist_lease = exist_lease->next){
		if(!memcmp(&(exist_lease->ip_addr.iabuf), &(comp->ip_addr.iabuf), comp->ip_addr.len)){
			log_debug("lease enqueue is %s\n",piaddr(comp->ip_addr));
			return 1;
		}
	}
	for(exist_lease = comp->pool->reserved; exist_lease; exist_lease = exist_lease->next){
		if(!memcmp(&(exist_lease->ip_addr.iabuf), &(comp->ip_addr.iabuf), comp->ip_addr.len)){
			log_debug("lease enqueue is %s\n",piaddr(comp->ip_addr));
			return 1;
		}
...
{
	struct lease **lq = NULL, *prev = NULL, *lp = NULL;
	...
	if (!comp -> pool)
		return 0;	
	#if 0
	if(lease_find_in_pool(comp)){
		log_debug("the lease %s have been exist!\n", piaddr(comp->ip_addr));
		return 1;
	}
	#endif
...
#ifndef __AX_PLATFORM__
	last_lq = comp->pool->last_lq;
	last_insert_point = comp->pool->last_insert_point;
#endif
	...
	if ((server_starting & SS_QFOLLOW) && (lq == last_lq) &&
		...
		lp = prev->next;
	} else if ((!add_range_flag) && dhcp_optimize_enable && (lq == last_lq) &&
	    (comp != last_insert_point) && 
	    (last_insert_point->sort_time <= comp->sort_time)) {
		prev = last_insert_point;
		lp = prev->next;
...
//		log_debug("prev is %s\n", piaddr(prev->ip_addr));
	...
	if (prev) {
		...
		log_debug("prev %s , comp is %s, next is %s\n",piaddr(prev->ip_addr), piaddr(comp->ip_addr), 
															 (comp->next)? piaddr(comp->next->ip_addr):"null\n");
	} else {
		if (*lq) {
			lease_reference (&comp -> next, *lq, MDL);
	//		log_debug("lq is %s\n", piaddr(comp->next->ip_addr));
			lease_dereference (lq, MDL);
		}
...
#ifndef __AX_PLATFORM__
	comp->pool->last_insert_point = comp;
	comp->pool->last_lq = lq;
#endif
...
void expire_all_pools ()
{
	...
	for (s = shared_networks; s; s = s -> next) {
	log_debug("@ expire_all_pools shared network name %s \n", s->name);
...
hw_hash_add_test(struct lease *lease)
{
	...
	if (!find_lease_by_hw_addr (&head, lease -> hardware_addr.hbuf,
		...
		log_debug("the lease is already in hw_lease_hash, hw_addr is %s\n", 
...
void expire_pool_test(struct shared_network* share, struct lease *lpchain)
{
	struct shared_network *s;
	struct pool *p;
	int i;
	...
	for (lp = lpchain; lp; ) {
		...
		lease_instantiate_test(NULL, 4, lp);
	//	log_debug(" --> %s <--\n", piaddr(lp->ip_addr));
		lp = next;
	}
...
/** 
  * @brief  check interface is ebr and hansi is current hansi
  * @param  ifname   
  * @return  
  * @author  zhangshu
  * @date  2012/11/13
  */
int dhcp_check_arp_interface(char * ifname, int hansi_id)
{
    log_debug("%s %s %d\n", __FILE__, __func__, __LINE__);
    int ret = 0;
    //char *ptr = NULL;
    log_debug("ifname = %s\n", ifname);
    if (!strncasecmp(ifname,"ebr",3) || !strncasecmp(ifname,"wlan",4)) {
            ret = 1;
    }
    return ret;
}
...
/** 
  * @brief  send arp table to interfaces
  * @return  
  * @author  zhangshu
  * @date  2012/11/13
  */
int dhcp_set_dynamic_arps(char *interface_name, int hansi_id)
{
    log_debug("%s %s %d\n", __FILE__, __func__, __LINE__);
	int count = 0;
	struct subnet *rv = NULL;
	struct pool *next = NULL;
	struct lease *tmp_active = NULL;
	//struct interface_info *subnet_interface = NULL;
	...
	for (rv = subnets; rv; rv = rv->next_subnet) {
		if ((rv->shared_network) && (rv->interface)) {
		    log_debug("subnet_if_name = %s, hansi_if_name = %s\n", rv->interface->name, interface_name);
		    if (!strncmp(rv->interface->name, interface_name, strnlen(interface_name, 16))
		    && (rv->shared_network->pools)) {
				next = rv->shared_network->pools;
				while (next) {				
					for(tmp_active = next->active; tmp_active ; tmp_active = tmp_active->next) {
					    if (tmp_active->subnet && tmp_active->subnet->interface) {
					        if(dhcp_check_arp_interface(tmp_active->subnet->interface->name, hansi_id)) {
                    			ifindex = if_nametoindex(tmp_active->subnet->interface->name);
                    			log_debug("ifindex is %d\n", ifindex);
                    			dhcp_dbus_four_char_to_int((unsigned char *)&(tmp_active->ip_addr.iabuf), &ipaddr);
                    			log_debug("ipaddrs is %d\n", ipaddr);
                    			dhcp_snp_netlink_do_ipneigh_dynamic(DHCPSNP_RTNL_IPNEIGH_ADD_E, ifindex, ipaddr, 
                    									&(tmp_active->hardware_addr.hbuf[1]));	
                    			count++;
                			}
                		}
					}
