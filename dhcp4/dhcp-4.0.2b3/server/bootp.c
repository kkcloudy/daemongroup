	...
	struct packet *packet;
{
	...
	if (!locate_network (packet)) {
		log_debug ("%s: network unknown", msgbuf);
		return;
	}
	...
	if (!lease || ((lease->flags & STATIC_LEASE) == 0)) {
		...
		if (lease == NULL) {
			log_debug("%s: BOOTP from dynamic client and no "
		...
		if (packet->haddr) {
			...
			log_debug("bootp packet->haddr is NULL \n");
		}
#endif
	...
	if ((oc = lookup_option (&server_universe, options, SV_ALLOW_BOOTP)) &&
	    !evaluate_boolean_option_cache (&ignorep, packet, lease,
					    ...
					    &lease -> scope, oc, MDL)) {
		if (!ignorep)
			log_debug ("%s: bootp disallowed", msgbuf);
		goto out;
	} 
	...
	if ((oc = lookup_option (&server_universe,
	    ...
	    !evaluate_boolean_option_cache (&ignorep, packet, lease,
					    ...
					    &lease -> scope, oc, MDL)) {
		if (!ignorep)
			log_debug ("%s: booting disallowed", msgbuf);
		goto out;
	}
