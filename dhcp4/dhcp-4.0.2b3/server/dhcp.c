...
unsigned int unicast_flag = 0;

#ifndef __AX_PLATFORM__

unsigned int discover_times = 0;	/* client discover total times */
...
unsigned int client_lease_time_enable ;
unsigned int dhcp_ping_check_enable = 1;	/* default is enable(1), disable(0) */

unsigned int dhcp_lease_expiry_time = DEFAULT_LEASE_EXPIRY_TIME;	

enum dhcpsnp_rtnl_ipneigh_op;

extern unsigned int dhcp_dbus_four_char_to_int(unsigned char* in_char, unsigned int *out_int);
extern int dhcp_snp_netlink_do_ipneigh
(
...
extern void dhcp_notify_to_trap_event
(
...
int
check_lease_in_static
(
	struct lease* check_lease
)
{
	struct dcli_host* head = head_host.next;
	
	while(head && check_lease) {
		if (addr_eq (head->hostip, check_lease->ip_addr)) {
			log_debug("check_lease_in_static ip %s is in static host list\n", piaddr(check_lease->ip_addr));
			return 1;
		}
		head = head->next;
	}
	...
	int ms_nulltp;
{
	...
	#ifndef __AX_PLATFORM__
	discover_times++;
	log_debug("client send discover total time %d\n", discover_times);
	#endif
	...
	if (!lease) {
		...
		if (lease && lease->ping_state) {
			log_info ("allocate lease %s set ping state.\n", piaddr (lease->ip_addr));
			return;
		}
	}
...
#if 1
	when = cur_time + dhcp_lease_expiry_time;
#else 
	when = cur_time + 120;
#endif
	if (when < lease -> ends)
...
#ifndef __AX_PLATFORM__
	if (packet->haddr) {
		memcpy(&(lease->haddr), packet->haddr, sizeof(struct hardware));
	}
	else {
		log_debug("dhcpdiscover packet->haddr is NULL \n");
	}
#endif
	if (lease->subnet) {
		lease->subnet->discover_times++;
	}
...
#ifndef __AX_PLATFORM__
	requested_times++;
	log_debug("client send dhcp request total times %d\n", requested_times);
#endif
...
/*add for request wrong vlan nak packet*/
	int ret = 0, i;
	struct dcli_subnet* subnode = NULL;
	ret = dhcp_dbus_find_subnet_by_ifname(packet->interface->name, &subnode);	
	struct iaddr* tmpmask = (struct iaddr*)(&(subnode->mask));
	if (!ret) {
		for (i = 0; i < cip.len; i++) { 		
//			log_debug("ifaddr->iabuf[%d] is %d \n", i, ifaddr.iabuf[i]);
//			log_debug("ipaddr->iabuf[%d] is %d \n", i, ipaddr->iabuf[i]);
			if ((cip.iabuf[i] & ((subnode->mask).iabuf[i])) != 
				(subnode->lowip.iabuf[i]&(subnode->mask.iabuf[i]))) {			
				/*when break i not ++*/
				break;
			}
		}
		if (cip.len == i) { 		
			log_debug("dhcprequest run interface name %s\n", packet->interface->name);
		}
		else {		
	...
	if (!packet -> shared_network ||
		...
		//	if (packet -> shared_network -> group -> authoritative)
			...
			log_debug("%s: ignored (not authoritative).", msgbuf);
			return;
		}
	}
	...
	if (lease) {
			...
			if (packet->haddr) {
				...
				log_error("dhcprequest packet->haddr is NULL \n");
			}
		#endif
		...
		if (subnet) {
			subnet->requested_times--;
			log_debug ("unkown lease %s, decrease requested times.\n", piaddr(cip));
		}
	}
	...
	int ms_nulltp;
{
	...
	if (!memcmp (cip.iabuf, "\0\0\0", 4)) {
		log_error("%s: ignored (null source address).", msgbuf);
		return;
	}
	...
	if (zeroed_ciaddr && (gip.len != 0)) {
		...
		if (subnet == NULL) {
			log_error("%s: unknown subnet for relay address %s",
				 msgbuf, piaddr(gip));
			return;
		}
	} else {
		...
		if (subnet == NULL) {
			log_error("%s: unknown subnet for %s address %s",
	...
	if (subnet && !subnet -> group -> authoritative) {
		static int eso = 0;
		log_debug("%s: not authoritative for subnet %s",
	...
	log_debug("%s %s", msgbuf, gip.len ? piaddr(gip) :
					    packet->interface->name);
...
#ifndef __AX_PLATFORM__ /* open by qinhs@autelan.com 2010-5-7 : NAK with unicast */
	if(unicast_flag) {
		hto.hbuf [0] = packet -> raw -> htype;
		hto.hlen = packet -> raw -> hlen;
		memcpy (&hto.hbuf [1], packet -> raw -> chaddr, hto.hlen);
	...
	struct host_decl *hp;
{
	...
	int ignorep;
	unsigned int ipaddr, ifindex;
	int default_flag,min_flag,remain_flag,adap_flag,max_flag;
	
	default_flag=0;

	/* If we're already acking this lease, don't do it again. */
	if (lease -> state) {
		log_info("already ack this lease %s.\n", piaddr(lease->ip_addr));
		if (lease->ack_count++ > 30) {
			free_lease_state(lease->state, MDL);
			lease->state = (struct lease_state *)0;
			abandon_lease (lease, "already ack this lease");			
		}
		return;
	} else {
		lease->ack_count = 0;
	 ...
	 */
#ifndef __AX_PLATFORM__
/* if receive packet don't contain options, packet->options is NULL, so just return */
	if (!packet->options_valid) {
		...
		log_info ("user %s from %s donot contain any option, ignore\n",
			  (packet->raw->hlen
			   ? print_hw_addr (packet->raw->htype, packet->raw->hlen, packet->raw->chaddr)
			   : print_hex_1 (packet->raw->hlen, packet->raw->chaddr, packet->raw->hlen)),
			  (packet->raw->giaddr.s_addr ? inet_ntoa (packet->raw->giaddr) : "(no dhcrelay)"));
		return ;
	}
#endif
	...
	if (offer == DHCPOFFER &&
		...
		if (evaluate_option_cache (&d1, packet, lease,
			...
			if (d1.len &&
				...
				log_debug("%s: configured min-secs value (%d) "
	...
	if (!host &&
	    ...
	    !evaluate_boolean_option_cache (&ignorep,
					    ...
					    &lease -> scope, oc, MDL)) {
		if (!ignorep)
			log_debug ("%s: unknown client", msg);
		free_lease_state (state, MDL);
		if (host)
			host_dereference (&host, MDL);
		return;
	...
	if (!offer &&
	    ...
	    !evaluate_boolean_option_cache (&ignorep,
					    ...
					    &lease -> scope, oc, MDL)) {
		if (!ignorep)
			log_debug ("%s: bootp disallowed", msg);
		free_lease_state (state, MDL);
		if (host)
			host_dereference (&host, MDL);
		return;
	...
	if (oc &&
	    !evaluate_boolean_option_cache (&ignorep,
					    ...
					    &lease -> scope, oc, MDL)) {
		if (!ignorep)
			log_debug ("%s: booting disallowed", msg);
		free_lease_state (state, MDL);
		if (host)
			host_dereference (&host, MDL);
		log_debug("reject bootp protocol\n");
		return;
	}
	...
	if (have_billing_classes && !(lease -> flags & STATIC_LEASE)) {
		...
		if (lease->billing_class == NULL) {
			...
			if (bill != 0 && i == packet->class_count) {
				log_debug("%s: no available billing: lease "
	...
	if (result != ISC_R_SUCCESS) {
		log_error("%s: can't allocate temporary lease structure: %s",
	...
	if (offer) {
		...
		if (client_lease_time_enable && (oc = lookup_option (&dhcp_universe, packet -> options,
				 DHO_DHCP_LEASE_TIME)))
		...
		if (s1)
			data_string_forget(&d1, MDL);
		if(ASN){if (packet->packet_type == DHCPREQUEST &&
			    packet->raw->ciaddr.s_addr ){
					if((oc = lookup_option (&dhcp_universe, packet -> options,
							    DHO_DHCP_CLIENT_IDENTIFIER) ) && (lease->uid_len))
					...
					lease_time = DEFAULT_MIN_LEASE_TIME;	
				}
			}
		...
		if (lease->pool &&
			...
			if (d1.len == 1 && d1.data[0] > 0 &&
				...
				if (poolfilled >= d1.data[0] &&
					...
					log_debug("Pool over threshold, time "
		...
		if (lease -> pool && lease -> pool -> failover_peer) {
			 ...
			 */
			if (offer == DHCPACK) {
				if(ASN){if(default_lease_time && default_flag){
					lease_time=default_lease_time;
					}else if(min_flag && min_lease_time){
						lease_time=min_lease_time;	
						}else if(remain_flag && remaining_time){
								lease_time=remaining_time;	
								}else if(adap_flag && adaptive_time){
										lease_time = adaptive_time;	
										}else if(max_flag && max_lease_time){
												lease_time = max_lease_time;	
												}else{
													lease_time = default_lease_time;	
													}
				 ...
				 * ack'd.
				 */
				/* zhangshu add for failover not_cooperating state. 2012-10-18 */
				if (lt->tstp < lt->tsfp) {
					lt->tsfp = lt->tstp;
				} else if((peer->me.state == partner_down)
				    || (peer->me.state == resolution_interrupted)
				    || (peer->me.state == conflict_done)
				    || (peer->me.state == communications_interrupted)
				    || (lt->tsfp <= cur_time)) {
                    lt->tsfp = lt->tstp;
                }
			} else
	...
	if (offer == DHCPOFFER && !(lease -> flags & STATIC_LEASE) &&
	    ...
	    (!(oc = lookup_option (&server_universe, state -> options,
	     ...
	     evaluate_boolean_option_cache (&ignorep, packet, lease,
					    ...
					    packet -> options,
					    state -> options,
					    &lease -> scope, oc, MDL)) && dhcp_ping_check_enable) {
...
#endif
		log_debug ("Ping timeout: %ld, lease %s\n", (long)ping_timeout, piaddr(lease->ip_addr));
	...
	if (lease->pool) {		
		if (!(lease->pool->free_leases) &&
			...
			lease->pool->pool_full = 1;
			dhcp_notify_to_trap_event(1, lease->pool->lease_count, lease->pool->free_leases, lease->subnet);
		}
	}
	
	if (dhcp_server_static_arp_enable) {
		if ((offer == DHCPACK) && lease && lease->subnet && lease->subnet->interface) {	
			ifindex = if_nametoindex(lease->subnet->interface->name);
			log_debug("DHCPACK ifindex is %d\n", ifindex);
			dhcp_dbus_four_char_to_int((unsigned char *)&(lease->ip_addr.iabuf), &ipaddr);
			log_debug("DHCPACK ipaddrs is %d\n", ipaddr);
			dhcp_snp_netlink_do_ipneigh(0, ifindex, ipaddr, 
	...
	struct lease *lease;
{
	...
	if(state->offer == DHCPACK)	{	
		response_times++;
		if(lease->subnet){
			lease->subnet->response_times++;				
		}
		log_debug("dhcp server send ack total times %d\n", response_times);		
	}
	
	if(state->offer == DHCPOFFER) {	
		offer_times++;
		if(lease->subnet){
			lease->subnet->offer_times++;				
		}
	...
	if(DHCPACK == state->offer) {
		log_info ("user %s %s %s%s%sfrom %s logon\n",
			  (lease -> hardware_addr.hlen
			   ? print_hw_addr (lease -> hardware_addr.hbuf [0],
						lease -> hardware_addr.hlen - 1,
						&lease -> hardware_addr.hbuf [1])
	...
	*/
	hto.hlen = lease -> haddr.hlen;
	memcpy (hto.hbuf, lease -> haddr.hbuf, hto.hlen);
#endif
	...
	/* Otherwise, broadcast it on the local network. */
		...
		if (!unicast_flag){	//The default mode is  broadcast reply 
	...
	log_debug("dhcp relay %s to shost mac is %02x:%02x:%02x:%02x:%02x:%02x \n",
			unicastp ? "ucast":"bcast",
			hto.hbuf[1], hto.hbuf[2], hto.hbuf[3], hto.hbuf[4], hto.hbuf[5], hto.hbuf[6]);
		...
		const char *file, int line)
{
...
#if defined (DEBUG_FIND_LEASE)
		if (fixed_lease) {
			log_debug("Found host for client identifier: %s.",
...
#if defined (DEBUG_FIND_LEASE)
			if (fixed_lease) {
				log_debug("Found host for link address: %s.",
				      piaddr (fixed_lease -> ip_addr));
			}
#endif
		}
    ...
    /* zhangshu add for checking fixed_lease's bind state */
	if(fixed_lease && (fixed_lease -> binding_state == FTS_ACTIVE)) {
	    lease_dereference (&fixed_lease, MDL);
	    log_info("find a fixed_lease but it is already in use.\n");
	}
...
#if defined (DEBUG_FIND_LEASE)
		log_debug("Client's fixed-address %s doesn't match %s%s",
	...
	while (uid_lease) {
#if defined (DEBUG_FIND_LEASE)
		log_debug("trying next lease matching client id: %s",
			  piaddr (uid_lease -> ip_addr));
#endif

//#if defined (FAILOVER_PROTOCOL)
if(packet->interface->vgateway_flag){
...
#if defined (DEBUG_FIND_LEASE)
			log_error("not mine to allocate: %s",
				  piaddr (uid_lease -> ip_addr));
#endif
			goto n_uid;
		}
...
#if defined (DEBUG_FIND_LEASE)
			log_error("wrong network segment uid_lease: %s, share name %s\n",
				  piaddr (uid_lease -> ip_addr), share->name);
#endif
			goto n_uid;
		}
...
#if defined (DEBUG_FIND_LEASE)
	if (uid_lease)
		log_debug("Found lease for client id: %s.",
		      piaddr (uid_lease -> ip_addr));
#endif
	...
	while (hw_lease) {
#if defined (DEBUG_FIND_LEASE)
		log_debug("trying next lease matching hw addr: %s",
...
#if defined (DEBUG_FIND_LEASE)
			log_debug ("not mine to allocate: %s",
				  piaddr (hw_lease -> ip_addr));
#endif
			goto n_hw;
		}
		...
		if (hw_lease -> binding_state != FTS_FREE &&
		    ...
		    (!have_client_identifier ||
		     ...
		     memcmp (hw_lease -> uid, client_identifier.data,
			     hw_lease -> uid_len))) {
#if defined (DEBUG_FIND_LEASE)
			log_error("wrong client identifier: %s",
				  piaddr (hw_lease -> ip_addr));
#endif
			goto n_hw;
			continue;
...
#if defined (DEBUG_FIND_LEASE)
			log_error("wrong network segment: %s",
				  piaddr (hw_lease -> ip_addr));
#endif
			goto n_hw;
			continue;
		...
		if ((hw_lease -> pool -> prohibit_list &&
			...
			if (hw_lease -> n_hw) {
#if defined (DEBUG_FIND_LEASE)
				log_debug("hw_lease->n_hw is exist\n");
#endif				
				lease_reference (&next, hw_lease -> n_hw, MDL);
#if defined (DEBUG_FIND_LEASE)
				log_debug("hw_lease next lease ip is : %s", piaddr(next->ip_addr));
#endif	
			}
...
#if defined (DEBUG_FIND_LEASE)
	if (hw_lease)
		log_debug("Found lease for hardware address: %s.",
		      piaddr (hw_lease -> ip_addr));
#endif
	...
	if (ip_lease_in)
		...
		log_debug("find ip from find_lease_by_ip_addr \n");
		find_lease_by_ip_addr (&ip_lease, cip, MDL);
	}
#if defined (DEBUG_FIND_LEASE)
	if (ip_lease)
		log_debug("Found lease for requested address: %s.",
	...
	if (ip_lease && (ip_lease -> subnet -> shared_network != share)) {
		if (ours)
			*ours = 1;
#if defined (DEBUG_FIND_LEASE)
		log_debug("...but it was on the wrong shared network.");
#endif
...
#if defined (DEBUG_FIND_LEASE)
			log_debug("rejecting lease for requested address.");
#endif
...
#if defined (DEBUG_FIND_LEASE)
		log_debug("ip lease not ours to offer.");
#endif
		lease_dereference (&ip_lease, MDL);
	}
...
#if defined (DEBUG_FIND_LEASE)
		log_debug("hardware lease and uid lease are identical.");
#endif
		lease_dereference (&hw_lease, MDL);
	}
	if (ip_lease && ip_lease == hw_lease) {
		lease_dereference (&hw_lease, MDL);
#if defined (DEBUG_FIND_LEASE)
		log_debug ("hardware lease and ip lease are identical.");
#endif
	}
	if (ip_lease && ip_lease == uid_lease) {
		lease_dereference (&uid_lease, MDL);
#if defined (DEBUG_FIND_LEASE)
		log_debug ("uid lease and ip lease are identical.");
#endif
	}
...
#if defined (DEBUG_FIND_LEASE)
		log_debug ("no applicable lease found for DHCPREQUEST.");
#endif
		goto out;
	}
	...
	if (fixed_lease) {
		...
		lease_dereference (&fixed_lease, MDL);
#if defined (DEBUG_FIND_LEASE)
		log_debug ("choosing fixed address.");
#endif
	}
	...
	if (ip_lease) {
		if (lease) {
			if (!packet -> raw -> ciaddr.s_addr)
				release_lease (ip_lease, packet);
#if defined (DEBUG_FIND_LEASE)
			log_debug ("not choosing requested address (!).");
#endif
		} else {
#if defined (DEBUG_FIND_LEASE)
			log_debug ("choosing lease on requested address.");
#endif
	...
	if (uid_lease) {
		if (lease) {
			...
			if (!packet -> raw -> ciaddr.s_addr &&
				...
				release_lease(uid_lease, packet);
#if defined (DEBUG_FIND_LEASE)
			log_debug ("not choosing uid lease.");
#endif
		} else {
	...
	if (hw_lease) {
		if (lease) {
#if defined (DEBUG_FIND_LEASE)
			log_debug ("not choosing hardware lease.");
#endif
		} else {
			...
			if (!hw_lease -> uid_len ||
				...
				if (lease -> host)
					host_dereference (&lease -> host, MDL);
#if defined (DEBUG_FIND_LEASE)
				log_debug ("choosing hardware lease.");
#endif
			} else {
#if defined (DEBUG_FIND_LEASE)
				log_debug ("not choosing hardware lease: %s.",
					  "uid mismatch");
#endif
			}
		}
...
#if defined (DEBUG_FIND_LEASE)
	log_debug ("Not returning a lease.");
#endif
	return 0;
}
...
#ifndef __AX_PLATFORM__
/*for static host do not allocate*/
		if (!lease) {
			lease = candl;		
			log_debug("allocate_lease a lease ip %s\n", piaddr(lease->ip_addr));
		check:	
			ret = check_lease_in_static(lease);
			if (ret)  {
				if (lease->next) {
					lease = lease->next;
					goto check;
		...
		if (option_cache_allocate(&oc, MDL)) {
			...
			if (make_const_data(&oc->expression,
				...
				option_code_hash_lookup(&oc->option, 
							...
							&option_num, 0, MDL);
				save_option(&dhcp_universe, options, oc);
				log_debug("  save option DHO_DHCP_SERVER_IDENTIFIER(54) %s\n", inet_ntoa(*a));
			}
			option_cache_dereference(&oc, MDL);
		}
...
#if defined (FAILOVER_PROTOCOL)	
	if(packet->interface->vgateway_flag){
		log_debug("  interface %s virtual ip %s\n", packet->interface->name, inet_ntoa(packet->interface->virtual_ip));
		*from = packet->interface->virtual_ip;
	}else{
		log_debug("  interface %s ip %s\n", packet->interface->name, inet_ntoa(packet->interface->addresses[0]));
		*from = packet->interface->addresses[0];
	}
#endif 
