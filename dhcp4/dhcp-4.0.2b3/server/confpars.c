	...
	int declaration;
{
	...
	switch (token) {
		...
		return 1;
	      ...
	      case HOST:
		next_token (&val, (unsigned *)0, cfile);
log_debug("HOST next_token val is %s, type is %d\n", val, type);			
		...
		return 1;
	      ...
	      case HARDWARE:
		next_token (&val, (unsigned *)0, cfile);
log_debug("HARDWARE next_token %s\n", val);					
#ifdef DHCPv6
...
log_debug("parse statement HARDWARE hlen is %d\n", hardware.hlen);		
		if (host_decl)
...
log_debug("FIXED_ADDR next_token %s\n", val); 				
		...
		goto finish_option;
	      ...
	      case OPTION:
		token = next_token (&val, (unsigned *)0, cfile);
log_debug("next token is %d val is %s\n", token, val);	
		token = peek_token (&val, (unsigned *)0, cfile);
log_debug("peek token is %d val is %s\n", token, val);	
		if (token == SPACE) {
			if (type != ROOT_GROUP) {
				parse_warn (cfile,
...
log_debug("peek token is %d val is %s\n", token, val);	
		...
		if (!et)
			return declaration;
	      insert_statement:
log_debug("option add statement\n");  		  	
		if (group -> statements) {
log_debug("option add statement group\n");					
			int multi = 0;
...
int 
parse_failover_peer_no_file
(
	struct dcli_failover *conf,
	struct group *group,
	int type
)
{
	...
	memset (hba, 0, sizeof(u_int8_t)*32);
...
/*
	...
	*/
	/* Make sure this isn't a redeclaration. */
//	if (peer) {
//		log_error ("redeclaration of failover peer %s \n", name);
//		dhcp_failover_state_dereference (&peer, MDL);
//		return 2;
//	}
	...
	do {
		...
		log_debug_failover(DEBUG_TYPE_CONNECT,"%s failover %s state is %s\n", __func__, peer->name , peer->i_am ? "secondary" : "primary");
	     ...
	     /* case ADDRESS:*/
		expr = (struct expression *)0;
		make_const_data (&expr, (const unsigned char *)(conf->myaddr.iabuf), conf->myaddr.len, 0, 1, MDL);

		option_cache (&(peer->me.address),
			      (struct data_string *)0, expr,
			      (struct option *)0, MDL);
		expression_dereference (&expr, MDL);
		
		expr = (struct expression *)0;
		make_const_data (&expr, (const unsigned char *)(conf->parteraddr.iabuf), conf->parteraddr.len, 0, 1, MDL);

		option_cache (&(peer->partner.address),
			      (struct data_string *)0, expr,
			      (struct option *)0, MDL);
		expression_dereference (&expr, MDL);
	      ...
	      /*case PORT:*/
		peer->me.port = conf->myport;
		peer->partner.port = conf->parterport;
		
		  /*case MAX_RESPONSE_DELAY: 	
		if (conf->max_response_delay) {
			peer->me.max_response_delay = conf->max_response_delay;
		}*/

	      /*case MAX_UNACKED_UPDATES:
		if (conf->max_flying_updates) {
			peer->me.max_flying_updates = conf->max_flying_updates;
		}*/
		peer->me.max_response_delay = 30;
		peer->me.max_flying_updates = 10;
		peer->load_balance_max_secs = 3;

		
	     /* case SPLIT: secondary not need split*/
		if (!conf->primary) {
			 /*case MCLT:*/
			peer->mclt = conf->mclt;
...
*/	
	 ...
	 */
	if (!peer->partner.address) {
		log_error ("peer address may not be omitted\n");
	}
	/* XXX - when/if we get a port number assigned, just set as default */
	if (!peer->me.port) {
		log_error ("local port may not be omitted \n");
	}
	if (!peer->partner.port) {
		log_error ("peer port may not be omitted \n");
	}
	if (peer->i_am == primary) {
	    if (!peer -> hba) {
			log_error ("primary failover server must have hba or split.\n");
	    } 
		else if (!peer->mclt) {
			log_error ("primary failover server must have mclt. \n");
	    }
	}
...
log_debug("parse_host_declaration group  %d\n", root_group->authoritative);		
log_debug("parse_host_declaration group  %d\n", group->authoritative);		
log_debug("parse_host_declaration host name %s\n", name);		
	...
	do {
		token = peek_token (&val, (unsigned *)0, cfile);
log_debug("parse_host_declaration 1 peek_token token is %d, val is %s\n", token, val);		
		...
		if (token == UID) {
			...
			token = next_token (&val, (unsigned *)0, cfile);
log_debug("parse_host_declaration 1 next_token token is %d, val is %s\n", token, val);		
			...
			token = peek_token (&val, (unsigned *)0, cfile);
log_debug("parse_host_declaration 2 peek_token token is %d, val is %s\n", token, val);		
			
			if (token == STRING) {
				token = next_token (&val, &len, cfile);
log_debug("parse_host_declaration 2 next_token token is %d, val is %s\n", token, val);		
...
log_debug("parse_host_declaration 4 next_token token is %d, val is %s\n", token, val);		
	...
	struct group *group;
{
	...
	if (status != ISC_R_SUCCESS)
		log_fatal ("Can't allocate shared subnet: %s",
			   isc_result_totext (status));
	if(clone_group (&share -> group, group, MDL)){
		log_debug("clone group success!\n");
	}else{
		log_debug("clone group failed!\n");
	}
	shared_network_reference (&share -> group -> shared_network,
				  share, MDL);
...
) 
{
	...
	tmp = NULL;
	if (type == FIXED_ADDR) {
		make_const_data (&tmp, addr, len, 0, 1, MDL);
...
)
{
	...
	/* If we're in a subnet declaration, just do the parse. */
	if (group->shared_network) {
		parse_subnet_declaration_no_cfile(sub_conf, group->shared_network);
		log_debug("we're in a subnet declaration, just do the parse\n");
		return 1;
	}
...
{
	struct pool *pool = NULL, **p = NULL, *pp = NULL;
	isc_result_t status;
	struct lease *lpchain = (struct lease *)0, *lp;
	...
	for (pp = pool->shared_network ->pools; pp; pp = pp->next) {		
		...
		/* Okay, we can merge these two pools.    All we have to
		   do is fix up the leases, which all point to their pool. */
		for (lp = lpchain; lp; lp = lp->next) {
			log_debug("&& merge lease is %s\n",  piaddr(lpchain->ip_addr));
			pool_dereference (&lp->pool, MDL);
			pool_reference (&lp->pool, pp, MDL);			
		}
	...
	expire_pool_test(pool->shared_network, lpchain);
	/*
	debug_print_share_lease(pool->shared_network); */
...
#if 0
void debug_print_share_lease(struct shared_network *share)
{
	struct lease *lp = NULL;
	struct pool *p = NULL;

	if (share && share->pools) {
		p = share->pools;
	} else {
		return;
	}
	...
	for (; p; p = p->next) {
		log_debug(" pool %#x\n", p);
		for (lp = p->free; lp; lp = lp->next) {
			log_debug("  -->lease %s\n", piaddr(lp->ip_addr));
		}
	}
}

void debug_print_pool(struct shared_network *share)
{
	log_debug("=======================================\n");
	struct pool *tmp = share->pools;
	while (tmp) {
		log_debug(" --> pool %#x \n", tmp);
		tmp = tmp->next;
	}
