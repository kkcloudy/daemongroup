...
pthread_mutex_t DhcpthreadMutex = PTHREAD_MUTEX_INITIALIZER;
...
extern char default_vendor_class_identifier[DHCP_OPTION60_ID_DEFAULT_LENGTH];
extern void dhcp_dbus_start(void);
extern int dhcp_failover_sockfd;
extern int (*dhcp_failover_state_change_hook)(int);
extern int dhcp_failover_state_change(int sockfd);


void set_dhcp_startup_state(int flag);
int init_dhcp_failover_sock(void);

extern int dhcp_server_sock_init(unsigned int port);
#endif
...
//const char *path_dhcpd_db = "/mnt/dhcpd.leases";
...
int read_default_vendor_class_identifier(void);
...
main(int argc, char **argv) {
	...
	/*open debug_error and debug_info*/
	dhcp_log_level |= DEBUG_TYPE_INFO;
	dhcp_log_level |= DEBUG_TYPE_ERROR;
	log_debug("init is dhcp_log_level is %d\n", dhcp_log_level);
	/*open nak to client  whose requested ip is not same with the interface immedietely when receive request */
		dhcp_nak_rsp=nak_enable;
	/*prevent  attack */
		ASN=ASN_DIABLE;
...
#ifndef __AX_PLATFORM__
	system("mkdir /var/lib/dhcp4");
	system("touch /var/lib/dhcp4/dhcpd.leases");
	system("chmod 777 /var/lib/dhcp4/dhcpd.leases");
	system(">/var/lib/dhcp4/dhcpd.leases");
	read_default_vendor_class_identifier();
#endif
	...
	log_debug("initialize dhcp common options...\n");
	initialize_common_option_spaces ();
	log_debug("initialize dhcp server options...\n");
	...
	log_debug("attaching interface and packet handler...\n");
...
/*test for del this handle*/
	...
	log_debug("initialize dhcp server interface...\n");
...
#ifndef __AX_PLATFORM__
	/* before dbus */
	init_dhcp_failover_sock();

	/* listen bootps and bootpc port, to avoid udp port 67 unreachable  */
	bootps_sock = dhcp_server_sock_init(local_port);
	log_info("listern udp port bootps(%d) %s\n", 
		local_port, (bootps_sock > 0) ? "." : "failed.");

	bootpc_sock = dhcp_server_sock_init(remote_port);
	log_info("listern udp port bootps(%d) %s\n", 
		remote_port, (bootpc_sock > 0) ? "." : "failed.");
	
	dhcp_dbus_start();
#if 0
/* boot stage check */
	/*dhcp_server_enable = 1;*/
	int boot_state = DEVICE_STARTUP;
	do {
		sleep(2);		
		boot_state = dhcp_check_boot_status();
	} while (DEVICE_STARTUP == boot_state);	
#endif	
#endif

	/* Receive packets and dispatch them... */
	log_debug("start packet scheduling...\n");	
	dispatch ();
...
void postconf_initialization (int quiet)
{
	...
	log_debug("%-slease file name %s...", "post conf initialize",path_dhcpd_db);
	...
	log_debug("%-4spid file name %s...", "",path_dhcpd_db);
#ifdef DHCPv6
        if (local_family == AF_INET6) {
                /*
	...
	log_debug("%-4somapi port %d...", "",omapi_port);
	...
	log_debug("%-4slocal port %d...", "", local_port);
	...
	log_debug("%-4sremote port %d...", "", remote_port);
	...
	log_debug("%-4slimited bcast %s...", "", inet_ntoa(limited_broadcast));
	...
	log_debug("%-4slocal addr %s...", "", inet_ntoa(local_address));
	...
	log_debug("%-4supdate style %d...", "", ddns_update_style);
	...
	int length;
{
	...
	if (!lp -> state) {
		...
		#ifndef __AX_PLATFORM__
		if (lp->ping_state) {
			/* if receive icmp echo reply, clear ping_state flag */
			lp->ping_state = 0;		/* 0 : false */
...
#ifndef __AX_PLATFORM__
void lease_ping_next(struct lease *lp)
{
	struct lease *lease = NULL;

	log_debug("lease %s ping next\n", lp ? piaddr(lp->ip_addr) : "");
	
	if (lp && lp->pool && lp->pool->free) {

		lease = lp->pool->free;
		lease->ping_state = 1;	/* set ping_state flag */
		log_debug("ping next lease %s\n", piaddr(lease->ip_addr));
		icmp_echorequest(&lease->ip_addr);
		
		/* add next free lease ping-check to timeout list */
		add_timeout (cur_time + DEFAULT_PING_TIMEOUT, lease_ping_timeout, lease,
...
#endif
	log_debug("lease %s ping timeout, ping state %d\n", piaddr(lp->ip_addr), lp->ping_state);
	--outstanding_pings;
...
{ 
	FILE *fp = NULL;
	memset(default_vendor_class_identifier, 0, sizeof(default_vendor_class_identifier));
	if (NULL == (fp = fopen(VENDOR_CLASS_IDENTIFIER_FILE, "r"))) {
		log_error("open %s failed %m\n", VENDOR_CLASS_IDENTIFIER_FILE);
		return -1;
	}
	if (fscanf(fp, "%s", default_vendor_class_identifier)) {
		log_info("default VCI is %s\n", default_vendor_class_identifier);
	}
	fclose(fp);
	return 0;
}
int dhcp_check_boot_status(void)
{	
	static int dhcp_boot_state_fd = -1;
	int state = DEVICE_STARTUP;
	char buf[16] = {0};
	if (dhcp_boot_state_fd < 0) {
		if ((dhcp_boot_state_fd = open(DHCP_SYSTEM_STARTUP_STATE_PATH, O_RDONLY)) < 0) {
			log_error("open file %s failed %m\n", DHCP_SYSTEM_STARTUP_STATE_PATH);
			return DEVICE_STARTUP;
		}
	}
    lseek(dhcp_boot_state_fd, 0L, SEEK_SET);
    read(dhcp_boot_state_fd, buf, sizeof(buf));

	if (DHCP_SYSTEM_STARTUP_STATE_FLAG == buf[0]) {
		state = DEVICE_NORMAL;	/* work normal */
		log_info("dhcp got device state normal work\n");
	    close(dhcp_boot_state_fd);
		dhcp_boot_state_fd = -1;
	} else {
		state = DEVICE_STARTUP;	/* load config stage */
		log_info("device boot state, waiting...\n");
	}
    return state;
}
void set_dhcp_startup_state(int flag)
{
	...
	again: 
	fd = open(DHCP_STARTUP_STATE_PATH , O_WRONLY | O_TRUNC | O_CREAT);
	if (fd < 0) {
		log_error("open filed %s failed: %m\n", DHCP_STARTUP_STATE_PATH);
		if (count++ < 5) {
			goto again;
		} else {
			log_error("open filed %s failed %d times\n", DHCP_STARTUP_STATE_PATH, count);		
			return;
		}		
	}
	...
	if (DHCP_NORMAL == flag)  {
		buf[0] = DHCP_NOMARL_STATE_FLAG;	/* dbus start, work normal */
	} else {
		buf[0] = DHCP_NOMARL_STATE_FLAG - 1;	/* set state, MUST wait for dhcp_dbus startup */
	}
	buf[1] = '\n';
	
	if (strlen(buf) != write(fd, buf, strlen(buf))) {
		log_error("write file %s(%c) failed: %m\n", DHCP_STARTUP_STATE_PATH, buf[0]);		
	}

	close(fd);
}

/* if command line restart dhcp(ip dhcp server restart), return 1 */
int dhcp_check_manual_restart(void)
{	
	int fd = -1;
	int flag = 0;	/* flag of manual restart */
	char buf[16];
	
	if ((fd = open(DHCP_MANUAL_RESTART_FLAG_PATH, O_RDONLY)) < 0) {
		log_error("open file %s failed: %m\n", DHCP_MANUAL_RESTART_FLAG_PATH);
		return 0;
	}
	...
	if (DHCP_MANUAL_RESTART_FLAG == buf[0]) {
		flag = 1;	/* manual restart */
	} else {
		flag = 0;
	}
	if(fd){
...
int init_sock_unix_server(char *path)
{
	...
	if (-1 == bind(serv_fd, (struct sockaddr*)&sock, sizeof(sock))) {
		log_error("init unix sock: sock bind error : %m\n");
        close(serv_fd);
		serv_fd = -1;
        unlink(path);
        return -1;   
...
int init_dhcp_failover_sock(void)
{
	...
	dhcp_failover_sockfd = init_sock_unix_server(DHCP_FAILOVER_SOCK_PATH);
	if (dhcp_failover_sockfd < 0) {
		log_error("init dhcp failover socket failed.\n");
		return -1;
	}
	dhcp_failover_state_change_hook = dhcp_failover_state_change;

	log_info("init dhcp failover socket(%s) succuess! socket fd %d\n", 
								DHCP_FAILOVER_SOCK_PATH, dhcp_failover_sockfd);
