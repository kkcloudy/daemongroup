...
extern int dhcp_dynamic_arp_switch;
long int system_time_difference = 0;
...
#define SYSTEM_MAX_TIME_OFFSET		(60 * 5)
...
log_info("end dhcp failover startup \n");	
}

#ifndef __AX_PLATFORM__
void 
dhcp_failover_startup_one
(
...
)
{
	...
	isc_result_t status;
	for (state = failover_states; state; state = state->next) {
		/* for a special state by name*/
		if (strcmp(state->name, failover_peer->name)) {
			continue;
		}		
		if(state -> link_to_peer){
			if(state -> link_to_peer->outer){
				omapi_disconnect ((state -> link_to_peer->outer) , 1);
			}
		}else{
			omapi_disconnect ((omapi_object_t *)state,1);
		}

		/* zhangshu add for reset curent update state */
		state->curUPD = 0;
		...
		/* In case the peer is already running, immediately try
		   to establish a connection with it. */
		   /*when primary, listen the socket*/
		if (state->i_am) {			
			status = dhcp_failover_link_initiate ((omapi_object_t *)state);
			if (status != ISC_R_SUCCESS && status != ISC_R_INCOMPLETE) {
#if defined (DEBUG_FAILOVER_TIMING)
...
log_info("end dhcp failover startup one \n");	
}
#endif
...
unsigned dhcp_failover_change_state(char *name, unsigned int state)
{
	u_int8_t hba [32] = {0};
	dhcp_failover_state_t *p = NULL;
	int i = 0;
	...
	if(p){		
		...
		if(primary == p->i_am){	/* secondary change to primary (if primary, must have hba) */
			...
			if (NULL == p->hba) {
				p->hba = dmalloc (32, MDL);
				if (!p->hba) {
					log_error("%s: %s malloc hba failed\n", __func__, p->name);
					dfree (p->name, MDL);
					dfree (p, MDL);
					return 1;
				}
...
isc_result_t dhcp_failover_link_initiate (omapi_object_t *h)
{
	...
	log_info("dhcp failover link initiate......\n");	
...
# endif
		...
		if (link -> imsg -> type == FTM_CONNECT) {
		    ...
		    if (!state) {
			  ...
			  badconnect:
			    ...
			    log_error("Failover CONNECT from %.*s: %s",
				      slen, sname, errmsg);
				log_info("..local time: %s (%d)\n", asctime(localtime(&cur_time)), (int)cur_time);
				log_info("..peer time : %s (%d)\n", asctime(localtime((time_t *)&link->imsg->time)), (int)link->imsg->time);
		    ...
		    if ((cur_time > link -> imsg -> time &&
			 cur_time - link -> imsg -> time > SYSTEM_MAX_TIME_OFFSET) ||
			(cur_time < link -> imsg -> time &&
			 link -> imsg -> time - cur_time > SYSTEM_MAX_TIME_OFFSET)) {
			    errmsg = "time offset too large";
			    reason = FTR_TIMEMISMATCH;
			    goto badconnect;
		    }*/
		    if(cur_time != link -> imsg -> time){
				system_time_difference=cur_time - link -> imsg -> time;
			}
...
/*	 See if we can find a failover_state object that
	...
	struct iaddr addr;
	addr.len = 4;
	memcpy(addr.iabuf, &(c->local_addr.sin_addr.s_addr), addr.len);
	log_debug("local ip is %x.%x.%x.%x \n", addr.iabuf[0], addr.iabuf[1], addr.iabuf[2], addr.iabuf[3]);

	memcpy(addr.iabuf, &(c->remote_addr.sin_addr.s_addr), addr.len);
	log_debug("remote ip is %x.%x.%x.%x \n", addr.iabuf[0], addr.iabuf[1], addr.iabuf[2], addr.iabuf[3]);
	*/
	if (!state) {
	...
	log_info("dhcp_failover_listener_signal  call omapi_signal_in for connect\n");
					 ...
					 const char *name, va_list ap)
{
	...
	if (!strcmp (name, "disconnect")) {
		...
		log_info("$failover %s state is %s\n", state->name,  state -> i_am? "seconary" : "primary");		
		...
		if (link -> imsg -> type == FTM_CONNECT) {
			...
			if (state -> link_to_peer) {
				...
				log_debug("(1)link->imsg address %p\n",link->imsg);
				log_debug("(1)link->imsg->refcnt %d\n",link->imsg->refcnt);
				return ISC_R_SUCCESS;
			}
			if (!(link -> imsg -> options_present & FTB_MCLT)) {
				dhcp_failover_send_connectack
					((omapi_object_t *)link, state,
					 ...
					 "no MCLT provided");
				omapi_disconnect (link -> outer, 1);
				log_debug("(2)link->imsg address %p\n",link->imsg);
				log_debug("(2)link->imsg->refcnt %d\n",link->imsg->refcnt);
				return ISC_R_SUCCESS;
			}				
			...
			if (status != ISC_R_SUCCESS) {
				...
				log_info ("dhcp_failover_send_connectack: %s",
					  isc_result_totext (status));
				omapi_disconnect (link -> outer, 1);
				log_debug("(3)link->imsg address %p\n",link->imsg);
				log_debug("(3)link->imsg->refcnt %d\n",link->imsg->refcnt);
				return ISC_R_SUCCESS;
			}
			...
			omapi_disconnect (link -> outer, 1);
			log_debug("(4)link->imsg address %p\n",link->imsg);
			log_debug("(4)link->imsg->refcnt %d\n",link->imsg->refcnt);
			return ISC_R_SUCCESS;
		    }
			...
			log_debug("(5)link->imsg address %p\n",link->imsg);
			log_debug("(5)link->imsg->refcnt %d\n",link->imsg->refcnt);
			return ISC_R_SUCCESS;
		    }
		    ...
		    omapi_disconnect (link -> outer, 1);
			log_debug("(6)link->imsg address %p\n",link->imsg);
			log_debug("(6)link->imsg->refcnt %d\n",link->imsg->refcnt);
...
/** 
  ...
  */
/*
static int dhcp_failover_get_vrrid(char *name)
{
				      ...
				      enum failover_state new_state)
{
	...
	/*state will enter into shutdown for some illegal reasons.
	* Inorder to recover ,making the "saved_state" "recover"
	*/
    state -> me.state = new_state;
    if (new_state == startup && saved_state != startup)
	state -> saved_state = saved_state;
	if(new_state == startup && saved_state == shut_down)
	state -> saved_state = recover;
    ...
    switch (new_state) {
        /* zhangshu add from 4.2.4-p2, 2012.10.10 */
	  case communications_interrupted:
	    /*
	     ...
	     */
	    state->auto_partner_down = (state->mclt ? state->mclt : 60);
...
#endif
     	if(state -> name)   
	    add_timeout(cur_time + state->auto_partner_down, dhcp_failover_auto_partner_down, state,
			(tvref_t)omapi_object_reference,
			(tvunref_t)omapi_object_dereference);
	    break;
	  ...
	  case potential_conflict:
		...
		else if(state -> i_am == secondary && state->partner.state == conflict_done){
			dhcp_failover_send_update_request (state);
			}
	    break;
					       ...
					       failover_message_t *msg)
{
	...
	log_debug("(13)link->imsg address %p\n",msg);
	log_debug("(13)link->msg->refcnt %d\n",msg->refcnt);
	...
	switch (state -> me.state == startup ?
		...
		switch (new_state) {
			...
			break;
		      ...
		      default:
		      /* book modify fatal to err, 2012-10-19 */
			log_error("Peer %s: Invalid attempt to move from %s "
...
int dhcp_failover_queue_update (struct lease *lease, int immediate)
{
	...
	if (!lease -> pool ||
	    ...
	    if(!lease -> pool){
			log_info("lease -> pool is null\n");
			}
		if(lease -> pool){
			if(!lease -> pool -> failover_peer) {
				log_info("lease -> pool -> failover_peer is null\n");
				}
			}
		return 1;
		}
	...
	if (lease -> flags & ON_UPDATE_QUEUE){
		log_info("lease -> flags & ON_UPDATE_QUEUE is true\n");
		return 1;
		}
...
isc_result_t 
dhcp_failover_state_destroy_by_name
(
	struct dcli_failover* failover_peer
)
{
	dhcp_failover_state_t *state;
	dhcp_failover_state_t *prev = (dhcp_failover_state_t *)0;
	for (state = failover_states; state; state = state->next) {
		/* for a special state by name*/
		if (!strcmp(state->name, failover_peer->name)) {
			if(!strcmp(state->name, failover_states->name)){
				dhcp_failover_state_dereference (&failover_states,
							 MDL);
				dhcp_failover_state_reference (&failover_states, state -> next, MDL);
			}else{
				if(prev){
				dhcp_failover_state_dereference (&prev->next,
							 MDL);
				dhcp_failover_state_reference (&prev -> next, (state -> next), MDL);
				dhcp_failover_state_dereference (&(state->next),
							 MDL);
				}
			}
			//disconnect for derefrence link_to_peer ,dhcp may dead in case of receiving message
			if(state->link_to_peer){
				if(state -> link_to_peer->outer){
					omapi_disconnect ((state -> link_to_peer->outer) , 1);
				}
			...
			cancel_timeout (dhcp_failover_send_contact, state);
			cancel_timeout (dhcp_failover_reconnect, state);
			cancel_timeout (dhcp_failover_listener_restart, state);
			cancel_timeout (dhcp_failover_timeout, state);
			cancel_timeout (dhcp_failover_auto_partner_down, state);
			cancel_timeout (dhcp_failover_startup_timeout, state);
			cancel_timeout (dhcp_failover_recover_done, state);
			cancel_timeout (dhcp_failover_pool_rebalance, state);
			cancel_timeout (dhcp_failover_toack_queue_timeout, state);
			cancel_timeout (commit_leases_timeout, state);
			dhcp_failover_state_destroy((omapi_object_t *)state, MDL);
			return 0;
		}
		prev = state;
	}
...
#if defined(DEBUG_FAILOVER_MESSAGES) && defined(DEBUG_FAILOVER_CONTACT_MESSAGES)
	char obuf [64];
	unsigned obufix = 0;
...
# define FMA (char *)0, (unsigned *)0, 0
	...
	if (!l -> outer || l -> outer -> type != omapi_type_connection)
		return ISC_R_INVALIDARG;
    if(!state && (link->imsg->options_present & FTB_RELATIONSHIP_NAME)) {
        op = dmalloc (sizeof (failover_option_t), MDL);
        *op = link->imsg->relationship_name;
    }
	status =
	    (dhcp_failover_put_message
	     (link, l -> outer,
...
# define FMA (char *)0, (unsigned *)0, 0
	...
	if (!state -> link_to_peer ||
	    state -> link_to_peer -> type != dhcp_type_failover_link){
		if(!state -> link_to_peer){
		log_info("state -> link_to_peer is null\n");
		}else if(state -> link_to_peer){
			if(state -> link_to_peer -> type != dhcp_type_failover_link){
				log_info("state -> link_to_peer -> type != dhcp_type_failover_link\n");
				}
			}
		return ISC_R_INVALIDARG;
		}
	...
	if (!link -> outer || link -> outer -> type != omapi_type_connection){
		if(!link -> outer){
			log_info("link -> outer is null\n");
			}else if(link -> outer){
				if(link -> outer -> type != omapi_type_connection){
				log_info("link -> outer -> type != omapi_type_connection\n");
					}
	...
	if (state -> curUPD && (state -> curUPD != FTM_UPDREQ)){
		log_info("state -> curUPD != FTM_UPDREQ\n");
		return ISC_R_ALREADYRUNNING;
		}
	status = (dhcp_failover_put_message
		  (link, link -> outer,
			 ...
			 failover_message_t *msg)
{
	...
	if ((msg->options_present & FTB_CLTT) != 0){
		if(0 != system_time_difference)
		msg_cltt = msg->cltt + system_time_difference;
		}
	else
		msg_cltt = 0;
					       ...
					       failover_message_t *msg)
{
	...
	if (msg -> options_present & FTB_STOS) {
		if(0 != system_time_difference)
		lt -> starts = msg -> stos + system_time_difference;
	}
	if (msg -> options_present & FTB_LEASE_EXPIRY) {
		if(0 != system_time_difference)
		lt -> ends = msg -> expiry + system_time_difference;
	}
	if (msg->options_present & FTB_POTENTIAL_EXPIRY) {
		if(0 != system_time_difference)
		lt->atsfp = lt->tsfp = msg->potential_expiry +system_time_difference;
	}
	...
	if (!supersede_lease (lease, lt, 0, 0, 0)) {
		...
		log_debug("(7)link->imsg address %p\n",msg);
		log_debug("(7)link->imsg->refcnt %d\n",msg->refcnt);
		goto out;
	} else {
		dhcp_failover_queue_ack (state, msg);
		log_debug("(8)link->imsg address %p\n",msg);
		log_debug("(8)link->imsg->refcnt %d\n",msg->refcnt);
	}
					     ...
					     failover_message_t *msg)
{
	...
	log_debug("(9)link->imsg address %p\n",msg);
	log_debug("(9)link->imsg->refcnt %d\n", msg->refcnt);
...
isc_result_t dhcp_failover_generate_update_queue_add_pool (int everythingp,
...
#define RESERVED_LEASES 5
	...
	/* Loop through each pool in sub shared network and call the
	   expiry routine on the pool. */
	    s=sub->shared_network;
	    for (p = s->pools; p; p = p -> next) {
	...
	val = subnode->owned_failover.name;
    status = find_failover_peer (&state, val, MDL);
	if (status != ISC_R_SUCCESS) {
	log_error("failover peer %s: %s \n", val, isc_result_totext (status));
	}

	if (state -> update_queue_tail) {
		//log_info("state -> send_update_done exists\n");
				      ...
				      failover_message_t *msg)
{
	...
	log_debug("(11)link->imsg address %p\n",msg);
	log_debug("(11) link->imsg->refcnt %d\n",msg->refcnt);
					  ...
					  failover_message_t *msg)
{
	...
	state->updxid = msg->xid;
	log_debug("(12)link->imsg address %p\n",msg);
	log_debug("(12)link->imsg->refcnt %d",msg->refcnt);
	if (state -> update_queue_tail) {
		lease_reference (&state -> send_update_done,
				 state -> update_queue_tail, MDL);
		dhcp_failover_send_updates (state);
				   ...
				   failover_message_t *msg)
{
	...
	switch (state -> me.state) {
		...
		break;	/* shouldn't happen. */
		  ...
		  case communications_interrupted:
		  		{
			log_info("case communications_interrupted --> set_state normal!\n");	
			dhcp_failover_set_state (state, normal);
		  }
		break;
		...
		if (state->partner.state == conflict_done) {
			if (state->i_am == secondary) {
				dhcp_failover_set_state (state, normal);
					...
					if(flag_shutdown){
						log_info("In order not to transit to shut_down for swichover between master and back,Here set state normal \n");
						dhcp_failover_set_state (state, normal);
						flag_shutdown = 0;
					}else{
