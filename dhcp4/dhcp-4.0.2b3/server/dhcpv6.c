		      ...
		      int reply_len)
{
	...
	log_debug("Client %s declines address %s, which is not offered to it.",
		 print_hex_1(client_id->len, client_id->data, 60),
		    ...
		    struct iaaddr *lease)
{
	...
	log_debug("Client %s releases address %s",
		 print_hex_1(client_id->len, client_id->data, 60),
		 inet_ntop(AF_INET6, iaaddr->data, tmp_addr, sizeof(tmp_addr)));
	if (lease != NULL) {
		      ...
		      int reply_len)
{
	...
	log_debug("Client %s releases address %s, which is not leased to it.",
		 print_hex_1(client_id->len, client_id->data, 60),
		 inet_ntop(AF_INET6, iaaddr->data, tmp_addr, sizeof(tmp_addr)));
...
/* For testing, we reply to the sending port, so we don't need a root client */
		...
		log_debug("Sending %s to %s port %d", 
