...
int dhcp_failover_sockfd = -1;
int (*dhcp_failover_state_change_hook)(int) = NULL;
#endif
				 ...
				 struct timeval *t)
{
	...
	if(dhcp_failover_sockfd >= 0) {
		FD_SET (dhcp_failover_sockfd, &r);	

		/*make sure select fd max is right*/
		if(max < dhcp_failover_sockfd){
			max = dhcp_failover_sockfd;
		}
	}
...
#define fds_bits __fds_bits
	...
	/* socket get message from dhcp dbus pthread (failover start or state change) */
	if ((dhcp_failover_sockfd >= 0) && (FD_ISSET(dhcp_failover_sockfd, &r))) {
		dhcp_failover_state_change_hook(dhcp_failover_sockfd);
	}
