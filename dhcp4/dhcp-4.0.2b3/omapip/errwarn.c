...
#define DHCP_SYSLOG_BUFFER_SIZE	(256)
/* Log an error message, then exit... */

void log_fatal (const char * fmt, ... )
{
  va_list list;
  char fatal_mbuf[CVT_BUF_MAX + 1] = {0};
  char fatal_fbuf[CVT_BUF_MAX + 1] = {0};

  do_percentm (fatal_fbuf, fmt);

  /* %Audit% This is log output. %2004.06.17,Safe%
   * If we truncate we hope the user can get a hint from the log.
   */
  va_start (list, fmt);
  vsnprintf (fatal_mbuf, sizeof fatal_mbuf, fatal_fbuf, list);
  va_end (list);

#ifndef DEBUG
  syslog (log_priority | LOG_ERR, "%s", fatal_mbuf);
#endif
...
{
  va_list list;
  char err_mbuf[CVT_BUF_MAX + 1] = {0};
  char err_fbuf[CVT_BUF_MAX + 1] = {0};

  do_percentm (err_fbuf, fmt);
  ...
  va_start (list, fmt);
  vsnprintf (err_mbuf, sizeof err_mbuf, err_fbuf, list);
  va_end (list);

#ifndef DEBUG
	if(dhcp_log_level & DEBUG_TYPE_ERROR){
  syslog (log_priority | LOG_ERR, "%s", err_mbuf);
	}
#endif
 
  IGNORE_RET (write (STDERR_FILENO, err_mbuf, strlen (err_mbuf)));
  IGNORE_RET (write (STDERR_FILENO, "\n", 1));
...
int log_info (const char *fmt, ...)
{
  ...
  char info_fbuf[CVT_BUF_MAX + 1] = {0};
	...
	if(dhcp_log_level & DEBUG_TYPE_INFO){
	  	syslog (log_priority | LOG_INFO, "%s", info_mbuf);
	}
#endif
...
{
	va_list ptr;
	char buf[DHCP_SYSLOG_BUFFER_SIZE] = {0};
	int len = 0;
	int fd = -1;
	if(!format) {
		return;
	...
	va_end(ptr);
	fd = open(DHCP_CRIT_LOGFILE, O_CREAT | O_RDWR | O_APPEND);
	if(fd < 0) {
		log_error("write crit log to %s open error!\n", DHCP_CRIT_LOGFILE);
		return;
	}
	write(fd, buf, (len < DHCP_SYSLOG_BUFFER_SIZE) ? len : DHCP_SYSLOG_BUFFER_SIZE);
	close(fd);
	return;	
}
