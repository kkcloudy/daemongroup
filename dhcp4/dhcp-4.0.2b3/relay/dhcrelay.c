...
extern struct dcli_relay dhcrelay_head;
...
extern void dhcrelay_dbus_start(void);
...
main(int argc, char **argv) {
...
#else
	openlog ("dhcrelay", LOG_NDELAY, DHCPD_LOG_FACILITY);
#endif	
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp (argv [i], "-p")) {
			...
			log_debug ("isc-dhcrelay-%s", PACKAGE_VERSION);
			exit (0);
 		} else if (argv [i][0] == '-') {
			usage ();
 		} else {
...
#ifndef __AX_PLATFORM__
unsigned int
get_relay_node_from_downif
(
	char *downifname,
	struct dcli_relay **relay_node
)
{
	...
	while (relay_head) {
		if (!strncmp(relay_head->downifname, downifname, strlen(downifname))) {
			log_debug("get_relay_node_from_downif  interface %s is in used \n", downifname);
			*relay_node = relay_head;
			return 1;
		}
...
{
	struct server_list *sp = NULL, *tmp = NULL;
	struct in_addr *iap = (struct in_addr *)0;

	iap = malloc(sizeof(struct in_addr));
	if(NULL == iap){
		log_error("malloc failed!!!\n");	
		return 1;
	}
	memset(iap, 0, sizeof(struct in_addr));
	
	iap->s_addr = ia;

	for (tmp = servers; tmp; tmp = tmp->next) {
		if (tmp->to.sin_addr.s_addr == iap->s_addr) {
			log_error("server is used ip is %x !!!\n", iap->s_addr);
			free(iap);
			return 1;
		}
	}
...
int
del_server_addr_ipv4
(
	unsigned int ia
)

{
	struct server_list *ifprev = NULL, *ifnode = NULL;

	ifnode = servers;

	while (ifnode) {
		if (ia == ifnode->to.sin_addr.s_addr) {
			if (ifprev) {
...
{
	struct interface_info *tmpif = NULL;

	for (tmpif = interfaces; tmpif; tmpif = tmpif->next) {
		if (!strcmp(tmpif->name, ifname)) {
			log_debug("delete interface for relay is %s \n", ifname);
			#if 1
			dhcp_interface_remove((omapi_object_t *)tmpif, NULL);
			#else
			omapi_unregister_io_object(tmpif);
			dhcp_interface_destroy(tmpif, MDL);
			#endif
			break;
		}
	}
	...
	for (tmpif = interfaces; tmpif; tmpif = tmpif->next) {
		log_debug("relay interfaces name %s flag is %d \n", tmpif->name, tmpif->flags);
	}
	...
	struct hardware *hfrom;
{
	...
	struct sockaddr_in to;
	struct interface_info *out, *upsv;
	struct hardware hto, *htop;
	unsigned int ret = 0;
	struct dcli_relay *relay_node = NULL;

	if (packet -> hlen > sizeof packet -> chaddr) {
		log_debug("Discarding packet with invalid hlen.");
		return;
	}
	...
	if (packet -> op == BOOTREPLY) {
		...
		if (send_packet(out, NULL, packet, length, out->addresses[0],
			...
			log_info("forwarded BOOTREPLY for %s to %s",
	...
	/* Otherwise, it's a BOOTREQUEST, so forward it to all the
	   servers. */
	for (sp = servers; sp; sp = sp -> next) {
		log_debug("relay run here relay_hash_lookups interface name is %s \n", ip->name);			
		ret = get_relay_node_from_downif(ip->name, &relay_node);
		if (!ret) {
			log_debug("no relay this port here \n");
			break;
		}
		for (upsv = interfaces; upsv; upsv = upsv->next) {
			log_debug("relay run here before send packet all servers interface name is %s , relay_node->upifname %s \n", upsv->name, relay_node->upifname);			

			if (strncmp(relay_node->upifname, upsv->name, strlen(upsv->name))) {
				continue;
			}
			log_debug("relay run here before send packet all servers interface name is %s , relay_node->upifname %s \n", upsv->name, relay_node->upifname);			
		#if 0
		if (send_packet((fallback_interface
					 ? fallback_interface : upsv),
				 NULL, packet, length, ip->addresses[0],
				 &sp->to, NULL) < 0) {
		#else 
		if (send_packet_relay2server(upsv, NULL, packet, length, ip->addresses[0], &sp->to)) {
		#endif
			++client_packet_errors;
		} else {
			log_info ("forwarded BOOTREQUEST for %s to %s",
			       print_hw_addr (packet -> htype, packet -> hlen,
					      packet -> chaddr),
