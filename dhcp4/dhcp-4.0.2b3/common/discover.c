...
extern unsigned int
check_interface_in_failover_backup_list
(
	char* interface_name
);


struct interface_info *interfaces = NULL, *dummy_interfaces = NULL, *fallback_interface = NULL;
...
/* 
 * Start a scan of interfaces. Solaris support 
 *
...
/* 
 * Start a scan of interfaces. Linux support
 *
 * The iface_conf_list structure maintains state for this process.
 */
int 
	...
	for (;;) {
		...
		if (ioctl(ifaces->sock, SIOCGIFADDR, &tmp) < 0) {
			...
			log_error("Error getting interface address "
				  "for '%s'; %m", name);
			continue;  //To void no way to restart dhcpd for other bad interfaces;
			*err = 1;
			return 0;
		}
		...
		memcpy(&info->addrmask, &tmp.ifr_netmask, sizeof(tmp.ifr_netmask));
...
/* 
 * Start a scan of interfaces.BSD support
 *
 * The iface_conf_list structure maintains state for this process.
 */
int 
...
void just_for_debug()
{
	...
	for (tmp_interface = interfaces; tmp_interface; tmp_interface = tmp_interface->next){
		log_debug("interface name <%s>[%p][%p][%p]\n", tmp_interface->name,
					(tmp_interface->addresses[0].s_addr), (tmp_interface->addresses[1].s_addr),					
					(tmp_interface->addresses[2].s_addr));					
	}
	log_debug("====================after========================\n");
}
*/
...
/**********************************************************************************
 *  alloc_interface_ipv4_list
 *
 *	DESCRIPTION:
 * 		dmalloc interface ipv4 list
 *
 *	INPUT:
 *		NULL 
 *	
 ...
 *		
 **********************************************************************************/
int alloc_interface_ipv4_list(struct interface_info *iface)
{	
	int new_max;
	...
	if (iface->addresses == NULL) {
		...
		if (iface->addresses == NULL) {
			log_error("Out of memory saving IPv4 address "
			          "on interface.");
			return -1;
		}
...
/**********************************************************************************
 *  scan_all_ipv4_address
 *
 *	DESCRIPTION:
 * 		scan all interface and add all ipv4 address to struct interface_info
 *
 *	INPUT:
 *		NULL 
 *	
 ...
 *		
 **********************************************************************************/
int scan_all_ipv4_address(struct interface_info *iface)
{
	...
	if ( -1 == getifaddrs(&ifaddr)) {
		log_error("discover.c getifaddrs failed\n");
		return -1;
	}

	if (iface->addresses) {
		dfree(iface->addresses, MDL);
		iface->addresses = NULL;
		iface->address_count = 0;
		iface->address_max = 4;
	}
	...
	for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
		...
		/* For an AF_INET* interface address, add to the address list */
		if (family == AF_INET) {
			sockaddr = (struct sockaddr_in *)(ifa->ifa_addr);
			addr = &(sockaddr->sin_addr);
			log_debug("infterface name<%s><%s>\n", ifa->ifa_name, inet_ntoa(sockaddr->sin_addr));
			if (-1 == alloc_interface_ipv4_list(iface)) {
				continue;
			}
...
add_ipv4_addr_to_interface(struct interface_info *iface, 
	...
	int i = 0;
	if(NULL == iface){
		log_error("add ipv4 addr to interface£¬ iface is NULL\n");
		return;
	}
	if (NULL == addr) {
		log_error("add ipv4 addr to interface£¬addr is NULL\n");
		return;
	}
		
	if(-1 == scan_all_ipv4_address(iface)){
		return ;
	}
#if 0
	/*
...
#endif
	for (i = 0; i < iface->address_count; i++) {
		if (iface->addresses[i].s_addr == addr->s_addr)
			return;
	}
	if (-1 == alloc_interface_ipv4_list(iface)) {
		return;
...
*/
int 
discover_interfaces_to_server(int state, char *name) {
	...
	while (next_iface(&info, &err, &ifaces)) {
		...
		/*add for continue the interface we already have*/
	...
	if (err) {
		log_error("Error getting interface information.");
	}
	...
	while (tmp) {	
		...
		/* skip interfaces that are running already or has own before*/
		
		if ((tmp -> flags & INTERFACE_RUNNING) || 
			strncmp(tmp->name, name, sizeof(tmp->name))) {
			interface_dereference(&tmp, MDL);
			if(next)
		...
		if (local_family == AF_INET) {
			...
			if_register_send_to_server(tmp);
#ifdef DHCPv6
		} else {
	...
	if ((state == DISCOVER_SERVER || state == DISCOVER_RELAY) && wifcount == 0) {			
		log_info("Not configured to listen on any interfaces!");
		return 1;
	}
...
/*for selet run a circle, add new fd in FD_SET*/	
...
int 
discover_interfaces_sharenetwork_test(int state, char *name, struct dcli_subnet *head_subnet) {
	struct iface_conf_list ifaces;
	struct iface_info info;
	int err;
	...
	int i = 0;
	unsigned int subnet_addr = 0, subnet_mask = 0;
	unsigned int iface_addr = 0;
	...
	while (next_iface(&info, &err, &ifaces)) {
		...
		if (0 == strncmp("obc", info.name, 3)) {
			log_debug("%s: interface %s ignore\n", __func__, info.name);
			continue;
		}
		...
		if ((info.addr.ss_family == AF_INET) && 
			...
			*/
			...
			if (NULL == head_subnet) {
				log_error("discover ifaces share, pool subnet is NULL\n");
				return 1;
			}
			memcpy(&subnet_addr, head_subnet->ipaddr.iabuf, sizeof(subnet_addr));
			memcpy(&subnet_mask, head_subnet->mask.iabuf, sizeof(subnet_mask));
			log_debug("discover sharenetwork, subnet_addr<%#x>, subnet_mask<%#x>\n",
							subnet_addr, subnet_mask);
			
			for (i=0; i<tmp->address_count; i++) {
				addr.len = 4;
				memcpy(&iface_addr, &(tmp->addresses[i].s_addr), sizeof(iface_addr));				
				log_debug("discover sharenetwork, iface_addr<%#x>\n",  iface_addr);
				if ((subnet_addr & subnet_mask) != (iface_addr & subnet_mask)) {
					continue;
				}
				memcpy(addr.iabuf, &(tmp->addresses[i].s_addr), addr.len);
			...
			if (NULL == tmp->shared_network) {
				a = (struct sockaddr_in*)&info.addr;
			...
			if (tmp->shared_network) {
				if (tmp->shared_network->subnets) {
					log_debug("interface <%s> share network<%s>\n",
								tmp->name, piaddr(tmp->shared_network->subnets->net));
				}
			} else  {
			}
		}
...
#if 1	//Remove restriction of interface ip and dhcp pool 's ip not  in the same network 
...
int 
discover_interfaces_test(int state, char *name) {
	struct iface_conf_list ifaces;
	struct iface_info info;
	int err;
...
discover_interfaces(int state) {
	...
	while (next_iface(&info, &err, &ifaces)) {
		...
		if (((!(info.flags & IFF_BROADCAST) ||
		      ...
		      info.flags & IFF_POINTOPOINT) && !tmp))
/*		    ||(!(info.flags & IFF_UP) &&
		     state != DISCOVER_UNCONFIGURED))*/ /*test for down interface*/
			continue;
...
/*************
			...
			if (0 == strncmp(tmp->name, testname, sizeof(tmp->name))) {
				status = omapi_register_io_object((omapi_object_t *)tmp,
								  if_readsocket, 
								  0, got_one, 0, 0);
...
)
{
	...
	while (backup) {
		for (i = 0; i < backup->interface_num; i++) {
			if (!(backup->state)) {
				/* when primary, do not need deny recv packet*/
				continue;
			}
			if (!strncmp(interface_name, backup->dhcp_interface[i].if_name, strlen(interface_name))) {
				return 0;
			}
		}
		backup = backup->next;
	...
	omapi_object_t *h;
{
	...
	if (!dhcp_server_enable) {
		log_info("global dhcp server is disable\n");
		return ISC_R_SUCCESS;
	}
	if (vrrp_state_secondary == ip->vrrp_state_flag) {
		log_debug("interface %s vrrp state secondary donot process dhcp request\n", ip->name);
		return ISC_R_SUCCESS;
	} 
	#ifndef __AX_PLATFORM__
	unsigned char *t = hfrom.hbuf;
	if (0 == (t[1] | t[2] | t[3] | t[4] | t[5] | t[6])) {
		log_debug("client MAC is 00:00:00:00:00:00, ignore!\n");
		return ISC_R_SUCCESS;
	}
	#endif
	if (!check_interface_in_failover_backup_list(ip->name)) {	
		log_debug("failover peer is backup, interface name is %s\n", ip->name);
		return ISC_R_SUCCESS;
	}
...
int dhcp_server_sock_init(unsigned int port)
{	
	int sock = -1;	
	...
	if ((sock = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
		log_error("dhcp server sock init create socket failed: %m.\n");
		return -1;
	}	
	...
	*/
	...
	addr.sin_port = htons(port);		/* dhcp server port : bootps */
	addr.sin_addr.s_addr = INADDR_ANY;	

	if (bind(sock, (struct sockaddr *)&addr, sizeof(struct sockaddr)) < 0) {	
		log_error("dhcp server socket init bind failed: %m.\n");
		if(sock){
			close(sock);
		}
		return -1;
