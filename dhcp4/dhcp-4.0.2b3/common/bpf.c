...
int dhcp_set_none_lpf_filter(int sock);
	...
	struct interface_info *info;
{
	...
	if (!quiet_interface_discovery)
		log_debug ("Sending on   BPF/%s/%s%s%s",
		      info -> name,
	...
	struct interface_info *info;
{
	...
	if (!quiet_interface_discovery)
		log_debug ("Disabling output on BPF/%s/%s%s%s",
		      info -> name,
		      print_hw_addr (info -> hw_address.hbuf [0],
				     info -> hw_address.hlen - 1,
				     &info -> hw_address.hbuf [1]),
	...
	struct interface_info *info;
{
	...
	if (!quiet_interface_discovery)
		log_debug ("Listening on BPF/%s/%s%s%s",
		      info -> name,
		      print_hw_addr (info -> hw_address.hbuf [0],
				     info -> hw_address.hlen - 1,
				     &info -> hw_address.hbuf [1]),
	...
	struct interface_info *info;
{
	...
	if (!quiet_interface_discovery)
		log_debug ("Disabling input on BPF/%s/%s%s%s",
		      info -> name,
		      print_hw_addr (info -> hw_address.hbuf [0],
				     info -> hw_address.hlen - 1,
				     &info -> hw_address.hbuf [1]),
...
/*
	receive nothing
*/
struct bpf_insn dhcp_none_filter [] = {
	/* drop all. */
	BPF_STMT(BPF_RET+BPF_K, 0),
};
int dhcp_none_filter_len = sizeof(dhcp_none_filter)/sizeof(struct bpf_insn);
