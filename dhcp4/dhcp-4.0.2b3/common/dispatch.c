...
pthread_mutex_t DhcpTimeEventMutex = PTHREAD_MUTEX_INITIALIZER;
#endif
...
struct timeval *process_outstanding_timeouts (struct timeval *tvp)
{
	...
	/*
	add for server add new interface
	{	
		gettimeofday (&now, (struct timezone *)0);
		tvp -> tv_sec = now.tv_sec + 2;
		tvp -> tv_usec = 0;
	}
*/	
}
...
void dispatch ()
{
	...
	do {
		pthread_mutex_lock(&DhcpTimeEventMutex);
		tvp = process_outstanding_timeouts (&tv);
		pthread_mutex_unlock(&DhcpTimeEventMutex);		
...
void cancel_timeout_one_all (where, what)
	void (*where) PROTO ((void *));
	void *what;
{
	struct timeout *t, *q;
	...
	for (q = timeouts; q; q = q -> next) {
		if (q -> what == what) {
			log_debug("remove for time list \n");
			if (t) {
				t -> next = q -> next;
			}
			else {
