	...
	struct interface_info *info;
{
	...
	info -> wfdesc = info -> rfdesc;
		...
		log_debug("Sending on   NIT/%s%s%s",
	...
	struct interface_info *info;
{
	...
	info -> wfdesc = -1;
        if (!quiet_interface_discovery)
		log_debug ("Disabling output on NIT/%s%s%s",
		      print_hw_addr (info -> hw_address.hbuf [0],
				     info -> hw_address.hlen - 1,
				     &info -> hw_address.hbuf [1]),
		      (info -> shared_network ? "/" : ""),
	...
	struct interface_info *info;
{
	...
	if (ioctl (info -> rfdesc, I_SRDOPT, RMSGN) != 0)
		log_debug ("I_SRDOPT failed on %s: %m", info -> name);
	...
	if (ioctl (info -> rfdesc, I_STR, &sio) < 0)
        ...
        if (!quiet_interface_discovery)
		log_debug ("Listening on NIT/%s%s%s",
		      print_hw_addr (info -> hw_address.hbuf [0],
				     info -> hw_address.hlen - 1,
				     &info -> hw_address.hbuf [1]),
		      (info -> shared_network ? "/" : ""),
	...
	struct interface_info *info;
{
	...
	info -> rfdesc = -1;
        ...
        if (!quiet_interface_discovery)
		log_debug ("Disabling input on NIT/%s%s%s",
		      print_hw_addr (info -> hw_address.hbuf [0],
				     info -> hw_address.hlen - 1,
				     &info -> hw_address.hbuf [1]),
		      (info -> shared_network ? "/" : ""),
