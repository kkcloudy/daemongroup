...
int dhcp_bootp_enable = 0;
char default_vendor_class_identifier[DHCP_OPTION60_ID_DEFAULT_LENGTH];
#endif
...
void dhcp_notify_to_trap_event_base(char *str);
int is_set_vendor_class_identifier(struct dcli_pool *pool, char *vendor_id);


extern void dhcp_write_crit_log(char *format, ...);
			  ...
			  unsigned length, struct universe *u)
{
	...
	if (!bp -> data [0]) {
		...
		while (s < &bp -> data[0] + length + 2) {
			len = *s;
			if (len > 63) {
				log_debug ("fancy bits in fqdn option");
				return 0;
			}	
			if (len == 0) {
				terminated = 1;
			...
			if (s + len > &bp -> data [0] + length + 3) {
				log_debug ("fqdn tag longer than buffer");
				return 0;
			}
...
#define PRIORITY_COUNT 300
	...
	if (inpacket &&
		...
		t=evaluate_option_cache(&ds, inpacket,
	...
	for (i = 0; i < priority_len; i++) {
		...
		if (s && t) {
			...
			if (vendor_cfg_option) {
			    ...
			    if (tmp){
					k=evaluate_option_cache (&name, packet, lease,
							       client_state,
							       in_options,
							       cfg_options,
							       scope, tmp, MDL);
	    ...
	    od.len = 0;
	    if (oc) {
		k=evaluate_option_cache (&od, packet,
				       lease, client_state, in_options,
	...
	struct universe *universe;
{
	...
	for (ocp = head -> first; ocp; ocp = ocp -> cdr) {
		...
		if (oc -> option -> code > FQDN_SUBOPTION_COUNT)
			continue;
		t=evaluate_option_cache (&results [oc -> option -> code],
			       ...
			       struct universe *universe)
{
	...
	for (ocp = head->first ; ocp != NULL ; ocp = ocp->cdr) {
		...
		t=evaluate_option_cache(&results[oc->option->code], packet,
				      lease, client_state, in_options,
				      cfg_options, scope, oc, MDL);
		if(!t){
			log_error("evaluate_option_cache failed!\n");
	...
	struct hardware *hfrom;
{
	...
	if(!(interface->hw_address.hbuf[1]|interface->hw_address.hbuf[2]|
		interface->hw_address.hbuf[3]|interface->hw_address.hbuf[4]|
		interface->hw_address.hbuf[5]|interface->hw_address.hbuf[6])){
		/*add for update mac*/
		 strcpy(req.ifr_name,interface->name);
		 ret = ioctl(interface->wfdesc,SIOCGIFHWADDR,&req);
		 if(-1 != ret){
		 	memcpy (&interface->hw_address.hbuf[1],req.ifr_ifru.ifru_addr.sa_data, 6);
		 }

		 log_debug("mac is %02x:%02x:%02x:%02x:%02x:%02x\n", interface->hw_address.hbuf[1], 
		 													interface->hw_address.hbuf[2], 
		 													interface->hw_address.hbuf[3], 
		 													interface->hw_address.hbuf[4], 
		 													interface->hw_address.hbuf[5], 
		 													interface->hw_address.hbuf[6]);
	}
	
	if(!interface->unbind_pool_flag){
		...
		if (packet -> hlen > sizeof packet -> chaddr) {
			...
			log_debug("Discarding packet with bogus hlen.");
			return;
		}
		...
		if (decoded_packet -> packet_length >= DHCP_FIXED_NON_UDP + 4) {
			...
			if (decoded_packet -> options_valid &&
				...
				t=evaluate_option_cache (&dp, decoded_packet,
						       (struct lease *)0,
...
#ifndef __AX_PLATFORM__
			if (interface->pool_ref) {				
				if (interface->pool_ref->owned_option.option60_enable) {
					if (decoded_packet -> options_valid &&
					    (op = lookup_option (&dhcp_universe,
								 decoded_packet -> options, 
								 DHO_VENDOR_CLASS_IDENTIFIER))) {
						struct data_string dp;
						char option60[DHCP_OPTION60_ID_DEFAULT_LENGTH];						
						memset (&dp, 0, sizeof dp);
						t=evaluate_option_cache (&dp, decoded_packet,
								       (struct lease *)0,
								       (struct client_state *)0,
						...
						if (dp.len > DHCP_OPTION60_ID_DEFAULT_LENGTH - 1) {  
							log_error("option60 length %d too long, VCI %s. maxlength %d\n", dp.len, dp.data, DHCP_OPTION60_ID_DEFAULT_LENGTH);							
							data_string_forget (&dp, MDL);
							packet_dereference (&decoded_packet, MDL);							
							return;
						}
						memset(option60, 0, sizeof(option60));
						memcpy(option60, dp.data, dp.len);
						if (1 != is_set_vendor_class_identifier(interface->pool_ref, option60)) {
							log_debug("%s %s from %s option60 VCI %s\n",
									print_hw_addr(hfrom->hbuf[0], hfrom->hlen - 1, &(hfrom->hbuf[1])),	/* client MAC */
									piaddr(from),														/* client IP */
						...
						sprintf(buf, "user %s %s from %s : no option60",
								print_hw_addr(hfrom->hbuf[0], hfrom->hlen - 1, &(hfrom->hbuf[1])),	/* client MAC */
								piaddr(from),														/* client IP */
								interface->name); 													/* interface name */
						log_error("%s\n", buf);						
						dhcp_notify_to_trap_event_base(buf);
						packet_dereference (&decoded_packet, MDL);						
						return;
					}
				}
		...
		if (decoded_packet -> packet_type) {
			...
			if (dhcp_bootp_enable) {
				...
				log_info("not enable BOOTP Protocol support.\n");
			}			
		}
...
#if defined (DEBUG_MEMORY_LEAKAGE)
		log_debug ("generation %ld: %ld new, %ld outstanding, %ld long-term",
			  dmalloc_generation,
...
int is_set_vendor_class_identifier(struct dcli_pool *pool, char *vendor_id)
{
	int len = -1;
	int i = 0;
	if ((NULL == pool) || (NULL == vendor_id)) {
		return 0;
	}
	len = strlen(vendor_id);
	if (len == strlen(default_vendor_class_identifier)) {
		if (0 == strncmp(default_vendor_class_identifier, vendor_id, len)) {
			return 1;
		}
	}
	for (i = 0; i < pool->owned_option.option60_cnt; i++) {
		if (len != strlen(pool->owned_option.option60[i].id)) {
			continue;
		}
		if (0 == strncmp(pool->owned_option.option60[i].id, vendor_id, len)) {
			return 1;
		}
	}
	if (i >= pool->owned_option.option60_cnt) {
		return 0;
	}
...
void dhcp_notify_to_trap_event_base(char *str)
{
	...
	dhcp_write_crit_log("%% DHCP warn: %s %s, please check!\n", buf, str);
#if 0
	...
	if (NULL == conn) {
		...
		if (NULL == conn) {
			log_error("%s:%d notify trap get dbus Connection null\n", MDL);
			return;
		}
	}
