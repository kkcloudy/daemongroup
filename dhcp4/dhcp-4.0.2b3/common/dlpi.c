...
# endif /* USE_DLPI_PFMOD */
	...
	info -> wfdesc = info -> rfdesc;
		...
		log_debug ("Sending on   DLPI/%s/%s%s%s",
		      info -> name,
	...
	struct interface_info *info;
{
	...
	info -> wfdesc = -1;
        ...
        if (!quiet_interface_discovery)
		log_debug ("Disabling output on DLPI/%s/%s%s%s",
		      info -> name,
		      print_hw_addr (info -> hw_address.hbuf [0],
				     info -> hw_address.hlen - 1,
				     &info -> hw_address.hbuf [1]),
...
# endif /* USE_DLPI_RAW */
	...
	if (strioctl (info -> rfdesc, PFIOCSETF, INFTIM,
        ...
        if (!quiet_interface_discovery)
		log_debug("Listening on DLPI/%s/%s%s%s",
		      info -> name,
		      print_hw_addr (info -> hw_address.hbuf [0],
				     info -> hw_address.hlen - 1,
				     &info -> hw_address.hbuf [1]),
	...
	struct interface_info *info;
{
	...
	info -> rfdesc = -1;
        ...
        if (!quiet_interface_discovery)
		log_debug ("Disabling input on DLPI/%s/%s%s%s",
		      info -> name,
		      print_hw_addr (info -> hw_address.hbuf [0],
				     info -> hw_address.hlen - 1,
				     &info -> hw_address.hbuf [1]),
