...
unsigned int test_restart;
#ifndef __AX_PLATFORM__
#define DHCP_SYSTEM_STARTUP_STATE_PATH	"/var/run/aw.state"
#define DHCP_SYSTEM_STARTUP_STATE_FLAG	'1'
#define DEVICE_NORMAL		(0x01)
#define DEVICE_STARTUP		(0x0)
#define nak_enable          (0x01)
#define nak_disable          (0x0)
#define ASN_ENABLE          (0x01)
#define ASN_DIABLE          (0x0)


#define DHCP_STARTUP_STATE_PATH	"/var/run/dhcp.state"
#define DHCP_NOMARL_STATE_FLAG	'1'
#define DHCP_NORMAL			(0x01)		/* startup dhcp dbus thread, then loadconfig and dispatch */
#define DHCP_STARTUP		(0x0)
#define DHCP_MANUAL_RESTART_FLAG_PATH 		"/var/run/dhcp.restart"
#define DHCP_MANUAL_RESTART_FLAG			'1'
#define DHCP_FAILOVER_SOCK_PATH			"/var/run/dhcp_failover_unsock"
unsigned int dhcp_server_enable;
unsigned int dhcp_server_static_arp_enable;
unsigned int dhcp_debug_enable;
...
#define DEFAULT_LEASE_EXPIRY_TIME		(120)

#define DELETE_DYNAMIC
...
typedef struct vrrp_interface
{
	unsigned int master_ip;
	unsigned int backp_ip;
	unsigned int virtual_ip;
	char if_name [MAX_IF_NAME_LEN];
}vrrp_interface_t;

typedef struct vrrp_param
{
	unsigned int vrid;
	unsigned int pre_state;
    unsigned int state;
	unsigned int uplink_cnt;
	vrrp_interface_t uplink_interface[MAX_PHA_IF_NUM];
	unsigned int downlink_cnt;
	vrrp_interface_t downlink_interface[MAX_PHA_IF_NUM];
	unsigned int vgateway_cnt;
	vrrp_interface_t vgateway_interface[MAX_PHA_IF_NUM];
	unsigned char  heartlink_if_name [MAX_IF_NAME_LEN];
	unsigned int heartlink_local_ip;
	unsigned int heartlink_opposite_ip;	
	/* test */
	
    unsigned int master_uplinkip;
    unsigned int master_downlinkip;
    unsigned int back_uplinkip;
    unsigned int back_downlinkip;
    unsigned int virtual_uplink_ip;
    unsigned int virtual_downlink_ip;
	unsigned char uplink_if[MAX_IF_NAME_LEN];
	unsigned char  downlink_if[MAX_IF_NAME_LEN];
	unsigned char  heartlink_if[MAX_IF_NAME_LEN];
	//UINT32 heartlink_local_ip;
	unsigned int heartlink_master_ip;	
	int failover_flag;		/* 1 : set failover, 0 : not config failover */
}vrrp_param_t;

struct dcli_failover
{
	char name[21];
	struct dcli_subnet* attribute_sub;
	unsigned int primary;/*bool 0 for primary 1 for secondary*/
	unsigned int split;
	unsigned int mclt;
	unsigned int myport;
	unsigned int parterport;
	struct iaddr myaddr;
	struct iaddr parteraddr;
	unsigned int load_balance_max_secs;
	unsigned int max_flying_updates;
	unsigned int max_response_delay;
	unsigned int enable;
};

struct failover_backup_list
{	
	vrrp_interface_t dhcp_interface[MAX_PHA_IF_NUM*2];
	unsigned int interface_num;
	unsigned int vrrpid;
	unsigned int state;
	struct failover_backup_list* next;
};

struct dcli_option43
{
	unsigned int type;
	unsigned int len;
	unsigned int ipnum;
...
struct dcli_option {
	char* domainname;
	struct dcli_option43 option43;
	struct iaddr   option138[16];
	unsigned int   option138_num;
	struct iaddr* dnsip;
	unsigned int dnsnum;	
	...
	unsigned int option60_enable;		/* dhcp option60(Vendor Class Identifier) flag. 1 : enable, 0 : disable */
	unsigned int option60_cnt;			/* Vendor Class Identifier count */
	struct dcli_option60 *option60;		/* Vendor Class Identifier */
};

struct dcli_ifname {	
	char* interface_name;
	struct dcli_ifname* next;
};
...
struct dcli_subnet {
	...
	unsigned int interface_num;
	unsigned int ip_num;
	struct dcli_pool* attribute_pool;
	struct dcli_option* owned_option;
	struct dcli_failover owned_failover;
	struct iaddr ipaddr;
	struct iaddr lowip;
	struct iaddr highip;
	struct iaddr mask;
...
struct dcli_host {
	...
	struct dcli_host* next;
};
#define VENDOR_CLASS_IDENTIFIER_FILE 		"/devinfo/enterprise_snmp_oid"
#define DHCP_OPTION60_ID_DEFAULT_LENGTH		(256)	/* RFC1533 (255) */
#define DHCP_OPTION60_DEFAULT_NUMBER		(4)
struct dcli_option60 {						/* VCI */
	char id[DHCP_OPTION60_ID_DEFAULT_LENGTH];
};
...
struct dcli_pool {
	...
	struct dcli_subnet* headsubnet;
	unsigned int count_sub;
	struct dcli_option owned_option;
	unsigned int state;
	struct dcli_pool* next;
};
...
struct dcli_relay {
	...
	struct dcli_relay* next;
};
const char *print_time_leases(TIME t);
int discover_interfaces_test(int state, char *name);
void test_for_del_subnet
(
	struct dcli_subnet *delsub
);

isc_result_t save_lease_hw_info_file(const void *key, unsigned len, void *object);
isc_result_t save_lease_ip_info_file(const void *key, unsigned len, void *object);
void save_dhcp_info_file(void);
struct dhcp_lease_info* get_dhcp_info(void);
/*
...
struct dhcp_lease_ip_info {
	...
	struct dhcp_lease_ip_info *next;
};

struct dhcp_lease_info { /*add interface info*/
	int whole_times_info; /* ip pool size */
	int segment_times_info; /* ip address assigned */
	unsigned int requested_times_info; /* dhcp request times */
	unsigned int response_times_info; /* dhcp response times */
...
# define DEFAULT_MAX_LEASE_TIME (86400*365)
#endif
...
struct pool {
	...
	int free_leases;
	int backup_leases;
	int pool_full; 			/* 0 pool has free leases, 1 pool has no free lease*/
	int index;
...
#endif
#ifndef __AX_PLATFORM__
	struct lease **last_lq;
	struct lease *last_insert_point;
#endif	
};
...
struct subnet {
	...
	unsigned int discover_times;		/* client discover times */
...
struct interface_info {
	...
	struct in_addr *addresses;	/* Addresses associated with this
					 ...
					 */
	int vgateway_flag;
	struct in_addr virtual_ip; /*virtual ip*/
...
#ifndef __AX_PLATFORM__
	struct dcli_pool *pool_ref;			/* reference pool_node bind to interface */
	enum {
	     vrrp_state_none,				/* not hansi, recive packet as normal */
	     vrrp_state_master,				/* master process dhcp request */
	     vrrp_state_secondary			/* not master donot process dhcp request */ 
	}vrrp_state_flag;
#endif
};
...
void cancel_timeout_one_all PROTO ((void (*) PROTO ((void *)), void *));
...
isc_result_t dhcp_failover_generate_update_queue (dhcp_failover_state_t *,
						  int);
isc_result_t dhcp_failover_generate_update_queue_add_pool (int,
						  struct dcli_subnet *);
...
void set_unbind_flag_fuc(unsigned int unbind_flag);

int init_sock_unix_server(char *path);
void lease_ping_next(struct lease *lp);
int if_register_lpf_to_server (struct interface_info *);
void if_register_send_to_server(struct interface_info *);
int lpf_none_filter_setup (int sock);
ssize_t send_packet_relay2server(
	struct interface_info *interface,
	struct packet *packet,
	struct dhcp_packet *raw,
	size_t len,
