...
create_lease6(struct ipv6_pool *pool, struct iasubopt **addr, 
	...
	*attempts = 0;
	for (;;) {
		 ...
		 */
		reserved_iid = ISC_FALSE;/* search reason later
		if (memcmp(&tmp.s6_addr[8], &rtany, 8) == 0) {
			reserved_iid = ISC_TRUE;
		}
		log_debug("reserved_iid is %d\n", reserved_iid);
		if (!reserved_iid &&
...
move_lease_to_active(struct ipv6_pool *pool, struct iasubopt *lease) {
	isc_result_t insert_result;
	int old_heap_index;
log_debug("move_lease_to_active\n");
...
mark_lease_unavailable(struct ipv6_pool *pool, const struct in6_addr *addr) {
	...
	isc_result_t result;
log_debug("mark_lease_unavailable run 111\n");
	dummy_iasubopt = NULL;
...
void
delete_pool_lease(struct ipv6_pool *pool) {
	struct ia_xx *ia;
	...
	log_debug("--------------------------> table->hash_count %d \n", table->hash_count);
	for (i = 0; i < table->hash_count; ++i) {
		bp = table->buckets[i];
		while (bp) {
			next = bp->next;
			ia = (struct ia_xx *)bp->value;
			log_debug("--------------------------> ia->num_iasubopt %d \n", ia->num_iasubopt);
			for (j=0; j<ia->num_iasubopt;) {
				lease = ia->iasubopt[j];		

				if(0 == memcmp(&lease->ipv6_pool->start_addr,&pool->start_addr, sizeof(struct in6_addr))){					
					
					ia_remove_iasubopt(ia, lease, MDL);
					continue;
				}
...
/**********************************************************************************
*  get_dhcp6_lease_info()
*
*  DESCRIPTION:
*	 get dhcp ipv6 information
*
*  INPUT:
*	 
*  
...
struct dhcp6_lease_info *  get_dhcp6_lease_info(int *seg_times){
	...
	struct hash_bucket *bp, *next;
	int count = 0;
	struct hash_table *table = ia_na_active;	
	struct ia_xx *ia = NULL;
	int segment_times; 
	struct iasubopt *iasubopt;
	struct dhcp6_lease_info *lease_info = NULL;
	int max_ipv6_len = sizeof("ffff:ffff:ffff:ffff:ffff:ffff.255.255.255.255");	
	
	segment_times = hash_foreach(table, save_lease_ipv6_info_file);

	*seg_times = segment_times;
	lease_info = malloc(sizeof(struct dhcp6_lease_info ) * segment_times);
	if(NULL == lease_info){
		return NULL;
	}
	...
	for (i = 0; i < table->hash_count; i++) {
		...
		while (bp) {
			...
			log_debug("ia->iaid_duid.len is %d\n", ia->iaid_duid.len);
			
			log_debug("type is %d \n", ia->ia_type);
			memcpy(lease_info[count].client_mac, &(ia->iaid_duid.data[12]),  6);
			
			log_debug("mac is %02x:%02x:%02x:%02x:%02x:%02x \n", ia->iaid_duid.data[12],ia->iaid_duid.data[13],ia->iaid_duid.data[14],ia->iaid_duid.data[15],ia->iaid_duid.data[16],ia->iaid_duid.data[17]);
			lease_info[count].ipv6_num = ia->num_iasubopt;

			lease_info[count].ipv6_addr = malloc(sizeof(char *) * lease_info[count].ipv6_num);
			if(NULL == lease_info[count].ipv6_addr){
				for(k=count-1;k>=0;k--){
					for(j=0; j<ia->num_iasubopt; j++){
						if(lease_info[k].ipv6_addr[j]){
							free(lease_info[k].ipv6_addr[j]);
							lease_info[k].ipv6_addr[j]=NULL;
						}
					}
					if(lease_info[k].ipv6_addr){
						free(lease_info[k].ipv6_addr);
						lease_info[k].ipv6_addr=NULL;
					}
				}
			...
			log_debug("-------------->ia ipv6 address is %d\n", ia->num_iasubopt);
			for (j=0; j<ia->num_iasubopt; j++) {
				lease_info[count].ipv6_addr[j] = malloc(max_ipv6_len);
				if(NULL == lease_info[count].ipv6_addr[j]){
					for(j=j-1;j>=0;j--){
						if(lease_info[count].ipv6_addr[j]){
							free(lease_info[count].ipv6_addr[j]);
							lease_info[count].ipv6_addr[j]=NULL;
						}
					}
					if(lease_info[count].ipv6_addr){
						free(lease_info[count].ipv6_addr);
						lease_info[count].ipv6_addr=NULL;
					}
					for(k=count-1;k>=0;k--){
						for(j=0; j<ia->num_iasubopt; j++){
							if(lease_info[k].ipv6_addr[j]){
				...
				iasubopt = ia->iasubopt[j];					
				inet_ntop(AF_INET6, &iasubopt->addr, lease_info[count].ipv6_addr[j], max_ipv6_len);				
				log_debug("ipv6 address is %s\n", lease_info[count].ipv6_addr[j]);
