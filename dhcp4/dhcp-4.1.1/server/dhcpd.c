...
extern void dhcp6_dbus_start(void);
#endif
...
main(int argc, char **argv) {
...
#ifndef __AX_PLATFORM__
	system("mkdir /var/db");
	system("touch /var/db/dhcpd6.leases");
	system("chmod 777 /var/db/dhcpd6.leases");
#endif
	...
	openlog ("dhcp6d", LOG_NDELAY, DHCPD_LOG_FACILITY);
	...
	if (set_server_duid_from_option() == ISC_R_SUCCESS) {
		...
		if (!server_duid_isset()) {
			if (generate_new_server_duid() != ISC_R_SUCCESS) {		
				log_error("Unable to set server identifier.");
