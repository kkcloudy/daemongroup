...
int set_server_duid_new(void);
void add_ipv6_pool_to_subnet(struct subnet *subnet, u_int16_t type, struct iaddr *lo_addr, int bits, int units);
void new_address_range_no_cfile(struct iaddr,struct iaddr,struct subnet *,struct pool *,	struct lease **);
void parse_address_range_no_cfile(struct range_config *,struct group *,int,struct pool *,struct lease **);
int common_subnet_parsing_no_cfile(	struct subnet_config *,struct shared_network *,struct subnet *); 
void parse_subnet_declaration_no_cfile(struct subnet_config *sub_conf,struct shared_network *share);
void parse_pool_statement_no_file(struct range_config *range_conf,struct group *group,	int type);
int parse_statement_subnet_no_file_v6(struct subnet_config *sub_conf,struct group *group);
	...
	struct group *group;
{
	...
	if (status != ISC_R_SUCCESS)
		log_fatal ("Can't allocate shared subnet: %s",
			   isc_result_totext (status));
	t=clone_group (&share -> group, group, MDL);
	if(!t){
		log_error("clone_group failed!\n");
	}
	shared_network_reference (&share -> group -> shared_network,
				  share, MDL);
...
) 
{
	...
	memcpy(addr, ipaddr->iabuf, 4);
	log_debug("parse_fixed_addr_param_test addr is %x.%x.%x.%x \n", 
			addr[0],addr[1],addr[2],addr[3]);
	tmp = NULL;
	if (type == FIXED_ADDR) {
		t=make_const_data (&tmp, addr, len, 0, 1, MDL);
		if(!t){
			log_error("make_const_data failed!\n");
		}
...
/*
	...
	*/
log_debug("parse_host_declaration host name %s\n", name);	
	if(hostipaddr){
		return;
	}
...
//	char testhw[6] = {0x0, 0x1a, 0xa0, 0xb9, 0x3d, 0x60};
	if (host && memcmp(&hardware, &(host->interface),
				sizeof(hardware)) != 0) {
		log_error("Host %s hardware address already "
				  "configured.", host->name);
	}
	//parse_hardware_param (cfile, &hardware);
//	memcpy(&hardware.hbuf[1], testhw, 6);
//	hardware.hlen = 7;
//	hardware.hbuf[0] = HTYPE_ETHER;
	memcpy(&hardware, hosthwaddr, sizeof(struct hardware));
log_debug("parse_host_declaration_test HARDWARE %x.%x.%x.%x.%x.%x\n", 
				hardware.hbuf[1],hardware.hbuf[2],hardware.hbuf[3],hardware.hbuf[4],hardware.hbuf[5],hardware.hbuf[6]); 				
	
	if (host)
		host -> interface = hardware;
	else
		log_error ("hardware address parameter %s",
				"not allowed here.");
//	break;
...
//	  case FIXED_ADDR6:
	...
	cache = NULL;
	if (parse_fixed_addr_param_test(&cache, hostipaddr, FIXED_ADDR)) {
		if (host) {
			if (host->fixed_addr) {
				option_cache_dereference(&cache, MDL);
			} else {
				host->fixed_addr = cache;
			}
		} else {
			option_cache_dereference(&cache, MDL);
		}
...
//	break;
	...
	if (deleted) {
		...
		status = enter_host (host, dynamicp, 0);
		if (status != ISC_R_SUCCESS)
			log_error ("host %s: %s", host -> name,
				    isc_result_totext (status));
	}
	host_dereference (&host, MDL);
}

int delete_host_by_name
(
	char* name
) 
{
	struct host_decl *hp = (struct host_decl *)0;
	if (host_hash_lookup(&hp, host_name_hash, (void *)name, strlen (name), MDL)) {
		log_debug("delete_host_by_name delete static host name %s \n", name);
		delete_host (hp, 0);
		host_dereference (&hp, MDL);
	}
	return 0;
...
/* test for no cfile configure subnet*/
...
)
{
	...
	if (group->shared_network) {
		parse_subnet_declaration((struct parse *)sub_conf, group->shared_network);
		log_info("we're in a subnet declaration, just do the parse\n");
		return 1;
	}
log_info("parse_statement_subnet_no_file run 1 \n");
	...
	parse_subnet_declaration_no_cfile(sub_conf, share);
log_info("parse_statement_subnet_no_file run 2 \n");
...
log_info("parse_statement_subnet_no_file run 3 \n");
		/* Make the shared network name from network number. */
		n = piaddrmask(&share->subnets->net, &share->subnets->netmask);
		...
		share->group->authoritative =
			share->subnets->group->authoritative;
		enter_shared_network(share);
log_info("parse_statement_subnet_no_file run 4 \n");
		
	}
	shared_network_dereference(&share, MDL);
log_info("parse_statement_subnet_no_file run 5 \n");

	return 1;
}

void parse_subnet_declaration_no_cfile
(
	struct subnet_config *sub_conf,
	struct shared_network *share
)
{
	...
	if (status != ISC_R_SUCCESS)
		log_fatal ("Allocation of new subnet failed: %s",
			   isc_result_totext (status));
log_info("parse_subnet_declaration_no_cfile run 2 \n");
	
	shared_network_reference (&subnet -> shared_network, share, MDL);
	
log_info("parse_subnet_declaration_no_cfile run 3 \n");
	if (!clone_group (&subnet -> group, share -> group, MDL))
		log_fatal ("allocation of group for new subnet failed.");
	subnet_reference (&subnet -> group -> subnet, subnet, MDL);
log_info("parse_subnet_declaration_no_cfile run 4 \n");
...
*/
	...
	iaddr.len = len;
	subnet -> netmask = iaddr;*/
	subnet->netmask = sub_conf->netmask;
log_info("parse_subnet_declaration_no_cfile run 6 \n");
	...
	if (host_addr (subnet -> net, subnet -> netmask)) {
		...
		maskstr = strdup (piaddr (subnet -> netmask));
		log_error ("subnet %s netmask %s: bad subnet number/mask combination.",
			    piaddr (subnet -> net), maskstr);
		free(maskstr);
		subnet_dereference (&subnet, MDL);
		/*
...
log_info("parse_subnet_declaration_no_cfile run 10 \n");

	common_subnet_parsing_no_cfile(sub_conf, share, subnet);
}
	
	
int
common_subnet_parsing_no_cfile
(
	struct subnet_config *sub_conf,
	struct shared_network *share,
	struct subnet *subnet
...
*/
log_info("common_subnet_parsing_no_cfile run 2 \n");
	/*
	parse_address_range_no_cfile (&(sub_conf->range_conf), subnet->group, SUBNET_DECL, (struct pool *)0,
				 (struct lease **)0);	
*/
	/* Add the subnet to the list of subnets in this shared net. */
	parse_pool_statement_no_file(&(sub_conf->range_conf), subnet->group, SUBNET_DECL);
...
log_info("common_subnet_parsing_no_cfile run 5 \n");
	
	subnet_dereference(&subnet, MDL);
	return 1;
}

void parse_pool_statement_no_file
(
...
)
{
	...
	if (type == SUBNET_DECL)
		...
		log_error("Dynamic pools are only valid inside "
				  "subnet or shared-network statements.");
		return;
	}
...
/*
	...
	parse_address_range_no_cfile (range_conf, group, SUBNET_DECL, pool, (struct lease **)0);
*/	
	parse_address_range_no_cfile (range_conf, group, SUBNET_DECL, pool, &lpchain);
...
/* address-range-declaration test for no cfile */

void parse_address_range_no_cfile
(
	struct range_config *range_conf,
	struct group *group,
	int type,
...
log_info("parse_address_range_no_cfile run 1 \n");
	low = range_conf->low;
	high = range_conf->high;
	...
	*/
log_info("parse_address_range_no_cfile run 2 \n");

	if (type == SUBNET_DECL) {
		subnet = group -> subnet;
		share = subnet -> shared_network;
	} else {
log_info("parse_address_range_no_cfile run 222 \n");
	
		share = group -> shared_network;
		
log_info("parse_address_range_no_cfile run 333 \n");
		for (subnet = share -> subnets;
		     subnet; subnet = subnet -> next_sibling) {
log_info("parse_address_range_no_cfile run 7777777 \n");

			net = subnet_number (low, subnet -> netmask);
			if (addr_eq (net, subnet -> net))
				break;
		}
log_info("parse_address_range_no_cfile run 4444 \n");
			 
		if (!subnet) {
			log_error("address range not on network %s", group -> shared_network -> name);
			log_error ("Be sure to place pool statement after %s",
				   "related subnet declarations.");
			return;
		}
	}
log_info("parse_address_range_no_cfile run 3 \n");
...
log_info("parse_address_range_no_cfile run 4 \n");
...
log_info("parse_address_range_no_cfile run 6 \n");

	/* Create the new address range... */
	new_address_range_no_cfile (low, high, subnet, pool, lpchain);
log_info("parse_address_range_no_cfile run 7 \n");

	pool_dereference (&pool, MDL);
}
	
void new_address_range_no_cfile
(
	struct iaddr low,
	struct iaddr high,
	struct subnet *subnet,
...
) 
{
    ...
    if (local_family != AF_INET6) {
        log_error("range6 statement is only supported in DHCPv6 mode.");
        return;
    }
	...
	/*if (token == SLASH) {
		...
		*/
		...
		if (range2cidr(&nets, &lo, &hi) != ISC_R_SUCCESS) {
			log_error("Error converting range to CIDR networks");
		}
...
*/
log_info("common_subnet_parsing_no_cfile_v6 run 2 \n");
	/*
	parse_address_range_no_cfile (&(sub_conf->range_conf), subnet->group, SUBNET_DECL, (struct pool *)0,
				 (struct lease **)0);	
*/
...
void parse_subnet6_declaration_no_cfile
(
	struct subnet_config *sub_conf,
	struct shared_network *share
)
{
 /* defined(DHCPv6) */
	...
	if (local_family != AF_INET6) {
		log_error("subnet6 statement is only supported in DHCPv6 mode.");
		return;
	}
...
save it static now, change later */
	...
	if (memcmp(&iaddr, &subnet->net, 16) != 0) {
		log_error( "subnet %s/%d: prefix not long enough for address.\n",
				piaddr(subnet->net), subnet->prefix_len);
		subnet_dereference(&subnet, MDL);
		return;
	}

	common_subnet_parsing_no_cfile_v6(sub_conf, share, subnet);

#endif /* defined(DHCPv6) */
}

int parse_statement_subnet_no_file_v6
(
	struct subnet_config *sub_conf,
	struct group *group
)
{
	...
	parse_subnet6_declaration_no_cfile(sub_conf, share);
...
)
{
	...
	sub_conf.net.iabuf[0] = 0x20;
	sub_conf.net.iabuf[1] = 0x1;
	sub_conf.net.iabuf[2] = 0xd;
	sub_conf.net.iabuf[3] = 0xb8;
	sub_conf.net.iabuf[4] = 0x0;
	sub_conf.net.iabuf[5] = 0x0;
	sub_conf.net.iabuf[6] = 0x0;
	sub_conf.net.iabuf[7] = 0x1;
	sub_conf.net.iabuf[8] = 0x0;
	sub_conf.net.iabuf[9] = 0x0;
	sub_conf.net.iabuf[10] = 0x0;
	sub_conf.net.iabuf[11] = 0x0;
	sub_conf.net.iabuf[12] = 0x0;
	sub_conf.net.iabuf[13] = 0x0;
	sub_conf.net.iabuf[14] = 0x0;
	sub_conf.net.iabuf[15] = 0x0;
	sub_conf.net.len = 16;
	
	sub_conf.netmask.iabuf[0] = 0xff;
	sub_conf.netmask.iabuf[1] = 0xff;
	sub_conf.netmask.iabuf[2] = 0xff;
	sub_conf.netmask.iabuf[3] = 0xff;
	sub_conf.netmask.iabuf[4] = 0xff;
	sub_conf.netmask.iabuf[5] = 0xff;
	sub_conf.netmask.iabuf[6] = 0xff;
	sub_conf.netmask.iabuf[7] = 0xff;
	sub_conf.netmask.iabuf[8] = 0x0;
	sub_conf.netmask.iabuf[9] = 0x0;
	sub_conf.netmask.iabuf[10] = 0x0;
	sub_conf.netmask.iabuf[11] = 0x0;
	sub_conf.netmask.iabuf[12] = 0x0;
	sub_conf.netmask.iabuf[13] = 0x0;
	sub_conf.netmask.iabuf[14] = 0x0;
	sub_conf.netmask.iabuf[15] = 0x0;
	sub_conf.netmask.len = 16;

	sub_conf.range_conf.low.iabuf[0] = 0x20;
	sub_conf.range_conf.low.iabuf[1] = 0x1;
	sub_conf.range_conf.low.iabuf[2] = 0xd;
	sub_conf.range_conf.low.iabuf[3] = 0xb8;
	sub_conf.range_conf.low.iabuf[4] = 0x0;
	sub_conf.range_conf.low.iabuf[5] = 0x0;
	sub_conf.range_conf.low.iabuf[6] = 0x0;
	sub_conf.range_conf.low.iabuf[7] = 0x1;
	sub_conf.range_conf.low.iabuf[8] = 0x0;
	sub_conf.range_conf.low.iabuf[9] = 0x0;
	sub_conf.range_conf.low.iabuf[10] = 0x0;
	sub_conf.range_conf.low.iabuf[11] = 0x0;
	sub_conf.range_conf.low.iabuf[12] = 0x0;
	sub_conf.range_conf.low.iabuf[13] = 0x0;
	sub_conf.range_conf.low.iabuf[14] = 0x0;
	sub_conf.range_conf.low.iabuf[15] = 129;
	sub_conf.range_conf.low.len = 16;

	sub_conf.range_conf.high.iabuf[0] = 0x20;
	sub_conf.range_conf.high.iabuf[1] = 0x1;
	sub_conf.range_conf.high.iabuf[2] = 0xd;
	sub_conf.range_conf.high.iabuf[3] = 0xb8;
	sub_conf.range_conf.high.iabuf[4] = 0x0;
	sub_conf.range_conf.high.iabuf[5] = 0x0;
	sub_conf.range_conf.high.iabuf[6] = 0x0;
	sub_conf.range_conf.high.iabuf[7] = 0x1;
	sub_conf.range_conf.high.iabuf[8] = 0x0;
	sub_conf.range_conf.high.iabuf[9] = 0x0;
	sub_conf.range_conf.high.iabuf[10] = 0x0;
	sub_conf.range_conf.high.iabuf[11] = 0x0;
	sub_conf.range_conf.high.iabuf[12] = 0x0;
	sub_conf.range_conf.high.iabuf[13] = 0x0;
	sub_conf.range_conf.high.iabuf[14] = 0x0;
	sub_conf.range_conf.high.iabuf[15] = 254;
	sub_conf.range_conf.high.len = 16;
	...
	memcpy(&addr, &(sub_conf.net), sizeof(struct iaddr));
	...
	discover_interfaces_test(1, ifname);
}
...
int set_server_duid_new(void)
{
	 ...
	 * set server duid
		*/ 
	//char *srvDuid = PRODUCT_MAC_ADDRESS;
	...
	putULong(duid.buffer->data + 2, HTYPE_ETHER);
	memcpy(duid.buffer->data + 4, mac, strlen(mac));
