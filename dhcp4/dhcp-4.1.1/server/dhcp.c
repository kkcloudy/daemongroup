...
/*add for request wrong vlan nak packet*/
	...
	ret = dhcp6_dbus_find_subnet_by_ifname(packet->interface->name, &subnode);	
...
//			log_debug("ipaddr->iabuf[%d] is %d \n", i, ipaddr->iabuf[i]);
		...
		if (cip.len == i) { 		
			log_info("dhcprequest run interface name %s\n", packet->interface->name);
		}
		else {		
			log_info("dhcprequest wrong vlan run interface name %s\n", packet->interface->name);
	...
	struct host_decl *hp;
{
	...
	if (!oc)
		oc = lookup_option (&dhcp_universe, packet -> options,
				    DHO_DHCP_PARAMETER_REQUEST_LIST);
	if (oc)
		t=evaluate_option_cache (&state -> parameter_request_list,
				       packet, lease, (struct client_state *)0,
				       packet -> options, state -> options,
				       &lease -> scope, oc, MDL);
	if(!t){
