...
again:
	do {
		...
		if (!strncmp(buf, "fe80", 4) || !strcmp(buf, "::1")) {
			memset(buf, 0, 256);
			goto again;
		}
#endif
...
log_debug("info name %s, add name %s 111111111\n", info.name, name);
...
log_debug("info name %s tmp name\n", info.name);
...
*/		
		...
		if ((tmp -> flags & INTERFACE_RUNNING) || 
			...
			log_debug("discover interface continue for owned interface %s\n", tmp->name);
			interface_dereference(&tmp, MDL);
			if(next)
	...
	log_debug("discover use interface %s\n", tmp->name);
...
log_debug("discover use interface %s kkkkkkkkk\n", tmp->name);
if(!tmp -> ifp)
log_debug("discover use interface %s kkkkkkkkk ifp is null\n", tmp->name);
		...
		last = tmp;
log_debug("discover use interface %s run 11111111111\n", tmp->name);
		...
		tmp -> index = -1;
log_debug("discover use interface %s run 333333333333\n", tmp->name);
...
got_one_v6(omapi_object_t *h) {
	...
	if (!dhcp_server_enable) {
		log_info("global dhcpv6 server is disable\n");
		return ISC_R_SUCCESS;
	}
