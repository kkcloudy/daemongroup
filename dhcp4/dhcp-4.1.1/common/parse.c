...
int parse_cshl_no_file(struct data_string *data, char* value);
int parse_option_data_test(struct expression **,	char *,int,	int ,unsigned int,struct option *);
int parse_option_name_test(	char* name,	int ipv6,int allocate,int *known,	struct option **opt);
int parse_option_statement_test(struct executable_statement **,char* value,int,int,unsigned int,struct option *,enum statement_op);
...
)
{
	...
	MRns_name_pton(str, compbuf + clen,
							sizeof(compbuf) - clen);
...
/*valnum is the times of set value*/
isc_result_t
set_server_option
(
	char* name,
...
)
{
	...
	if (check) {		
		parse_option_name_test(name, ipv6, 1, &known, &option);
	}
	else {	
		if (config_universe) {
	...
	parse_option_statement_test(&et, value, valnum, 1, del, option, supersede_option_statement);
	
	option_dereference(&option, MDL);
	
	if (opgroup -> statements) {			
		int multi = 0;
		for (ep = opgroup -> statements; ep -> next;
		     ep = ep -> next)
			if (ep -> refcnt > 1) 
				multi = 1;
		if (!multi) {
		...
		executable_statement_dereference (&opgroup -> statements,
						  MDL);
		executable_statement_reference (&opgroup -> statements,
						ep, MDL);
...
)
{
	...
	if (ipv6) {		
		uname = dmalloc (strlen (dhcp6) + 1, MDL);
		if (!uname)
			log_fatal ("no memory for uname information.");
		strcpy (uname, dhcp6);
		
		val = dmalloc (strlen (name) + 1, MDL);
		if (!val)
			log_fatal ("no memory for uname information.");
		strcpy ((char *)val, name);
	}
	else {	
		uname = dmalloc (strlen (name) + 1, MDL);
		if (!uname)
			log_fatal ("no memory for uname information.");
		strcpy (uname, name);
	}	
	...
	if (ipv6) {
		...
		if (!universe_hash_lookup (&universe, universe_hash,
					   uname, 0, MDL)) {
			log_error ("no option space named %s.", uname);
			return ISC_R_NOTFOUND;
		}
		log_debug("parse_option_name_test name is %s val is %s\n", uname, val);	
	} else {
	 ...
	 */
		...
		if (code == 0 || code == universe->end) {
			log_error("Option codes 0 and %u are illegal "
	 ...
	 */
		...
		log_error("no option named %s in space %s",
			   val, universe->name);
		dfree(uname, MDL);
		return ISC_R_NOTFOUND;
	}
...
)
{
	...
	unsigned  tlen = 0;	
	unsigned char *rvp;
	struct iaddr *ipv6adr = NULL;
	char * ptr = NULL;
	int v;
	int total = 0;

	/*for test add option52*/		

	ipv6adr = (struct iaddr *)value;
	
	if (!buffer_allocate (&data->buffer, 128, MDL))
	{
	...
	for(v = 0; v < 8 ; v++){
		if(ipv6adr[v].len){
			memcpy(ptr, ipv6adr[v].iabuf, ipv6adr[v].len);
			log_debug("option52 list ipv6 address %d is %s\n", v+1, piaddr(ipv6adr[v]));
			ptr += ipv6adr[v].len;
			total += ipv6adr[v].len;
		}
		else{
	...
	memcpy (rvp, ibuf, total);
	
	return 1;
}

int parse_option_token_test
(
...
)
{
	...
	struct iaddr addr;
	int compress;
	struct iaddr* value_addr = NULL;
	unsigned int value_int = 0;
	
log_info("parse_option_token fmt %c\n", **fmt);			
...
//	while(valnum) {
		...
		/*	
			  case 'U':
			token = next_token (&val, &len, cfile);
	log_debug("parse_option_token next token is %d val is %s\n", token, val);			
		...
		*/
				...
				if (local_family == AF_INET6) {
					log_info("test for add ipv6 option52");
					if (!del) {
						if (!parse_cshl_no_file_v6(&t->data.const_data, value)) {
					...
					if (!del) {
						if (!parse_cshl_no_file(&t->data.const_data, value)) {
							expression_dereference (&t, MDL);
							return 0;
						}
					}
	...
	/*		  	
			token = peek_token (&val, (unsigned *)0, cfile);
	log_debug("parse_option_token peek token is %d val is %s\n", token, val);				
	...
	log_debug("parse_option_token next token is %d val is %s\n", token, val);			
			...
			*/
			...
			if ((*fmt)[1] == 'c') {
				...
				compress = 0;
log_debug("parse_option_token_test for D value is %s, compress is %d \n", value, compress);
			t = parse_domain_list_no_file(value, compress);
...
//					return 0;
				value_addr = (struct iaddr*)value;
				addr.iabuf[0] = value_addr->iabuf[0];
				addr.iabuf[1] = value_addr->iabuf[1];
				addr.iabuf[2] = value_addr->iabuf[2];
				addr.iabuf[3] = value_addr->iabuf[3];
				if (!del) {
					len = 4;
				}
				else {
				...
				if (!make_const_data (&t, addr.iabuf, len,
							  0, 1, MDL))
					return 0;
//			}
			break;
			  ...
			  case '6': 
				value_addr = (struct iaddr*)value;
				memcpy(&addr, value_addr, sizeof(struct iaddr));
				log_debug("parse_option_token_test token 6 addrss is %s\n", piaddr(addr));
...
//			value_int = atoi(value);
			...
			if (!make_const_data (&t, buf, len, 0, 1, MDL))
				return 0;
			break;
  /*			
			  case 'L': 
			...
			break;
			  ...
			  case 'B':
				...
				if (!make_const_data (&t, (unsigned char *)value, 1, 0, 1, MDL))
					return 0;
...
//	}
	if (expr) {
log_debug("parse_option_token_test expr not null\n");		
...
)
{
	...
	if(NULL == option->format){
		log_debug("format is null \n");
	}else{
		log_debug("format is %s\n", option->format);
	}
...
log_debug("parse_option_data fmt %c\n", *fmt);	

		do {
log_debug("parse_option_data fmt times %c uniform %d\n", *fmt, uniform);	
			...
			tmp = *expr;
			*expr = NULL;
			if (!parse_option_token_test(expr, value, valnum, del, &fmt, tmp,
                                                uniform, lookups)) {
				if (fmt [1] != 'o') {
					if (tmp)
...
log_debug("parse_option_data fmt run over \n");	

        return 1;
}

int parse_option_statement_test 
(
...
)
{
	...
	if(NULL == option){
		log_error("option struct is null\n");
		return 1;
	}
	/*for >= 2 ip */
	if(valnum&0xff) {
		addr = malloc(sizeof(struct iaddr));
		count = (valnum>>8)&0xff;
		while (count--) {

			memset(addr, 0, sizeof(struct iaddr));
			memcpy(addr, value+i*sizeof(struct iaddr), sizeof(struct iaddr));
log_debug("parse_option_statement_test addr ip %x.%x.%x.%x len is %d \n",
			addr->iabuf[0],addr->iabuf[1],addr->iabuf[2],addr->iabuf[3],addr->len);

			if (! parse_option_data_test(&expr, (char *)addr, valnum, lookups, del, option)) {
				return 0;
			}
			i++;
		}
		...
		if (! parse_option_data_test(&expr, value, valnum, lookups, del, option)) {
			return 0;
		}
	}
...
{
	u_int8_t ibuf [128];
	unsigned i = 0, ilen = 0, tlen = 0;
	struct dcli_option43* option43 = (struct dcli_option43*)value;	
	unsigned char *rvp;
/*	struct option_tag *sl = (struct option_tag *)0;
	struct option_tag *next, **last = &sl;
	enum dhcp_token token;
	...
	convert_num (cfile, &ibuf [ilen++], val, 16, 8);*/
	ibuf[0] = option43->type;
	ibuf[1] = option43->len;
	for (i = 0; i < option43->ipnum; i++) {
		ibuf[2+i*4] = ((option43->ip[i])>>24)&0xff;
		ibuf[3+i*4] = ((option43->ip[i])>>16)&0xff;
		ibuf[4+i*4] = ((option43->ip[i])>>8)&0xff;
		ibuf[5+i*4] = (option43->ip[i])&0xff;
	}
	ilen = option43->len + 2;
	if (!buffer_allocate (&data->buffer, tlen + ilen, MDL))
		log_fatal ("no memory to store octet data.");
