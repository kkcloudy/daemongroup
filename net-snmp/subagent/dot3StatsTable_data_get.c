 ...
 *       version : 12088 $ of $ 
 *
 * $Id: dot3StatsTable_data_get.c,v 1.1 2010/04/14 08:44:56 tangsiqi Exp $
 */
/*
...
/**
 * set mibStatsTable data by index(es)
 *
 * @param  mib data structure
 * @param dot3StatsIndex_val
 *
 * @retval MFD_SUCCESS     : success.
 * @retval MFD_ERROR       : other error.
 *
 * @remark
 *  This convenience function is useful for setting all the MIB data by index
 *  components with a single function call. It is assume that the C values
 *  have already been mapped from their native/rawformat to the MIB format.
 */
int
dot3StatsTable_set_data_by_dbus(dot3StatsTable_data *rowData,
                                   long dot3StatsIndex_val)
{
    DEBUGMSGTL(("verbose:dot3StatsTable:dot3StatsTable_index_to_set_data", "called\n"));

    /*
     * dot3StatsIndex(1)/InterfaceIndex/ASN_INTEGER/long(long)//l/A/w/e/R/d/H 
     */
     #if 0
    printf("has enter set data on load!dot3StatsIndex_val=%d\n",dot3StatsIndex_val);
	rowData->dot3StatsAlignmentErrors = 1;
	rowData->dot3StatsCarrierSenseErrors = 2;
	#else
	unsigned char port,slot;
	port = (unsigned char)dot3StatsIndex_val;
	slot = (unsigned char)(dot3StatsIndex_val>>8);
	printf("port=%d--slot=%d\n",port,slot);
	struct eth_port_counter_s *e_stat;
	e_stat = (struct eth_port_counter_s *)malloc(sizeof(struct eth_port_counter_s));
	memset(e_stat, 0, sizeof(struct eth_port_counter_s));
	//show_eth_port_statistic(e_stat, slot, port);

	/*
	printf("e_stat->tx.sent_deferred=%lld\n",e_stat->tx.sent_deferred);
	printf("e_stat->tx.excessiveCollision=%lld\n",e_stat->tx.excessiveCollision);
	printf("e_stat->oct_s.badCRC=%lld\n",e_stat->otc.badCRC);
	printf("e_stat->rx_s.overSizepkt=%lld\n",e_stat->rx.overSizepkt);
	printf("e_stat->oct_s.late_collision=%lld\n",e_stat->otc.late_collision);
	printf("e_stat->tx_s.sentMutiple=%lld\n",e_stat->tx.sentMutiple);
	printf("e_stat->rx_s.errorframe=%lld\n",e_stat->rx.errorframe);
	printf("e_stat->tx_s.crcerror_fifooverrun=%lld\n",e_stat->tx.crcerror_fifooverrun);
	*/

	rowData->dot3StatsAlignmentErrors = 0; /*not support*/
	rowData->dot3StatsCarrierSenseErrors = 0; /*not support*/
	rowData->dot3StatsDeferredTransmissions = e_stat->tx.sent_deferred;
	rowData->dot3StatsDuplexStatus = 0; /*no need*/

	rowData->dot3StatsExcessiveCollisions = e_stat->tx.excessiveCollision;
	rowData->dot3StatsFCSErrors = e_stat->otc.badCRC;
	rowData->dot3StatsFrameTooLongs = e_stat->rx.overSizepkt;
	rowData->dot3StatsLateCollisions = e_stat->otc.late_collision;
	rowData->dot3StatsMultipleCollisionFrames = e_stat->tx.sentMutiple;
	rowData->dot3StatsSingleCollisionFrames = 0; /*not support*/
	rowData->dot3StatsSQETestErrors = 0; /*not support*/
	rowData->dot3StatsSymbolErrors = 0; /*not support*/
	rowData->dot3StatsInternalMacReceiveErrors = e_stat->rx.errorframe;
	rowData->dot3StatsInternalMacTransmitErrors = e_stat->tx.crcerror_fifooverrun;
...
/*add by tsq 2010-4-9    target: to generate a index by slot and port */
unsigned long dot3stats_set_index_by_slot_port(unsigned char slot, unsigned char port)
{
	unsigned long index = 0;
	printf("slot=%u--port=%u\n",slot,port);
	index = ((slot<<8) | (port));
	printf("index=%ld\n",index);
	return index;
}
                             ...
                             u_long * dot3StatsAlignmentErrors_val_ptr)
{
   /** we should have a non-NULL pointer */
	...
	printf("has enter dot3StatsAlignmentErrors_get and value=%d\n",rowreq_ctx->data.dot3StatsAlignmentErrors);
                       ...
                       u_long * dot3StatsFCSErrors_val_ptr)
{
   /** we should have a non-NULL pointer */
	...
	printf("has enter dot3StatsFCSErrors_get and value=%d\n",rowreq_ctx->data.dot3StatsFCSErrors);
