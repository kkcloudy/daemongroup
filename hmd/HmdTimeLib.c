...
static void timer_dequeue(tl_timer_t * /*t*/, int isFree);
...
static void timer_free(tl_timer_t *t, int isFree) {
	...
	if (t == NULL) return;
	if(isFree){
		if(t->handler_arg != NULL){
			common *w = (common*)t->handler_arg;
			free(w);
			w = NULL;
			t->handler_arg = NULL;
		}
...
static void *cronometer(void *arg) {
	hmd_pid_write_v2("cronometer",HOST_SLOT_NO);
	...
	while(1) {
		...
		struct timeval		abs_now;
		gettimeofday(&abs_now, NULL);
		if(TV_LESS_THAN(abs_now, timerq.first->timeout)){
			pthread_mutex_unlock(&timerq.mutex);
			continue;
		}
