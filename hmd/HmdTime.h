...
typedef int             int4; 
#define TIMER_SIGNAL_TYPE       HMD_TIMER_INTERNAL_SIGNAL
...
struct timer { 
		...
		void (*ast)(void*, int);
		HmdTimerArg myArg;
} timerq[HMD_TIMER_MAX];                   /* timer queue */ 
...
void _clkini();
int _setimr(unsigned int timeout, int2 *event, void (*ast)(void*, int), HmdTimerArg myArg, int4 arg, char *pname, HmdTimerID *idPtr);
int _cantim(HmdTimerID *idPtr);
void start_timer(struct timer *t); 
void cancel_itimer();
void update_all_timers_by(unsigned int time); 

__inline__ HMDBool HmdTimersInitLib() {
	sigemptyset(&mask);
...
void HmdTimersInit()
{
	memset(timerq, 0, sizeof(struct timer) * HMD_TIMER_MAX);
}
...
__inline__ HMDBool HmdTimerCreate(int sec, HmdTimerID *idPtr, void (*hdl)(HmdTimerArg, HmdTimerID), HmdTimerArg arg) {
 	int2 ev;
	if(idPtr == NULL || hdl == NULL) return HMD_FALSE;
...
__inline__ HmdTimerArg HmdTimerGetArg(HmdTimerID *idPtr) 
{
	HmdTimerArg pResult;
...
__inline__ HMDBool HmdTimerSetArg(HmdTimerID *idPtr, HmdTimerArg arg) 
{
	if(idPtr == NULL)
	{
		return HMD_FALSE;
...
__inline__ HMDBool HmdTimerDestroy(HmdTimerID *idPtr) {
	if(idPtr == NULL) return HMD_FALSE;

	if(_cantim(idPtr) == TRUE) return HMD_TRUE;
	else return HMD_FALSE;
}
...
*/
	...
	if ( sigaction(TIMER_SIGNAL_TYPE, &siga, NULL) ) {
        ...
        for (t=timerq;t<&timerq[HMD_TIMER_MAX];t++) { 
                t->timeout = 0; 
                t->event = 0; 
        } 
}
...
int _setimr(	unsigned int timeout,		/* time to wait in 10msec ticks */ 
        ...
        *event = 0; 
        ...
        disable_interrupts(); 
        for (*idPtr = 0, t=timerq;t<&timerq[HMD_TIMER_MAX];t++, (*idPtr)++) { 
                if (!timer_inuse(t)) { 
			free_timer = t; 
                        break; 
                } 
        } 
        if (t == &timerq[HMD_TIMER_MAX]) {
			enable_interrupts()
...
void update_all_timers_by(unsigned int time) { 
	struct timer *t; int id;
        for (id=0, t=timerq;t<&timerq[HMD_TIMER_MAX];t++, id++) { 
		if (t->timeout) {
                        if (time < t->timeout) { 
				t->timeout -= time;
                        } else {
...
struct timer *shortest_timer() { 
        ...
        for (s_t = 0,t=timerq;t<&timerq[HMD_TIMER_MAX];t++) { 
