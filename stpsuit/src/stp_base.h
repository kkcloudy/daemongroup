...
#define RSTP_ERRORS { \
  ...
  CHOOSE(STP_Small_Max_Hops),                      \
  CHOOSE(STP_Large_Max_Hops),                      \
  CHOOSE(STP_Forward_Delay_And_Max_Age_Are_Inconsistent),\
  CHOOSE(STP_Hello_Time_And_Max_Age_Are_Inconsistent),  \
  CHOOSE(STP_Hello_Time_And_Forward_Delay_Are_Inconsistent),  \
  ...
  CHOOSE(STP_BRIDGE_NOTFOUND),                            \
  CHOOSE(STP_CREATE_PORT_FAIL),                            \
  CHOOSE(STP_PORT_NOTFOUND),                            \
...
#define STP_CIST_ID   0
#define STP_MSTI_MAX    64 
#define STP_MAX_VID  4094
...
#ifdef __LINUX__
extern char* stp_bridge_sprint_timestamp (void);
#define stp_trace(F, B...) stp_syslog_dbg("%s:" F "\n", stp_bridge_sprint_timestamp(), ##B)
#else
