    ...
    case M_UNIXTIME_STAMP:
      {
        ...
        switch (id)
          ...
          case M_MONTH:
            ...
            break;
          case M_MONTH_STR:
            g_string_sprintfa(result, "%s", mm2str(tm->tm_mon + 1));
            break;
          case M_DAY:
          ...
          case M_SEC:
            ...
            break;
					case M_MSEC:
            g_string_sprintfa(result, "%03d", stamp->time.tv_usec/1000);
            break;
          case M_DATE:
          case M_STAMP:
          case M_ISODATE:
        ...
        if (msg->saddr && g_sockaddr_inet_check(msg->saddr)) 
        ...
        else 
          {
            ip = "127.0.0.1";
				...
				/*CID 13708  (use_invalid: Using "ip", which points to an out-of-scope variable "buf") no problem*/
		...
		case M_HOST_NAME:
			{
				...
				if(!hostname){
					...
					fd = open("/etc/hostname", O_RDONLY | O_NONBLOCK, S_IRUSR);
					read(fd, hostname,256);
					close(fd);
					while(hostname[i++] != 0)
					{
						if( hostname[i] == '\n' ){
				...
				g_string_sprintfa(result, "%s", hostname);
			}
			break;
    default:
