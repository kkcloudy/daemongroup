...
log_writer_queue(LogPipe *s, LogMessage *lm, gint path_flags)
{
  LogWriter *self = (LogWriter *) s;
	...
	if(self->control && self->control->user_check){
		flush = self->control->user_check(self->control);
	}
#endif /*end*/
  ...
  g_queue_push_tail(self->queue, GUINT_TO_POINTER(0x80000000 | path_flags));
	#if 1/*start dongshu*/
  g_queue_push_tail(self->queue, GUINT_TO_POINTER(0x80000000 | flush));
	#endif/*end*/
}
...
#if 0/* start dongshu*/
static void
log_writer_flush_check(LogWriter *self, FDWrite *fd)
{
	gboolean ret; 

	msg_notice("log_writer_flush_check running...",NULL);
	msg_notice("******",evt_tag_printf("user_check func","%p",self->control->user_check),NULL);
	if(self->control && self->control->user_check){
		ret = self->control->user_check(self->control);
	}
	
	msg_notice("******",evt_tag_printf("user check return","%s",ret==TRUE?"TRUE":"FALSE"),NULL);
	
	if(ret == TRUE && fd->flush){
		msg_notice("******",evt_tag_printf("flush func","%p",fd->flush),NULL);
	  fd->flush(fd);
	}
	return;
}
...
#if 0 /*start dongshu*/
	log_writer_flush_check(self,fd);
#endif /*end*/
  ...
  while (!g_queue_is_empty(self->queue))
    {
...
#if 1/*start dongshu*/
		flush = GPOINTER_TO_UINT (g_queue_pop_head(self->queue)) & 0x7FFFFFFF;
		//msg_notice("******",evt_tag_printf("flush flag after queue: ","%d", flush),NULL);
		if(flush == 1 && fd->flush){
			fd->flush(fd);
		}
#endif/*end*/
      ...
      if (line->len)
        {
      rc = fd_write(fd, line->str, line->len); /*write log*/
