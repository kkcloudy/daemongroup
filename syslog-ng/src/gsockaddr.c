...
g_bind(int fd, GSockAddr *addr)
{
  GIOStatus rc;
	...
	/*CID 13873 :(check_after_deref: Null-checking "addr" suggests that it may be null) */
	if(!addr){
...
g_sockaddr_inet_bind_prepare(int sock, GSockAddr *addr)
{
  int tmp = 1;
	...
	/*CID 10355:(check_return: Calling function "setsockopt(sock, 65535, 4, &tmp, 4U)" )*/
  ...
  for (port = addr->min_port; port <= addr->max_port; port++)
    {
      ...
      if (bind(sock, (struct sockaddr *) &addr->sin, addr->salen) == 0)
        {
        	/* CID 10473 : (copy_paste_error: "last_port" in "addr->last_port" looks like a copy-paste error.)  no problem*/
          addr->last_port = port + 1;
          return G_IO_STATUS_NORMAL;
        }
    }
