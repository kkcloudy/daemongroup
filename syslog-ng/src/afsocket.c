  ...
  if (dir & AFSOCKET_DIR_RECV)
    {
      if (sock_options->rcvbuf){
				/* CID 10353 (#1 of 3): Unchecked return value from library */
        if(setsockopt(fd, SOL_SOCKET, SO_RCVBUF, &sock_options->rcvbuf, sizeof(sock_options->rcvbuf)))
					return FALSE;
      }
    }
  if (dir & AFSOCKET_DIR_SEND)
    {
      if (sock_options->sndbuf){
				/* CID 10353 (#2 of 3): Unchecked return value from library */
        if(setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &sock_options->sndbuf, sizeof(sock_options->sndbuf)))
					return FALSE;
      }
      if (sock_options->broadcast){
				/* CID 10353 (#3 of 3): Unchecked return value from library */
        if(setsockopt(fd, SOL_SOCKET, SO_BROADCAST, &sock_options->broadcast, sizeof(sock_options->broadcast)))
					return FALSE;
      }
    }
...
afsocket_sd_init(LogPipe *s, GlobalConfig *cfg, PersistentConfig *persist)
{
  ...
  if (!self->bind_addr)
  {
    msg_error("No bind address set;", NULL);
		/* fix CID 11058 #2 and #3: Dereference after null check  */
		return FALSE;
  }
  ...
  if ((self->flags & AFSOCKET_KEEP_ALIVE))
    {
      GList *p;
			/* CID 11058 (#1 of 3) : Dereference after null check , no problem*/
  ...
  if (self->flags & AFSOCKET_STREAM)
    {
      ...
      if (sock == -1)
        {
					/* CID 11058 (#2 of 3) : Dereference after null check , fix at line of 430*/
  ...
  else
    {
      if (!self->connections)
        {
					/* CID 11058 (#3 of 3) : Dereference after null check , fix at line of 430*/
          if (!afsocket_open_socket(self->bind_addr, !!(self->flags & AFSOCKET_STREAM), &sock))
            return self->super.optional;
        }
      self->fd = -1;
      ...
      if (!self->setup_socket(self, sock))
        {
        	/* CID 11515 : Argument cannot be negative no problem */
          close(sock);
          return FALSE;
        }
...
#else
	log_writer_reopen(self->writer, fd_write_new(self->fd,NULL));
#endif/*end*/
  return TRUE;
}
...
afsocket_dd_init(LogPipe *s, GlobalConfig *cfg, PersistentConfig *persist)
{
  ...
  if (!self->writer)
  {
  	/* CID 11057 : Dereference after null check , no problem*/
