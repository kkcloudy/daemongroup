...
static size_t print_set(void *data, unsigned options)
{
	...
	size_t offset;
	void *ulldata;
	unsigned int aligned_size,orgsize;
...
/* 
	Because test_bit will operate in 64-bits width which matches 64-bits kernel test_and_set_bit operations.
	So we copy original members bitmaps to an 64-bits aligned buffer which can be correctly analyzed by test_bit.
*/	
	orgsize = setlist->members_size;
	aligned_size = ((orgsize / 8) + ((orgsize % 8) ? 1:0))*8;
	DP("member size [%lu], aligned 64-bits size [%lu]\n",orgsize, aligned_size);
	ulldata = malloc(aligned_size);
	memset(ulldata,0,aligned_size);
	memcpy(ulldata , data + offset, orgsize);

#ifdef IPSET_DEBUG	
		char tmpstr[256];
		int i;
		
		for (i=0; i < ((aligned_size < 256)?aligned_size:256); i++)
			{
				sprintf((char *)&tmpstr[i*2],"%02x", ((char *)ulldata)[i]);
			}
		DP("ULL Members Data: [%s]",tmpstr);
#endif

	
	if (options & OPT_SORTED)
		settype->printips_sorted(set, ulldata,
					 setlist->members_size, options);
	else
		settype->printips(set, ulldata,
				  setlist->members_size, options);
			 ...
			 unsigned options)
{
	...
	DP("load_set_list got size %d",size);
	if (size) {
		...
		DP("size %d req_size %d", size, req_size);
