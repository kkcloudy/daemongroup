...
#ifdef _NOT_USE_KERNEL_TEST_BIT_
 ...
 * WE USE SAME FUNC as KERNEL, yet long in kernel is 64-bit, so we use long long in user space
 */
#define SZLONG_LOG 6
#define SZLONG_MASK 63UL
static inline unsigned int test_bit(unsigned long nr, const volatile unsigned long long *addr)
{
	unsigned int ret;
	DP("id [%u] array index [%u], array val [%llu], right move [%u] got [%llu]",
		nr,
		nr >> SZLONG_LOG, 
		addr[nr >> SZLONG_LOG], 
		nr & SZLONG_MASK, 
		(addr[nr >> SZLONG_LOG] >> (nr & SZLONG_MASK)));

	ret = addr[nr >> SZLONG_LOG] >> (nr & SZLONG_MASK);
	DP("ret before & 1UL [%u]",ret);
	ret = 1ULL & ret;
	DP("ret after & 1UL [%u]",ret);
	return ret;
}
#endif
