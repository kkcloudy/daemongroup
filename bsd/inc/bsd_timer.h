...
typedef int             int4; 
#define TIMER_SIGNAL_TYPE       BSD_TIMER_INTERNAL_SIGNAL
...
struct timer { 
		...
		void (*ast)(void*, int);
		BsdTimerArg myArg;
} timerq[BSD_TIMER_MAX];                   /* timer queue */ 
...
void _clkini();
int _setimr(unsigned int timeout, int2 *event, void (*ast)(void*, int), BsdTimerArg myArg, int4 arg, char *pname, BsdTimerID *idPtr);
int _cantim(BsdTimerID *idPtr);
void start_timer(struct timer *t); 
void cancel_itimer();
void update_all_timers_by(unsigned int time); 

__inline__ BSDBool BsdTimersInitLib() {
	sigemptyset(&mask);
...
void BsdTimersInit()
{
	memset(timerq, 0, sizeof(struct timer) * BSD_TIMER_MAX);
}
...
__inline__ BSDBool BsdTimerCreate(int sec, BsdTimerID *idPtr, void (*hdl)(BsdTimerArg, BsdTimerID), BsdTimerArg arg) {
 	int2 ev;
	if(idPtr == NULL || hdl == NULL) return BSD_FALSE;
...
__inline__ BsdTimerArg BsdTimerGetArg(BsdTimerID *idPtr) 
{
	BsdTimerArg pResult;
...
__inline__ BSDBool BsdTimerSetArg(BsdTimerID *idPtr, BsdTimerArg arg) 
{
	if(idPtr == NULL)
	{
		return BSD_FALSE;
...
__inline__ BSDBool BsdTimerDestroy(BsdTimerID *idPtr) {
	if(idPtr == NULL) return BSD_FALSE;

	if(_cantim(idPtr) == TRUE) return BSD_TRUE;
	else return BSD_FALSE;
}
...
*/
	...
	if ( sigaction(TIMER_SIGNAL_TYPE, &siga, NULL) ) {
        ...
        for (t=timerq;t<&timerq[BSD_TIMER_MAX];t++) { 
                t->timeout = 0; 
                t->event = 0; 
        } 
}
...
int _setimr(	unsigned int timeout,		/* time to wait in 10msec ticks */ 
        ...
        *event = 0; 
        ...
        disable_interrupts(); 
        for (*idPtr = 0, t=timerq;t<&timerq[BSD_TIMER_MAX];t++, (*idPtr)++) { 
                if (!timer_inuse(t)) { 
			free_timer = t; 
                        break; 
                } 
        } 
        if (t == &timerq[BSD_TIMER_MAX]) {
			enable_interrupts()
...
void update_all_timers_by(unsigned int time) { 
	struct timer *t; int id;
        for (id=0, t=timerq;t<&timerq[BSD_TIMER_MAX];t++, id++) { 
		if (t->timeout) {
                        if (time < t->timeout) { 
				t->timeout -= time;
                        } else {
...
struct timer *shortest_timer() { 
        ...
        for (s_t = 0,t=timerq;t<&timerq[BSD_TIMER_MAX];t++) { 
