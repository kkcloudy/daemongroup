...
/*added by gxd*/
struct route_node *
route_node_match_4rip (struct route_table *table, struct prefix *p,struct connected *connect)
{
  struct route_node *node;
  struct route_node *matched;
...
route_node_delete (struct route_node *node)
{
  ...
  if(node->lock)
  {
  	zlog_err("%s : line %d , route node lock is not zero : %p .\n",__func__,__LINE__,node->lock);
  }
  if(node->info)
  {
  	zlog_err("%s : line %d , route node info is not null : %p .\n",__func__,__LINE__,node->info);
  }
