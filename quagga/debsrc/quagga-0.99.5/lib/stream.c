...
#else
	if(!new || STREAM_SIZE(new) < src->endp)
	{
		zlog_warn ("stream_copy(): ARG ERROR!");
...
stream_read_unblock (struct stream *s, int fd, size_t size)
{
  ...
  /*CID 10317 (#1 of 2): Unchecked return value from library (CHECKED_RETURN)
	4. check_return: Calling function "fcntl(fd, 4, val | 0x80)" without checking return value. This library function may fail and return an error code.
	Add check return value.*/
   /*fcntl (fd, F_SETFL, val|O_NONBLOCK);*/
  ret = fcntl (fd, F_SETFL, val|O_NONBLOCK);
  if(ret < 0)
  	zlog_warn("%s : line %d , fcntl set noblock failed(%s).\n",__func__,__LINE__,safe_strerror(errno));
  
  nbytes = read (fd, s->data + s->endp, size);

  /*CID 10317 (#2 of 2): Unchecked return value from library (CHECKED_RETURN)
...
stream_read_try(struct stream *s, int fd, size_t size)
{
  ...
  char *ptr=NULL;/**add by gjd for debug**/
  STREAM_VERIFY_SANE(s);
  ...
  if ((nbytes = read(fd, s->data + s->endp, size)) >= 0)
    {
	#ifdef STREAM_DEBUG
    	{
	      ptr = s->data + s->endp;
		  zlog_debug("%s\n",ptr);
		  zlog_debug("%s : line %d, s->getp = %d, s->endp = %d, s->size = %d\n",
                ...
                size_t size)
{
  ...
  assert (msgh->msg_iovlen > 0);  
	...
	{
		zlog_warn("%s: msgh->msg_iovlen=%d", __func__,msgh->msg_iovlen);
