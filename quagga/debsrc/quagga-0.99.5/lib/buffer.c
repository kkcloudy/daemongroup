		     ...
		     int erase_flag, int no_more_flag)
{
     ...
     example: Solaris2.6 are defined IOV_MAX size at 16.     */
  {
    ...
    while (iov_index > 0)
	 ...
	 /*CID 12128 (#1 of 1): Out-of-bounds access (OVERRUN)
	 35. overrun-buffer-arg: Overrunning buffer pointed to by "c_iov" of 3 8-byte elements 
	 by passing it to a function which accesses it at element index 1023 (byte offset 8184)
	 using argument "iov_size" (which evaluates to 1024).*/
	 /*But this func buffer_flush_window, now we donnot use .*/
...
in one shot. */
/*gujd: 2013-01-16, pm 4:48. Make sure IOV_MAX is 1024.*/
#ifdef IOV_MAX
