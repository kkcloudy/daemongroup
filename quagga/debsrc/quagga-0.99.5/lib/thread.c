		 ...
		 int (*func) (struct thread *), void *arg, int fd, const char* funcname)
{
  ...
  assert (m != NULL);
	...
	if(!m)
	{
		zlog (NULL, LOG_WARNING, "%S m is NULL\n", __func__);
		return 	NULL;
	}
#endif
                                  ...
                                  const char* funcname)
{
  ...
  if(type != THREAD_TIMER && type != THREAD_BACKGROUND)
  	{
		  zlog (NULL, LOG_WARNING, "%S type is error\n", __func__);
		  return  NULL;
	  }
#endif
...
int thread_handle_by_errfd(struct thread_list *list,int fd)
{
	
	struct thread *thread;
	struct thread *next;
	int num = 0;/*add initialize 0*/
	
	for (thread = list->head; thread; thread = next)
	{
		next = thread->next;

		if (THREAD_FD (thread)==fd)
		{
			zlog_err("FD error: func %s cancel it",thread->funcname);
			thread_cancel(thread);
			/*CID 14330 (#1 of 1): Uninitialized scalar variable (UNINIT)
			4. uninit_use: Using uninitialized value "num".
			Add initialize num = 0*/
			num++;
		}
	}
	
	/*CID 11353 (#1 of 1): Missing return statement (MISSING_RETURN)
...
thread_checkfd (struct thread_master *m)
{
	...
	{
		if(FD_ISSET (i, &m->readfd))
		{
			if(fcntl(i, F_GETFL, 0) == -1)
			{
				thread_handle_by_errfd(&m->read,i);
			}
		
		}

		if(FD_ISSET (i, &m->writefd))
		{
			if( fcntl(i, F_GETFL, 0) == -1)
			{
				thread_handle_by_errfd(&m->write,i);
			}
