...
config_show_distribute (struct vty *vty)
{
  ...
  if (dist && (dist->list[DISTRIBUTE_OUT] || dist->prefix[DISTRIBUTE_OUT]))
    {
      vty_out (vty, "  Outgoing update filter list for all interface : set");
      if (dist->list[DISTRIBUTE_OUT])
	  	vty_out (vty, " Filter name : %s", 
	  		dist->list[DISTRIBUTE_OUT]);
      if (dist->prefix[DISTRIBUTE_OUT])
		vty_out (vty, " Prefix-list name : %s",
			 dist->prefix[DISTRIBUTE_OUT]);
	  vty_out (vty,"%s",VTY_NEWLINE);
    }
  else
    vty_out (vty, "  Outgoing update filter list for all interface : not set%s", VTY_NEWLINE);
    ...
    for (mp = disthash->index[i]; mp; mp = mp->next){
	   	...
	   	if (dist->ifname)
	   	  if (dist->list[DISTRIBUTE_OUT] || dist->prefix[DISTRIBUTE_OUT]){
	   	      vty_out (vty, "  Outgoing update filter list for interface : %s",
			  	dist->ifname);
	   	      if (dist->list[DISTRIBUTE_OUT])
	   		  	vty_out (vty, " Filter name : %s", 
	   		  		dist->list[DISTRIBUTE_OUT]);
  ...
  if (dist && (dist->list[DISTRIBUTE_IN] || dist->prefix[DISTRIBUTE_IN])){
      vty_out (vty, "  Incoming update filter list for all interface : set");
      if (dist->list[DISTRIBUTE_IN])
		vty_out (vty, " Filter name : %s", 
    ...
    vty_out (vty, "  Incoming update filter list for all interface : not set%s", VTY_NEWLINE);
    ...
    for (mp = disthash->index[i]; mp; mp = mp->next){
		...
		if (dist->ifname)
		  if (dist->list[DISTRIBUTE_IN] || dist->prefix[DISTRIBUTE_IN]){
		      vty_out (vty, "  Incoming update filter list for interface : %s",
			  	dist->ifname);
		      if (dist->list[DISTRIBUTE_IN])
				vty_out (vty, " Filter name : %s", 
					dist->list[DISTRIBUTE_IN]);
		      if (dist->prefix[DISTRIBUTE_IN])
				vty_out (vty, " Prefix-list name : %s%s",
				 	dist->prefix[DISTRIBUTE_IN]);
			  vty_out (vty,"%s",VTY_NEWLINE);
		  }
    }
