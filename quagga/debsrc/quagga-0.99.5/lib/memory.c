...
/* Memory reallocation. */
void *
z_realloc (int type, void *ptr, size_t size)
{
  void *memory;
...
/* String duplication. */
char *
z_strdup (int type, const char *str)
{
  void *dup;
...
mtype_zmalloc (const char *file, int line, int type, size_t size)
{
  ...
  memory = z_malloc (type, size);
  mtype_log ("z_malloc", memory, file, line, type);
...
mtype_zrealloc (const char *file, int line, int type, void *ptr, size_t size)
{
  ...
  memory = z_realloc (type, ptr, size);
