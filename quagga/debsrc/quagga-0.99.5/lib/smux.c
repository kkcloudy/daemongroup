		   ...
		   long errindex, u_char val_type, void *arg, size_t arg_len)
{
  ...
  if (debug_smux)
    {
      zlog_debug ("Enter func %s",__func__);
      zlog_debug ("SMUX GETRSP send");
      zlog_debug ("SMUX GETRSP reqid: %ld", reqid);
    }
  if(debug_smux)
	  smux_oid_dump( "smux_getresp_send", objid, objid_len );
  ...
  if (debug_smux)
    zlog_debug ("SMUX GETRSP errstat: %ld  len =%d ptr=%p\n", errstat, len,ptr);
  ...
  if(!ptr)
  	return;
  if (debug_smux)
    zlog_debug ("SMUX GETRSP errindex: %ld len=%d ptr=%p\n", errindex, len,ptr);
  ...
  if (debug_smux)
    zlog_debug ("SMUX GETRSP end errindex: %ld len=%d ptr=%p\n", errindex, len,ptr);
  ...
  if (debug_smux)
	  zlog_debug ("SMUX GETRSP %d end  len=%d ptr=%p\n",__LINE__, len,ptr);
  ...
  ret = send (smux_sock, buf, (ptr - buf), MSG_NOSIGNAL);/*when socket break , flag MSG_NOSIGNAL can avoid snmp process down*/
  if(ret==-1)
  	{
		zlog_err ("%s line %d send message error : %s", __func__,__LINE__,safe_strerror (errno));
    }
	      ...
	      u_char *val_type,void **val, size_t *val_len)
{
  ...
  if (debug_smux)
  {
	zlog_debug ("smux_getnext treelist %p", treelist );
	zlog_debug ("reqid = %d.%d.%d.%d.%d.%d.%d.%d", reqid[0], reqid[1], reqid[2],
						reqid[3], reqid[4], reqid[5], reqid[6], reqid[7] );
  }
  ...
  *reqid_len = savelen;
  ...
  if (debug_smux)
	zlog_debug ("smux_getnext SNMP_ERR_NOSUCHNAME");
...
smux_read (struct thread *t)
{
  ...
  if (len < 0)
    {
      ...
      if (++fail < SMUX_MAX_FAILURE){
		...
		zlog_warn ("restart smux_event (SMUX_CONNECT, 0)");
  ...
  if (ret < 0)
    {
		zlog_warn ("SMUX smux_parse error ");
#if 0
			  close (sock);
			  smux_sock = -1;
#else
...
smux_open (int sock)
{
  ...
  if (debug_smux)
    {
      zlog_debug ("Before asn_build_header:\n", progname);
      zlog_debug ("ptr=%p len = %d\n", ptr,len);
    }
  ...
  if (debug_smux)
    {
      zlog_debug ("After asn_build_header:\n");
      zlog_debug ("ptr=%p len = %d\n", ptr,len);
    }
  ...
  if (debug_smux)
    {
      zlog_debug ("Before asn_build_int:\n");
      zlog_debug ("ptr=%p len = %d version=%ul \n", ptr,len,version);
    }
  ...
  if (debug_smux)
    {
      zlog_debug ("After asn_build_int:\n");
      zlog_debug ("ptr=%p len = %d version=%ul \n", ptr,len,version);
    }
  ...
  if (debug_smux)
    {
      zlog_debug ("Before asn_build_objid:\n");
      zlog_debug ("ptr=%p len = %d smux_oid=%s smux_oid_len=%d\n", ptr,len,smux_oid,smux_oid_len);
    }
  ...
  if (debug_smux)
    {
      zlog_debug ("After asn_build_objid:\n");
      zlog_debug ("ptr=%p len = %d smux_oid=%s smux_oid_len=%d\n", ptr,len,smux_oid,smux_oid_len);
    }
  ...
  if (debug_smux)
    {
      zlog_debug ("Before asn_build_string:\n");
      zlog_debug ("ptr=%p len = %d progname=%s\n", ptr,len,progname);
    }
  ...
  if (debug_smux)
    {
      zlog_debug ("After asn_build_string:\n");
      zlog_debug ("ptr=%p len = %d \n", ptr,len);
    }
  ...
  if (debug_smux)
    {
      zlog_debug ("Before asn_build_string:\n");
      zlog_debug ("ptr=%p len = %d smux_passwd=%s\n", ptr,len,smux_passwd);
    }
  ...
  if (debug_smux)
    {
      zlog_debug ("Before asn_build_header:\n");
      zlog_debug ("buf=%p len = %d smux_passwd=%s\n", buf,len,smux_passwd);
    }
...
/*  return send (sock, buf, (ptr - buf), 0);*/
	ret = send (sock, buf, (ptr - buf), MSG_NOSIGNAL);/*when socket break , flag MSG_NOSIGNAL can avoid snmp process down*/
	if(ret==-1)
  	{
		zlog_err ("%s line %d send message error : %s", __func__,__LINE__,safe_strerror (errno));
		return ret;
  ...
  for (ALL_LIST_ELEMENTS (treelist, node, nnode, subtree))
    {
	  ...
	  if (debug_smux)
		zlog_debug ("Enter func %s line %d\n", __func__,__LINE__);
	  ...
	  if (debug_smux)
		zlog_debug ("Enter func %s len = %d\n", __func__,len);
      asn_build_header (buf, &len, (u_char) SMUX_RREQ, (ptr - buf) - 2);
	  if (debug_smux)
		zlog_debug ("Enter func %s line %d ptr=%p buf=%p\n", 
				__func__,__LINE__,ptr,buf);

	  ret = send (sock, buf, (ptr - buf), MSG_NOSIGNAL);/*when socket break , flag MSG_NOSIGNAL can avoid snmp process down*/
	  if (debug_smux)
		zlog_debug ("Enter func %s line %d ret = %d\n", __func__,__LINE__,ret);
      if (ret < 0)
      	{
			zlog_err ("%s line %d send message error : %s", __func__,__LINE__,safe_strerror (errno));
			return ret;
...
smux_connect (struct thread *t)
{
  ...
  if (debug_smux)
    zlog_debug ("Enter func %s line %d smux_sock=%d\n", __func__,__LINE__,smux_sock);
  if (smux_sock < 0)
    {

		zlog_debug ("Can not create smux_socket\n");
		smux_stop();
      if (++fail < SMUX_MAX_FAILURE){
...
smux_init (struct thread_master *tm)
{
  ...
  /* Install commands. */
  install_node (&smux_node, config_write_smux, "SMUX_NODE");
