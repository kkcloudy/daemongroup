...
#else
#define XMALLOC(mtype, size)       z_malloc ((mtype), (size))
#define XCALLOC(mtype, size)       z_calloc ((mtype), (size))
#define XREALLOC(mtype, ptr, size) z_realloc ((mtype), (ptr), (size))
#define XFREE(mtype, ptr)          do { \
                                     z_free ((mtype), (ptr)); \
                                     ptr = NULL; } \
                                   while (0)
#define XSTRDUP(mtype, str)        z_strdup ((mtype), (str))
#endif /* MEMORY_LOG */

/* Prototypes of memory function. */
extern void *z_malloc (int type, size_t size);
extern void *z_calloc (int type, size_t size);
extern void *z_realloc (int type, void *ptr, size_t size);
extern void  z_free (int type, void *ptr);
extern char *z_strdup (int type, const char *str);
