/*
   $Id: command.c,v 1.83 2011/05/19 06:55:28 houxiaoxian Exp $
...
const char *default_motd = "\r\n";
const char *default_motdfile = "/etc/motd";
vector get_cmdvec()
{
 	return cmdvec;
}
...
install_node (struct cmd_node *node, 
	      int (*func) (struct vty *),const char* node_name)
{
...
cmd_make_descvec (const char *string, const char *descstr)
{
  ...
  while (1)
      ...
      if (*cp == '|')
	{
	  if (! multiple)
	    {
			zlog_err("cmd_make_descvec line %d error",__LINE__);
	      EXIT(1);
	    }
	  cp++;
	}
...
config_write_host (struct vty *vty)
{
  ...
  if (zlog_default->maxlvl[ZLOG_DEST_SYSLOG] != LOG_DEBUG)
    {
		if (zlog_default->maxlvl[ZLOG_DEST_SYSLOG] == ZLOG_DISABLED)
			vty_out(vty,"no log syslog %s",VTY_NEWLINE);
		else{
...
cmd_try_do_shortcut (enum node_type node, char* first_word) {
  if ( first_word != NULL &&
    ...
    return 0;/*changed by scx for delete "do command"*/
			  ...
			  struct cmd_element **cmd)
{
  ...
  if (cmd)
    *cmd = matched_element;
  if (matched_element->daemon && !matched_element->func)
...
cmd_execute_command (vector vline, struct vty *vty, struct cmd_element **cmd,
  ...
  /* no command succeeded, reset the vty to the original node and
   ...
   /*CID 10795 (#1 of 1): Logically dead code (DEADCODE)
     dead_error_line: Execution cannot reach this statement "vty->node = onode;".*/
   #if 0
	 if ( tried )
	   vty->node = onode;
   #endif
			    ...
			    struct cmd_element **cmd)
{
  ...
  if (matched_count > 1)
  {
 // fprintf(stderr,"return AMBIGUOUS\n");
...
config_from_file (struct vty *vty, FILE *fp)
{
  ...
  while (fgets (vty->buf, VTY_BUFSIZ, fp))
	     ...
	     && ret != CMD_ERR_NOTHING_TODO && vty->node != CONFIG_NODE)
	{
      ...
      if (ret != CMD_SUCCESS && ret != CMD_WARNING && ret != CMD_FAILURE
       ...
       "Exit current mode and down to previous mode\n")
{
  ...
  switch (vty->node)
	...
	case POOL_NODE:
  	...
  	case IUH_NODE:
	case POOLV6_NODE: 
	case SUBNET6_CONF_NODE:
	case TRUNK_NODE:
	case ETH_PORT_NODE:
	case SYSTEM_DEBUG_NODE:
		vty->node = CONFIG_NODE;
    	break;
	case TIME_RANGE_NODE:
	case MIRROR_NODE: 
	case MST_NODE:
	case STP_NODE:
    ...
    case CWTUNNEL_NODE:
	case AP_GROUP_NODE:
	case	CBAT_NODE:/*wuwl add*/
	case	CNU_NODE:
	case  CBAT_RADIO_NODE:/*zhaoruijia add*/
#endif    
    case ZEBRA_NODE:
    case BGP_NODE:
    case RIP_NODE:
    ...
    case SNMP_SLOT_NODE:
        vty->node = SNMP_NODE;
        break;
	case EAG_RADIUS_NODE:
	case EAG_PORTAL_NODE:
	case EAG_NASID_NODE:
	case EAG_VLANMAP_NODE:
  	case EAG_INS_NODE:
		vty->index = NULL;
  		vty->node = EAG_NODE;
		break;
	...
	case	LOCAL_HANSI_POOL_NODE:			/* pool node */	
...
#endif
#ifdef _D_CC_
	case	HIDDEN_WLAN_NODE:
	case	HIDDEN_SECURITY_NODE:
	case	HIDDEN_CBAT_RADIO_NODE:
		vty->node = WOCDEBUG_NODE;
		break;	
#endif
    /* for distributed acl */
	case ACL_NODE_DISTRIBUTED:
		vty->node = CONFIG_NODE;
		break;	
	case DISTRIBUTED_DYNAMIC_TRUNK_NODE:
		vty->node = CONFIG_NODE;
		break;
	case DYNAMIC_TRUNK_NODE:
		vty->node = DISTRIBUTED_DYNAMIC_TRUNK_NODE;
		break;
	case ACL_GROUP_NODE:
	case ACL_EGRESS_GROUP_NODE:
		vty->node = ACL_NODE_DISTRIBUTED;
		break;
	case STP_NODE_DISTRIBUTED:
	    vty->node = CONFIG_NODE;
		break;
	case QOS_NODE_DISTRIBUTED:
		vty->node = CONFIG_NODE;
		break;
	case QOS_PROFILE_NODE:
	case POLICY_NODE:
    case POLICER_NODE:
	case SHQUEUE_NODE:
		vty->node = QOS_NODE_DISTRIBUTED;
      	break;
    case OUT_PROFILE_NODE:
		vty->node = POLICER_NODE;
		break;
	/* for distributed igmp */
	case IGMP_NODE_DISTRIBUTED:
		vty->node = CONFIG_NODE;
		break;
	case IGMP_VLAN_NODE:
...
#ifdef _D_CC_
	case	WOCDEBUG_NODE:
	case	HIDDEN_WLAN_NODE:			/* WLAN node */
	case	HIDDEN_SECURITY_NODE:			/* security node */
	case 	HIDDEN_CBAT_RADIO_NODE:
#endif
    case ZEBRA_NODE:
    case RIP_NODE:
       ...
       "Write to configuration file\n")
{
  ...
  if (fd < 0)
    {
      zlog_warn(  "!Can't open configuration file %s.%s", config_file_tmp,
	       VTY_NEWLINE);
      goto finished;
    }
  ...
  vty_out (file_vty, "\n! Rtsuit configuration saved from vty\n!   ");
  ...
  if (unlink (config_file_sav) != 0)
    if (errno != ENOENT)
      {
		zlog_warn("!Can't unlink backup configuration file %s.%s", config_file_sav,
		 VTY_NEWLINE);
        goto finished;
      }
  if (link (config_file, config_file_sav) != 0)
    {
      zlog_warn("!Can't backup old configuration file %s.%s", config_file_sav,
	        VTY_NEWLINE);
/*      goto finished; */
    }
  sync ();
  if (unlink (config_file) != 0)
    {
      zlog_warn("!Can't unlink configuration file %s.%s", config_file,
	        VTY_NEWLINE);
/*      goto finished; */
    }
  if (link (config_file_tmp, config_file) != 0)
    {
      zlog_warn("!Can't save configuration file %s.%s", config_file,
	       VTY_NEWLINE);
      goto finished;
    }
  sync ();
  ...
  if (chmod (config_file, CONFIGFILE_MASK) != 0)
    {
      zlog_warn("!Can't chmod configuration file %s: %s (%d).%s", 
       ...
       "Write to terminal\n")
{
  ...
  if (vty->type == VTY_SHELL_SERV)
	{
	  for (i = 0; i < vector_active (cmdvec); i++)
				if ((node = vector_slot (cmdvec, i)) && node->func && node->vtysh && node->node != IP_NODE)
				  {
				    (*node->func) (vty);
				  }
	}
  ...
  config_write_file(&config_write_file_cmd,vty,0,NULL);
  return CMD_SUCCESS;
}
/* Write current configuration into the terminal. */
DEFUN (config_write_terminal1,
       config_write_terminal_cmd1,
       "write terminal (access-list|interface|iproute|ospf|prefix-list|rip) ",
       "Write running configuration to memory, network, or terminal\n"
       "Write to terminal\n")
{
  unsigned int i,cmdnode;
  struct cmd_node *node;


		if(!strncmp(argv[0],"access-list",strlen(argv[0])))
		{
       ...
       "Contentes of startup configuration\n")
{
  ...
  if (confp == NULL)
    {
      vty_out (vty, "Configuration file is NULL%s",
	        VTY_NEWLINE);
      return CMD_WARNING;
    }
...
install_default (enum node_type node)
{
  ...
  if(node == STP_NODE_DISTRIBUTED
    ||node == ACL_NODE_DISTRIBUTED
  	||node == ACL_GROUP_NODE
	||node == ACL_EGRESS_GROUP_NODE	
	||node == IGMP_NODE_DISTRIBUTED
	||node == IGMP_VLAN_NODE
	||node == MLD_NODE_DISTRIBUTED
	||node == MLD_VLAN_NODE		
	||node ==  QOS_NODE_DISTRIBUTED
	||node ==  QOS_PROFILE_NODE
	||node ==  POLICY_NODE  
	||node ==  POLICER_NODE
	||node ==  SHQUEUE_NODE
	||node ==  OUT_PROFILE_NODE
	||node ==  TIME_RANGE_NODE
	||node == MIRROR_NODE
	||node == TRUNK_NODE
	||node == ETH_PORT_NODE
	||node == MST_NODE
	...
	||node == CWTUNNEL_NODE
	||node == AP_GROUP_NODE	
	||node == AP_GROUP_WTP_NODE
	||node == AP_GROUP_RADIO_NODE
	||node == HANSI_SECURITY_NODE
	||node == HANSI_ACIPLIST_NODE
	||node == HANSI_WLAN_NODE			/* WLAN node */
	||node == HANSI_WTP_NODE
	||node == HANSI_RADIO_NODE
	||node == HANSI_STA_NODE
	||node == HANSI_WQOS_NODE		/*wireless qos node.*/
	||node == HANSI_EBR_NODE
	||node == HANSI_CWTUNNEL_NODE
	||node == HANSI_AP_GROUP_NODE	
	||node == HANSI_AP_GROUP_WTP_NODE
	||node == HANSI_AP_GROUP_RADIO_NODE
	||node == HANSI_POOL_NODE
	||node == HANSI_POOLV6_NODE
	||node == HANSI_AC_GROUP_NODE			/*wireless ac_group_node*/
	||node == SNMP_VIEW_NODE	
	||node == ROUTE_POLICY_NODE
	||node == ACSAMPLE_NODE
	||node == SNMP_NODE
	||node == POOL_NODE
	||node == POOLV6_NODE
	||node == SUBNET6_CONF_NODE
	||node == DISTRIBUTED_DYNAMIC_TRUNK_NODE
	||node == DYNAMIC_TRUNK_NODE
	||node == SYSLOG_NODE
	||node == NTP_NODE
	||node == TIME_ZONE_NODE
	||node == ACINFO_NODE
	...
	||node == EAG_NODE
	||node == HANSI_EAG_NODE
	||node == LOCAL_HANSI_EAG_NODE
	||node == PDC_NODE
	||node == HANSI_PDC_NODE
	||node == LOCAL_HANSI_PDC_NODE
	||node == RDC_NODE
	||node == HANSI_RDC_NODE
	||node == LOCAL_HANSI_RDC_NODE
	||node == EAG_RADIUS_NODE
	||node == EAG_PORTAL_NODE
	||node == EAG_NASID_NODE
	||node == EAG_VLANMAP_NODE
  	||node == EAG_INS_NODE
  	||node == SPACIAL_CONFIG_NODE
  	||node == WEBSERVICE_NODE
  	||node == RADIUS_NODE
  	||node == IU_NODE
  	||node == HANSI_IU_NODE
  	||node == LOCAL_HANSI_IU_NODE  	
  	||node == IUH_NODE
  	||node == HANSI_IUH_NODE
  	||node == LOCAL_HANSI_IUH_NODE
  	||node == SIGTRANUDP_NODE
	||node == CBAT_NODE
	||node == CNU_NODE
	||node == HANSI_CBAT_NODE
	||node == HANSI_CNU_NODE
	||node == CBAT_RADIO_NODE
	||node == HANSI_CBAT_RADIO_NODE	
	||node == LOCAL_HANSI_SECURITY_NODE
	||node == LOCAL_HANSI_ACIPLIST_NODE
	||node == LOCAL_HANSI_WLAN_NODE			/* WLAN node */
	||node == LOCAL_HANSI_WTP_NODE
	||node == LOCAL_HANSI_RADIO_NODE
	||node == LOCAL_HANSI_STA_NODE
	||node == LOCAL_HANSI_WQOS_NODE		/*wireless qos node.*/
	||node == LOCAL_HANSI_EBR_NODE
	||node == LOCAL_HANSI_CWTUNNEL_NODE
	||node == LOCAL_HANSI_AP_GROUP_NODE	
	||node == LOCAL_HANSI_AP_GROUP_WTP_NODE
	||node == LOCAL_HANSI_AP_GROUP_RADIO_NODE
  ...
  {  
  install_element (node, &vtysh_end_all_cmd);
  }
else
{
  ...
  if(node == ENABLE_NODE || node == ZEBRA_NODE || node == RIP_NODE || node == OSPF_NODE )
  {
  	install_element (node, &config_write_terminal_cmd);
  	install_element (node, &config_write_terminal_cmd1);
...
//  host.motdfile = default_motdfile;
  ...
  /* Install top nodes. */
  install_node (&view_node, NULL, "VIEW_NODE");
  install_node (&enable_node, NULL, "ENABLE_NODE");
  install_node (&auth_node, NULL, "AUTH_NODE");
  install_node (&auth_enable_node, NULL, "AUTH_ENABLE_NODE");
  if (terminal)
    {
			install_node (&config_node, config_write_host, "CONFIG_NODE");
	  ...
	  install_element (VIEW_NODE, &config_write_terminal_cmd1);
//      install_element (VIEW_NODE, &show_logging_cmd);
  ...
  if (terminal)
	...
	//del 11/25/2010 by dingkang beacuse multi-bootconfig file
