/*
 * $Id: log.c,v 1.10 2011/05/19 06:55:28 houxiaoxian Exp $
 *
 * Logging of zebra
		     ...
		     unsigned int line, const char *function)
{
  ...
  zlog(NULL, LOG_CRIT, "Assertion `%s'  line %u, function %s",
       assertion,line,(function ? function : "?"));
	  ...
	  int syslog_flags, int syslog_facility)
{
  ...
  for (i = 0; i < sizeof(zl->maxlvl)/sizeof(zl->maxlvl[0]); i++)
    zl->maxlvl[i] = LOG_DEBUG;
	
	zlog_set_level (zl, ZLOG_DEST_SYSLOG, LOG_DEBUG);
#else
/*Set new zlog default level to LOG_INFO. */
  for (i = 0; i < sizeof(zl->maxlvl)/sizeof(zl->maxlvl[0]); i++)
    zl->maxlvl[i] = LOG_DEBUG;
...
static const struct zebra_desc_table command_types[] = {
  ...
  DESC_ENTRY	(ZEBRA_INTERFACE_LINKDETECTION_ENABLE),
  DESC_ENTRY	(ZEBRA_INTERFACE_LINKDETECTION_DISABLE),
  DESC_ENTRY    (ZEBRA_INTERFACE_PACKETS_REQUEST),
  DESC_ENTRY	(ZEBRA_INTERFACE_PACKETS_REQUEST_ALL),
  DESC_ENTRY 	(ZEBRA_INTERFACE_PACKETS_STATISTICS),
  DESC_ENTRY 	(ZEBRA_INTERFACE_INFOMATION_REQUEST),
  DESC_ENTRY 	(ZEBRA_ROUTE_INFOMATION_REQUEST),
  DESC_ENTRY	(ZEBRA_INTERFACE_DESCRIPTION_SET),
  DESC_ENTRY	(ZEBRA_INTERFACE_DESCRIPTION_UNSET),
  DESC_ENTRY	(ZEBRA_INTERFACE_UPLINK_FLAG_SET),
  DESC_ENTRY	(ZEBRA_INTERFACE_UPLINK_FLAG_UNSET),
  DESC_ENTRY	(INTERFACE_FLOW_STATISTICS_SAMPLING_INTEGRATED_SNMP),  
  DESC_ENTRY	(INTERFACE_FLOW_STATISTICS_SAMPLING_DIVIDED_SNMP),  
  DESC_ENTRY	(INTERFACE_FLOW_STATISTICS_SAMPLING_INTEGRATED_ACSAMPLE),
  DESC_ENTRY	(INTERFACE_FLOW_STATISTICS_SAMPLING_DIVIDED_ACSAMPLE),  
  DESC_ENTRY	(INTERFACE_FLOW_STATISTICS_SAMPLING_INTEGRATED_RTM),  
  DESC_ENTRY	(INTERFACE_FLOW_STATISTICS_DATA),  
  DESC_ENTRY	(ZEBRA_INTERFACE_ND_SUPPRESS_RA_ENABLE),
  DESC_ENTRY	(ZEBRA_INTERFACE_ND_SUPPRESS_RA_DISABLE),
  DESC_ENTRY	(ZEBRA_INTERFACE_RTADV_ND_INFO_UPDATE),
  DESC_ENTRY	(ZEBRA_INTERFACE_ND_PREFIX_ADD),
  DESC_ENTRY	(ZEBRA_INTERFACE_ND_PREFIX_DELETE),
...
zroute_lookup(u_int zroute)
{
  ...
  if (zroute >= sizeof(route_types)/sizeof(route_types[0]))
    {
      zlog_err("unknown rtm route type: %u", zroute);
      return &unknown;
    }
