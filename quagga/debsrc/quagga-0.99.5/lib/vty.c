...
#else
char integrate_default_bak[] = SYSCONFDIR INTEGRATE_DEFAULT_CONFIG;
char *integrate_default = NULL;
char *default_admin_user = NULL;
...
vtysh_read (struct thread *thread)
{
	  ...
	  if (ERRNO_IO_RETRY(errno))
	    {
		  ...
		  zlog_warn("%s ERRNO_IO_RETRY(errno): read failed on vtysh client fd %d, closing: %s",
  ...
  printf ("line: %.*s\n", nbytes, buf);
#endif /* VTYSH_DEBUG */
  zlog_debug("In func %s command %s\n",__func__,buf);
...
void read_config_restart(char* config_file)
{
	FILE *confp = NULL;
	char *fullpath;
	...
	if (confp == NULL)
	{
	  zlog_err( "%s: failed to open configuration file %s: %s\n",
			   __func__, config_file, safe_strerror (errno));
	  
	  confp = vty_use_backup_config (config_file);
	  if (confp)
		{
		...
		{
		  zlog_warn ("can't open configuration file [%s.bak]\n",
					 config_file);
			return ;
		}
	}	   
                 ...
                 char *config_default_dir)
{
	if(!config_file)
		return;
	return read_config_restart(config_file);
}
...
#if 0  
  install_node (&vty_node, vty_config_write, "VTY_NODE");
#else
