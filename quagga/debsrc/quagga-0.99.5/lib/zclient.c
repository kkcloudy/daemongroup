...
void zclient_event (enum event, struct zclient *);
...
//  if(zclient->redist_default != ZEBRA_ROUTE_SYNC)
  	return zclient_start (zclient);
//  else
//  	return tipc_zclient_start(zclient);
}

 /* 
...
void zclient_master(struct thread_master *thread_master)
{
	master = thread_master;
}
void
zclient_event (enum event event, struct zclient *zclient)
{
  switch (event)
    ...
    case ZCLIENT_SCHEDULE:
	  #ifdef ZCLIENT_DEBUG		
		zlog_debug("%s: line %d, zclient event = ZCLIENT_SCHEDULE\n",__func__,__LINE__);
	  #endif
      if (! zclient->t_connect)
			zclient->t_connect =
...
//	  else
//		  zclient->t_read = 
//		  thread_add_read (master, tipc_zclient_read, zclient, zclient->sock);
      break;
