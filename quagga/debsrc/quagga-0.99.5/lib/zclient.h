...
/*gujd: In order to decrease the warning when make img . For struct prefix_ipv4 .*/
#include "prefix.h"
/*gujd : For struct thread_master .*/
#include "thread.h"
...
struct zclient
{
  ...
  /**add by gjd**/
  int (*redistribute_route_all)(int, struct zclient *, uint16_t);
};
...
extern int zapi_ipv4_route (u_char, struct zclient *, struct prefix_ipv4 *, 
                            struct zapi_ipv4 *);
extern 	void zclient_master(struct thread_master *thread_master);

/**add by gjd**/
extern int tipc_zclient_read(struct thread * thread);\

#if 0
#ifndef ZCLIENT_DEBUG
#define ZCLIENT_DEBUG
#endif
#endif
