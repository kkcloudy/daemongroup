...
prefix_same (const struct prefix *p1, const struct prefix *p2)
{
  ...
  if(!p1 || !p2)
  {
  	zlog_warn("%s : line %d , pointer p1(%p),p2(%p).\n",__func__,__LINE__,p1,p2);
	return 0;
  	}
  #endif
...
/*added by gxd*/
int
prefix_verify_ipv4(int flag,const struct prefix_ipv4 *prefix)
{    
	int i=0;
	struct prefix_ipv4 tmpprefix;
	...
	if(prefix)
		while(flag){
			switch(flag&(1<<i)){
				case PREFIXHOST: {
					tmpprefix.family=prefix->family;
					tmpprefix.prefixlen=prefix->prefixlen;
					tmpprefix.prefix.s_addr=prefix->prefix.s_addr;
					tmpaddr.s_addr=ipv4_network_addr((tmpprefix.prefix.s_addr),tmpprefix.prefixlen);
					if(prefix->prefixlen!=32&&0==memcmp(&(tmpaddr.s_addr),&(prefix->prefix.s_addr),IPV4_MAX_BYTELEN))
						return -2;
					if(IN_CLASSD(prefix->prefix.s_addr))
						return -2;
					tmpaddr.s_addr=ipv4_broadcast_addr((tmpprefix.prefix.s_addr),tmpprefix.prefixlen);
					if(prefix->prefixlen!=32&&0==memcmp(&(tmpaddr.s_addr),&(prefix->prefix.s_addr),IPV4_MAX_BYTELEN))
						return -2;
					break;
				}
				case PREFIXCLASSA2C : {
					if(IN_CLASSA(prefix->prefix.s_addr))
   						break;
 				      else if(IN_CLASSB(prefix->prefix.s_addr))
   						break;
  				      else if(IN_CLASSC(prefix->prefix.s_addr))
   						break;
                               else
      						return -3;
				}
				case PREFIXNORMAL : {
					prefix2str((struct prefix *) prefix, prefixstr, BUFSIZ);
					if(!strcmp(prefixstr,"0.0.0.0")||!strcmp(prefixstr,"127.0.0.1"))
						return -4;
					else break;
				}
				case 0:
