...
router_id_find_node (struct list *l, struct connected *ifc)
{
  ...
  struct connected *c;
#if 0
  if(!l || !ifc || !ifc->address)
  {
  	zlog_warn("%s : line %d ,the pointer l(%p),ifc(%p),ifc->address(%p) !\n",__func__,__LINE__,l,ifc,ifc->address);
	return NULL;
  	}
  #endif
...
router_id_bad_address (struct connected *ifc)
{
  ...
  if (!ifc || !ifc->address)
  	{
	  zlog_warn("%s : line %d ,pointer ifc(%p),ifc->address(%p)!\n",__func__,__LINE__,ifc,ifc->address);
    	return 1;
  	}
 #endif 
...
router_id_add_address (struct connected *ifc)
{
  ...
  if(!l)
  {
  	zlog_warn("%s : line %d ,pointer l(%p) !\n",__func__,__LINE__,l);
	return;
  	}
  #endif
