          ...
          switch(nexthop->type) 
            {
              ...
              case NEXTHOP_TYPE_IPV4_IFINDEX:
			  	...
			  	If it is stream put gate way 0.0.0.0, not real gate way ip.*/
				ifname = ifindex_to_ifname(nexthop->ifindex);/*ifindex to ifname*/
              ...
              case NEXTHOP_TYPE_IPV6_IFNAME:
				  /*gujd: 2013-02-26, pm 4:08. Check nexthop is obc interface.
				  If it is stream put gate way 0.0.0.0, not real gate way ip.*/
				  if(judge_obc_interface(nexthop->ifname)==OBC_INTERFACE)
				  {
					 struct in6_addr empty;
                     memset (&empty, 0, sizeof (struct in6_addr));
                     stream_write (s, (u_char *) &empty, IPV6_MAX_BYTELEN);
  ...
  if (accept_sock < 0) 
    {
      zlog_warn ("Can't create zserv stream socket: %s", 
                 safe_strerror (errno));
      zlog_warn ("Rtm can't provice full functionality due to above error");
      return;
    }
...
zebra_event (enum event event, int sock, struct zserv *client)
{
  switch (event)
    {
    case ZEBRA_SERV:
      thread_add_read (zebrad.master, zebra_accept, client, sock);/*accept the client*/
      break;
    case ZEBRA_READ:
      client->t_read = 
	thread_add_read (zebrad.master, zebra_client_read, client, sock);/*server read info from client*/
      break;
    case ZEBRA_WRITE:
      /**/
      break;
...
zebra_init (void)
{
  ...
  install_node (&table_node, config_write_table, "TABLE_NODE");
  install_node (&forwarding_node, config_write_forwarding, "FORWARDING_NODE");
#else
install_node (&table_node, NULL, "TABLE_NODE");
install_node (&forwarding_node, NULL, "FORWARDING_NODE");
...
/*
 	install_element (HIDDENDEBUG_NODE, &show_table_cmd);
  	install_element (HIDDENDEBUG_NODE, &config_table_cmd);
*/
