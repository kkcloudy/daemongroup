...
ospf_apiserver_new_if (struct interface *ifp)
{
  ...
  if(IS_DEBUG_OSPF_EVENT)
	{
		zlog_debug ("ospf_apiserver_new_if");
		zlog_debug ("ifp name=%s status=%d index=%d", ifp->name, ifp->status,
		 ifp->ifindex);
	}
  ...
  if (!oi) {
	...
	if(IS_DEBUG_OSPF_EVENT)
    zlog_debug ("ospf_apiserver_new_if: interface %s not known to OSPFd?", 
	       ifp->name);
    return 0;
  }
...
ospf_apiserver_del_if (struct interface *ifp)
{
  ...
  if(IS_DEBUG_OSPF_EVENT)
	{ 
	zlog_debug ("ospf_apiserver_del_if");
  	zlog_debug ("ifp name=%s status=%d index=%d\n", ifp->name, ifp->status,
	     ifp->ifindex);
  	}
  oi = ospf_apiserver_if_lookup_by_ifp (ifp);
...
ospf_apiserver_ism_change (struct ospf_interface *oi, int old_state)
{
  ...
  /* zlog_warn for debugging */
  if(IS_DEBUG_OSPF_EVENT)
	  zlog_debug ("ospf_apiserver_ism_change");
  if (listcount (apiserver_list) > 0) {
    ospf_apiserver_clients_notify_ism_change (oi);
  }
  if(IS_DEBUG_OSPF_EVENT)
  	{
	  zlog_debug ("oi->ifp->name=%s", oi->ifp->name);
	  zlog_debug ("old_state=%d", old_state);
	  zlog_debug ("oi->state=%d", oi->state);
  	}
}
...
ospf_apiserver_nsm_change (struct ospf_neighbor *nbr, int old_status)
{
  ...
  if(IS_DEBUG_OSPF_EVENT)
 	 zlog_debug ("ospf_apiserver_nsm_change");
  if (listcount (apiserver_list) > 0) {
