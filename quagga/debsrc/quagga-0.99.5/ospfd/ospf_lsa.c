...
ospf_lsa_free (struct ospf_lsa *lsa)
{
  ...
  if (IS_DEBUG_OSPF (lsa, LSA)|| IS_DEBUG_OSPF (lsa, LSA_REFRESH))
...
u_char
ospf_lsa_option_get ( struct ospf_area *area, u_char type )
{
		u_char options = 0;

		if ( ( type == OSPF_AS_EXTERNAL_LSA )
						|| ( area && area->area_id.s_addr == OSPF_AREA_BACKBONE ) )
			 ...
			 struct ospf_lsa *lsa, struct external_info *ei)
{
  ...
  for (ALL_LIST_ELEMENTS (ospf->areas, node, nnode, area))
	...
	//  UNSET_FLAG(new->data->options, OSPF_OPTION_E);
...
ospf_lsa_maxage (struct ospf *ospf, struct ospf_lsa *lsa)
{
  ...
  if(ospf==NULL || lsa==NULL)
  	{
		if (IS_DEBUG_OSPF (lsa, LSA_FLOODING))
	  		zlog_debug ("In function %s paraments is NULL" ,__func__);
		return;
  ...
  if (lsa->refresh_list >= 0)
    {
	  ...
	  if (IS_DEBUG_OSPF (lsa, LSA_REFRESH))
		zlog_debug ("LSA[Refresh:%s]: ospf_refresher_unregister_lsa(): "
			   "refresh lsa %p (slot %d)", 
			   inet_ntoa (lsa->data->id), lsa, lsa->refresh_list);
