...
ALIAS (debug_ospf_zebra,
       ...
       "debug ospf rtm (interface|redistribute)",
       DEBUG_STR
       OSPF_STR
       "OSPF Rtm information\n"
       "Rtm interface\n"
...
ALIAS (no_debug_ospf_zebra,
       ...
       "no debug ospf rtm (interface|redistribute)",
       NO_STR
       DEBUG_STR
       OSPF_STR
       "OSPF Rtm information\n"
       ...
       OSPF_STR)
{
  ...
  if (IS_DEBUG_OSPF (zebra, ZEBRA) == OSPF_DEBUG_ZEBRA)
    vty_out (vty, "  OSPF Rtm debugging is on%s", VTY_NEWLINE);
  else
    {
      if (IS_DEBUG_OSPF (zebra, ZEBRA_INTERFACE))
	vty_out (vty, "  OSPF Rtm interface debugging is on%s", VTY_NEWLINE);
      if (IS_DEBUG_OSPF (zebra, ZEBRA_REDISTRIBUTE))
	vty_out (vty, "  OSPF Rtm redistribute debugging is on%s", VTY_NEWLINE);
    }
...
config_write_debug (struct vty *vty)
{
  ...
  if (IS_CONF_DEBUG_OSPF (zebra, ZEBRA) == OSPF_DEBUG_ZEBRA)
    vty_out (vty, "debug ospf rtm%s", VTY_NEWLINE);
  else
    {
      if (IS_CONF_DEBUG_OSPF (zebra, ZEBRA_INTERFACE))
	vty_out (vty, "debug ospf rtm interface%s", VTY_NEWLINE);
      if (IS_CONF_DEBUG_OSPF (zebra, ZEBRA_REDISTRIBUTE))
	vty_out (vty, "debug ospf rtm redistribute%s", VTY_NEWLINE);
...
#else
install_node (&debug_node, NULL, "DEBUG_NODE", "INTERFACE_NODE");
