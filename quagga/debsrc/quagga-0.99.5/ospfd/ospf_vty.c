       ...
       "router-id for the OSPF process\n")
{
  ...
  ospf_router_id_update (ospf);
  vty_out(vty,"This setting will not active before ospf restart! since changing router ID's is very disruptive\n");
  return CMD_SUCCESS;
}
       ...
       "Interface's address\n")
{
 ...
 if(!ifp)
 {
	vty_out(vty,"The interface %s is not exist\n",argv[0]);
	return CMD_WARNING;
 }
       ...
       "Interface's name\n")
{
  ...
  if (argc == 2)
      ...
      if (!ret)
	{
      ...
      if (params == NULL){
		  vty_out (vty, "Specify interface address %s doesn't exist %s",
		  		argv[1],VTY_NEWLINE);
       ...
       "Area range prefix\n")
{
  ...
  if (argc > 2)
    {
    	/* CID 11838 (#1 of 1): Macro compares unsigned to 0, no problem */
...
ospf_vl_set_timers (struct ospf_vl_data *vl_data,
		    struct ospf_vl_config_data *vl_config)
{
	/*CID 10850 (#1 of 1): Evaluation order violation*/
       ...
       VLINK_HELPSTR_IPADDR)
{
  ...
  for (i=2; i < argc; i++)
	...
	case 'm':
	  ...
	  {	  	
		vty_out (vty, "AUTH_KEY length should be <=16%s",
				 VTY_NEWLINE);
		return CMD_WARNING;
	  }
       ...
       "Configure OSPF area as stub\n")
{
  ...
  if(ospf->areas->count == 1 && IS_OSPF_ASBR(ospf))
  {
		vty_out(vty,"Warning: Stub command is invalid when the router is only ASBR.%s",VTY_NEWLINE);
		return CMD_SUCCESS;
  }
       ...
       "Stub's advertised default summary cost\n")
{
  ...
  VTY_GET_OSPF_AREA_ID_NO_BB ("default-cost", area_id, format, argv[0]);
	/* CID 11837 (#1 of 1): Macro compares unsigned to 0, no problem*/
       ...
       "Stub's advertised default summary cost\n")
{
  ...
  VTY_GET_OSPF_AREA_ID_NO_BB ("default-cost", area_id, format, argv[0]);
	/* CID 11836 (#1 of 1): Macro compares unsigned to 0, no problem */
  VTY_GET_INTEGER_RANGE ("stub default cost", cost, argv[1], 0, 16777215);
       ...
       "Filter networks sent from this area\n")
{
  ...
  if ((area = ospf_area_lookup_by_area_id (ospf, area_id)) == NULL)
	...
	/* CID 14555 (#1 of 1): Unused pointer value, fix here and line of 1925*/
  //plist = prefix_list_lookup (AFI_IP, argv[1]);
	/* CID 14555 end */
  if (strncmp (argv[2], "in", 2) == 0)
    {
       ...
       "Maximum hold time (msec)\n")
{
  ...
  /* CID 11844 (#1 of 1): Macro compares unsigned to 0  , no problem*/
       ...
       "Hold time (s) between consecutive SPF calculations\n")
{
  ...
  if (argc != 2)
	...
	/* CID 11843 (#1 of 1): Macro compares unsigned to 0, no problem */  
       ...
       "Neighbor IP address\n")
{
  ...
  if (argc > 1)
		/* CID 11841 (#1 of 1): Macro compares unsigned to 0, no problem */
    VTY_GET_INTEGER_RANGE ("neighbor priority", priority, argv[1], 0, 255);

  if (argc > 2)
    VTY_GET_INTEGER_RANGE ("poll interval", interval, argv[2], 1, 65535);
  
  if(-1==ospf_nbr_nbma_set (ospf, nbr_addr))
  	vty_out(vty,"%% Systerm warnning:added success but don't fit any nbma interface.%s",VTY_NEWLINE);
  if (argc > 1)
       ...
       "Seconds\n")
{
  ...
  if (argc > 2)
		/* CID 11842 (#1 of 1): Macro compares unsigned to 0, no problem */
    VTY_GET_INTEGER_RANGE ("neighbor priority", priority, argv[2], 0, 255);

  if(-1==ospf_nbr_nbma_set (ospf, nbr_addr))
  	vty_out(vty,"Add success but don't fit any nbma interface.%s",VTY_NEWLINE);
  if (argc > 1)
...
show_ip_ospf_area (struct vty *vty, struct ospf_area *area)
{
  ...
  /* Show Area ID. */
  vty_out (vty, "   Area ID: %s%s", inet_ntoa (area->area_id),VTY_NEWLINE);

  /* Show Area type/mode. */
  if (OSPF_IS_AREA_BACKBONE (area))
    vty_out (vty, "   Area type: Backbone%s", VTY_NEWLINE);
  else
    {
      if (area->external_routing == OSPF_AREA_STUB)
        vty_out (vty, "   Area type: stub%s"
        			   "   Statu:%s%s",
        		  VTY_NEWLINE,
		         area->no_summary ? ", no summary" : "summary",
		         VTY_NEWLINE);
      else if (area->external_routing == OSPF_AREA_NSSA)
        vty_out (vty, "   Area type:NSSA%s"
        			   "   Statu:%s%s",
        			   VTY_NEWLINE,
                 area->no_summary ? ", no summary" : "summary",
                 		  VTY_NEWLINE);
      vty_out (vty, "   Shortcutting mode: %s%s",
               ospf_shortcut_mode_descr_str[area->shortcut_configured],VTY_NEWLINE);
      vty_out (vty, "   S-bit consensus: %s%s",
               area->shortcut_capability ? "ok" : "no", VTY_NEWLINE);
    }
  /***added by gxd**********/
  /*Show default cost*/
  if(CHECK_FLAG ((area->ospf)->flags, OSPF_FLAG_ABR)&&area->external_routing == OSPF_AREA_STUB)
 	 vty_out (vty, "  default cost:%u%s ",area->default_cost,VTY_NEWLINE);
  /*show default metric*/
  vty_out(vty,"   default metric:%d%s",(area->ospf)->default_metric,VTY_NEWLINE);
 /*show fliter-list*/
  if(NULL!=PREFIX_NAME_OUT(area))
  	vty_out(vty,"  In fliter-list:%s%s",PREFIX_NAME_OUT(area),VTY_NEWLINE);
  if(NULL!=PREFIX_NAME_OUT(area))
  	vty_out(vty,"  Out fliter-list:%s%s",PREFIX_NAME_OUT(area),VTY_NEWLINE);
  /*show range*/
  if(CHECK_FLAG ((area->ospf)->flags, OSPF_FLAG_ABR))
	 for (rn = route_top (area->ranges); rn; rn = route_next (rn)){
		struct ospf_area_range *myrange;
		if(NULL!=(myrange=rn->info)){
			struct prefix myprefix;
			char buffer[BUFSIZ];
			myprefix.prefixlen=myrange->masklen;
			myprefix.u.prefix4=myrange->addr;
			myprefix.family=AF_INET;
			if(0!=prefix2str(&myprefix,buffer,BUFSIZ)){
			/*	return CMD_WARNING;*/
			/*gujd: 2012-02-09: am 10:00 . In order to decrease the warning when make img . 
			Change "return CMD_WARNING" to "return ", becaus this func return void*/
			return ;
			}	
			vty_out(vty,"   Range prefix:%s%s   Range type:%s%s   Range cost:%u%s",
				buffer,VTY_NEWLINE,
					(myrange->flags==OSPF_AREA_RANGE_ADVERTISE)?"advertise":"non-advertise",VTY_NEWLINE,
			      			myrange->cost,VTY_NEWLINE);
		}
 	 }
/*show default-information **/
 if(CHECK_FLAG ((area->ospf)->flags, OSPF_FLAG_ASBR)&&(area->ospf)->default_originate!=DEFAULT_ORIGINATE_NONE){
	vty_out(vty,"   Default-information status:%s%s",
			((area->ospf)->default_originate==DEFAULT_ORIGINATE_ALWAYS)?"always":"no-always",
				VTY_NEWLINE);
	if(-1!=(area->ospf)->dmetric[DEFAULT_ROUTE].type)
		vty_out(vty,"   Default-information type:%s%s",
				((area->ospf)->dmetric[DEFAULT_ROUTE].type==1)?"1":"2",
					VTY_NEWLINE);
	if(-1!=(area->ospf)->dmetric[DEFAULT_ROUTE].value)
		vty_out(vty,"   Default-information metric:%d%s",
				(area->ospf)->dmetric[DEFAULT_ROUTE].value,
					VTY_NEWLINE);
	if(ROUTEMAP_NAME (area->ospf, DEFAULT_ROUTE))
		vty_out(vty,"   Default-information route-map name:%s %s",ROUTEMAP_NAME (area->ospf, DEFAULT_ROUTE),
			VTY_NEWLINE);
 }
/***********2008-11-3 9:58:41***********************************/
  /* Show number of interfaces. */
  vty_out (vty, "   Total interface: %d%s "
	   "  Active: %d%s", listcount (area->oiflist),VTY_NEWLINE,
...
/*added by gxd*/
 vty_out (vty, "   Area authentication type: ");
  if (area->auth_type == OSPF_AUTH_NULL)
    vty_out (vty, "no authentication%s", VTY_NEWLINE);
  else if (area->auth_type == OSPF_AUTH_SIMPLE){
  	struct listnode *mylistnode;
  	 struct ospf_interface *oi; 
	struct ospf_if_params *myparams;
    	vty_out (vty, "simple password authentication%s",VTY_NEWLINE);
	vty_out(vty,"   Simple-key(IFname  Key):");
   	for(ALL_LIST_ELEMENTS_RO(area->oiflist,mylistnode,oi)){
		myparams=IF_DEF_PARAMS (oi->ifp);
		/* CID 11845 (#1 of 1): Array compared against 0 */
		//if(myparams->auth_simple)
		if(myparams->auth_simple[0] != '\0')
		/* CID 11845 (#1 of 1): Array compared against 0 */
			vty_out(vty," %s %s,",
					oi->ifp->name,
						myparams->auth_simple);
	}
	vty_out(vty,"%s",VTY_NEWLINE);
  }
  else if (area->auth_type == OSPF_AUTH_CRYPTOGRAPHIC){
  	 struct listnode *mylistnode;
	 struct listnode *cklistnode;
  	 struct ospf_interface *oi;
	 struct crypt_key *myck; 
	 struct ospf_if_params *myparams;
  	 vty_out (vty, "message digest authentication%s", VTY_NEWLINE);
	 for(ALL_LIST_ELEMENTS_RO(area->oiflist,mylistnode,oi)){
	 	myparams=IF_DEF_PARAMS (oi->ifp);
		if(!list_isempty(myparams->auth_crypt)){
			vty_out(vty,"   Smessage-digest-key(IFname KeyId Key):");
			for(ALL_LIST_ELEMENTS_RO(myparams->auth_crypt,cklistnode,myck)){
					vty_out(vty," %s  %d %s,",
							   oi->ifp->name,
								myck->key_id,
									myck->auth_key);
       ...
       "OSPF information\n")
{
  ...
  if (ospf == NULL)
    {
      vty_out (vty, " OSPF Routing Process: disable%s", VTY_NEWLINE);
      return CMD_SUCCESS;
    }
  
  /* Show Router ID. */
  vty_out(vty," OSPF Routing Process:enable.%s",VTY_NEWLINE);
  ...
  /* Show capability. */
  vty_out (vty, " Supports :TOS0 routes%s", VTY_NEWLINE);
  vty_out (vty, " Implementation conforms : RFC2328%s", VTY_NEWLINE);
  vty_out (vty, " RFC1583Compatibility flag : %s%s",
	   CHECK_FLAG (ospf->config, OSPF_RFC1583_COMPATIBLE) ?
	   "enabled" : "disabled", VTY_NEWLINE);
#ifdef HAVE_OPAQUE_LSA
  vty_out (vty, " OpaqueCapability flag : %s%s%s",
  ...
  /********added by gxd**********/
  /*Show auto-cost*/
  vty_out(vty," reference bandwidth :%u%s",ospf->ref_bandwidth,VTY_NEWLINE);
  /*show distance */
  if (ospf->distance_all)
        vty_out (vty, " distance :%d%s", ospf->distance_all, VTY_NEWLINE);
  if (ospf->distance_intra)
	  vty_out (vty, " intra-area :%d%s", ospf->distance_intra,VTY_NEWLINE);
  if (ospf->distance_inter)
	  vty_out (vty, " inter-area :%d%s", ospf->distance_inter,VTY_NEWLINE);
  if (ospf->distance_external)
	  vty_out (vty, " external :%d%s", ospf->distance_external,VTY_NEWLINE);

  /*show distribute-list*/
  for(i=0;i<ZEBRA_ROUTE_MAX;i++){
	if((ospf->dlist[i]).name){
		char mystr[20];
		switch(i){
			case ZEBRA_ROUTE_KERNEL:strcpy(mystr,"kernel");break;
			case ZEBRA_ROUTE_CONNECT:strcpy(mystr,"connected");break;
			case ZEBRA_ROUTE_STATIC:strcpy(mystr,"static");break;
			case ZEBRA_ROUTE_RIP:strcpy(mystr,"rip");break;
			case ZEBRA_ROUTE_BGP:strcpy(mystr,"bgp");break;
			case ZEBRA_ROUTE_ISIS:strcpy(mystr,"isis");break;
		}
		vty_out(vty," %s distribute-list:%s%s",mystr,
				(ospf->dlist[i]).name,
					VTY_NEWLINE);
	}	
  }
  ...
  for(i=0;i<ZEBRA_ROUTE_MAX;i++){
  	...
  	if(ospf_is_type_redistributed(i)){
		switch(i){
			case ZEBRA_ROUTE_KERNEL: strcpy(myname,"kernel");break;
			case ZEBRA_ROUTE_CONNECT: strcpy(myname,"connected");break;
			case ZEBRA_ROUTE_STATIC: strcpy(myname,"static");break;
			case ZEBRA_ROUTE_RIP: strcpy(myname,"rip");break;
			case ZEBRA_ROUTE_ISIS: strcpy(myname,"isisl");break;
			case ZEBRA_ROUTE_BGP: strcpy(myname,"bgp");break;
			default:
				continue;
		}
		vty_out(vty," Redistribute source: %s",myname);
		if(-1!=ospf->dmetric[i].value)
			vty_out(vty," metric:%d%s",ospf->dmetric[i].value,VTY_NEWLINE);
		else
			vty_out(vty,"%s",VTY_NEWLINE);
		if(-1!=ospf->dmetric[i].type)
			vty_out(vty," meric-type:%d%s",++(ospf->dmetric[i].type),VTY_NEWLINE);
		if((ROUTEMAP_NAME (ospf, i)))
			vty_out(vty," route-map name: %s%s",ROUTEMAP_NAME (ospf, i),VTY_NEWLINE);
  	}
  }
 /*****************2008-11-3 13:51:28********************/
  ...
  if (ospf->stub_router_startup_time != OSPF_STUB_ROUTER_UNCONFIGURED
    ...
    {
      vty_out (vty, " Stub router advertisement : configured%s",
               VTY_NEWLINE);
      if (ospf->stub_router_startup_time != OSPF_STUB_ROUTER_UNCONFIGURED)
  ...
  vty_out (vty, " Initial SPF scheduling delay :%d millisec(s)%s"
                ...
                " Hold time multiplier  currently :%d%s",
  ...
  if (CHECK_FLAG (ospf->flags, OSPF_FLAG_ABR))
    ...
    vty_out (vty, " Router type: ABR%s" " ABR type : %s%s",VTY_NEWLINE,
             ospf_abr_type_descr_str[ospf->abr_type], VTY_NEWLINE);

  if (CHECK_FLAG (ospf->flags, OSPF_FLAG_ASBR))
    vty_out (vty, " Router type: ASBR %s"
             "(injecting external routing information)%s", VTY_NEWLINE,VTY_NEWLINE);
  ...
  if (CHECK_FLAG(ospf->config, OSPF_LOG_ADJACENCY_CHANGES))
	...
	vty_out(vty, " Adjacency changes :All logged%s",VTY_NEWLINE);
      else
	vty_out(vty, " Adjacency changes : logged%s",VTY_NEWLINE);
    }
			    ...
			    struct interface *ifp)
{
  ...
  /*show ifname*/
  vty_out(vty,"%s",VTY_NEWLINE);
  vty_out(vty,"  Ifname:%s%s",ifp->name,VTY_NEWLINE);
   /* Is interface up? */
  vty_out (vty, "  Ifstatus:%s%s",  ((is_up = if_is_operative(ifp)) ? "up" : "down"), VTY_NEWLINE);
  vty_out (vty, "  Ifindex :%u%s  MTU :%u bytes%s  BW :%u Kbit %s  statu: %s%s",
  		   ifp->ifindex,VTY_NEWLINE,
  	 		  ifp->mtu,VTY_NEWLINE,
  	 		  	ifp->bandwidth,VTY_NEWLINE,
  	 		  		if_flag_dump(ifp->flags), VTY_NEWLINE);

  /* Is interface OSPF enabled? */
  if (ospf_oi_count(ifp) == 0)
    {
      vty_out (vty, "  OSPF status:not enabled on this interface%s", VTY_NEWLINE);
      return;
    }
  else if (!is_up)
    {
      vty_out (vty, "  OSPF  statu : enabled, but not running on this interface%s",
	       VTY_NEWLINE);
      return;
    }
  ...
  for (rn = route_top (IF_OIFS (ifp)); rn; rn = route_next (rn))
	...
	if (oi == NULL)
      ...
      vty_out (vty, "  Internet Address :%s/%d%s",
	       inet_ntoa (oi->address->u.prefix4), oi->address->prefixlen,VTY_NEWLINE);
	  ...
	  vty_out (vty, "  %s: %s%s", dstr, inet_ntoa (*dest),VTY_NEWLINE);
        }
      /*added by gxd*/
	/*show if authorfication information*/
	myparams=IF_DEF_PARAMS (ifp);
	if(*(myparams->auth_simple)) 
		vty_out(vty,"  Simple key:%s%s",myparams->auth_simple,VTY_NEWLINE);
	for(ALL_LIST_ELEMENTS_RO(myparams->auth_crypt,mylistnode,myck)){
			vty_out(vty,"  MD5keyid :%d%s" "  MD5key:%s%s",
				myck->key_id,
					VTY_NEWLINE,
						myck->auth_key,
							VTY_NEWLINE);
	 ...
	 /*2008-11-25 12:02:53*/
      ...
      vty_out (vty, "  Router ID :%s%s  Network Type :%s%s  Cost: %d%s",
	       inet_ntoa (ospf->router_id), VTY_NEWLINE ,
	       	ospf_network_type_str[oi->type],VTY_NEWLINE,
	       		oi->output_cost, VTY_NEWLINE);

      vty_out (vty, "  Transmit Delay : %d sec%s  State :%s%s  Priority :%d%s",
	       OSPF_IF_PARAM (oi,transmit_delay),VTY_NEWLINE,
	       	LOOKUP (ospf_ism_state_msg, oi->state),VTY_NEWLINE,
	       		PRIORITY (oi), VTY_NEWLINE);

  /* Show DR information. */
      if (DR (oi).s_addr == 0)
	vty_out (vty, "  Designated Router:have no%s", VTY_NEWLINE);
      else
	{
	  ...
	  if (nbr == NULL)
	    ...
	    {
	      vty_out (vty, "  Designated Router (ID) :%s%s",
		       inet_ntoa (nbr->router_id),VTY_NEWLINE);
      ...
      if (BDR (oi).s_addr == 0)
	vty_out (vty, "  Backup designated router: have no %s",
		 VTY_NEWLINE);
      else
	{
	  ...
	  if (nbr == NULL)
	    ...
	    {
	      vty_out (vty, "  Backup Designated Router (ID): %s%s",
		       inet_ntoa (nbr->router_id),VTY_NEWLINE);
	      vty_out (vty, "  Interface Address :%s%s",
		       inet_ntoa (nbr->address.u.prefix4), VTY_NEWLINE);
	    }
      ...
      if (OSPF_IF_PARAM (oi, fast_hello) == 0)
        vty_out (vty, ":%ds%s", OSPF_IF_PARAM (oi, v_hello),VTY_NEWLINE);
      else
        vty_out (vty, ":%dms%s", 1000 / OSPF_IF_PARAM (oi, fast_hello),VTY_NEWLINE);
      vty_out (vty, "  Dead :%ds%s  Wait: %ds%s  Retransmit :%d%s",
	       OSPF_IF_PARAM (oi, v_wait),VTY_NEWLINE,
	      		 OSPF_IF_PARAM (oi, v_wait),VTY_NEWLINE,
	     			  OSPF_IF_PARAM (oi, retransmit_interval),  VTY_NEWLINE);
      
      if (OSPF_IF_PARAM (oi, passive_interface) == OSPF_IF_ACTIVE)
        {
	  char timebuf[OSPF_TIME_DUMP_SIZE];
	  vty_out (vty, "  Hello statu :%s%s",
		   ospf_timer_dump (oi->t_hello, timebuf, sizeof(timebuf)), 
		   VTY_NEWLINE);
        }
      else /* OSPF_IF_PASSIVE is set */
	vty_out (vty, "   Hellos status: no (Passive interface)%s", VTY_NEWLINE);
      
      vty_out (vty, "  Neighbor Count: %d%s  Adjacent neighbor count : %d%s",
	       ospf_nbr_count (oi, 0),VTY_NEWLINE,
...
{
  vty_out (vty, "%s%-10s %3s %-10s %9s %-10s %-12s %5s %5s %5s%s",
           VTY_NEWLINE,
	...
	if (nbr->state != NSM_Down)
	  {
	    ...
	    vty_out (vty, "%-10s ", inet_ntoa (nbr->src));
	    vty_out (vty, "%-12s %5ld %5ld %5d%s",
       ...
       "Address of interface")
{
  ...
  if (argc == 2)
    {
      ...
      if (!ret)
	{
	  vty_out (vty, " %System warnning:please specify interface address by A.B.C.D%s",
		   VTY_NEWLINE);
	  return CMD_WARNING;
	}
  ...
  if(strlen(argv[0])>OSPF_AUTH_SIMPLE_SIZE)
  	{
	  vty_out (vty, "The key length should be <= %d! %s",
		   OSPF_AUTH_SIMPLE_SIZE,VTY_NEWLINE);
	  return CMD_WARNING;
       ...
       "Address of interface")
{
  ...
  if(strlen(argv[1])>OSPF_AUTH_MD5_SIZE)
  	{
	  vty_out (vty, "The key length should be <= %d!%s",
		   OSPF_AUTH_MD5_SIZE,VTY_NEWLINE);
	  return CMD_WARNING;
                            ...
                            const char *fast_hello_str)
{
  ...
  if (interval_str)
	...
	if(OSPF_IF_PARAM_CONFIGURED(params,v_hello)&&seconds<=params->v_hello)
	{
		vty_out (vty, "Dead-interval should be more than hello-interval%s",
				VTY_NEWLINE);
		return CMD_WARNING;
       ...
       "Address of interface")
{
  ...
  /*if hellointerval is between 1 and 16383 set deadinterval 4*hellointerval,else 65535*/

  sprintf(ditmp,"%u",(unsigned int)4*seconds);
  if(seconds>1&&seconds<16383){
  	if(2==argc)
  		ospf_vty_dead_interval_set (vty, ditmp, argv[1], NULL);
	else 
		ospf_vty_dead_interval_set (vty, ditmp, NULL, NULL);
  }
  else {
  	if(2==argc)
  		ospf_vty_dead_interval_set (vty, "65535", argv[1], NULL);
	else
		ospf_vty_dead_interval_set (vty, "65535", NULL, NULL);
  }
  return CMD_SUCCESS;
}
...
#endif
  old_type = IF_DEF_PARAMS (ifp)->type;
...
#define CHECK_STUB_AREA \	
if(ospf->areas->count == 1 && ((struct ospf_area * )(ospf->areas->head->data))->external_routing == OSPF_AREA_STUB)\
  {\
	  vty_out(vty,"Warning: router is currently an ASBR while having only one area which is a stub area.\n");\
	  return CMD_SUCCESS;\
}
       ...
       "Time (seconds) to advertise self as stub-router\n")
{
  ...
  /* CID 11840 (#1 of 1): Macro compares unsigned to 0, no problem */
       ...
       "Time (seconds) to wait till full shutdown\n")
{
  ...
  if (argc != 1)
	...
	/*CID 11839 (#1 of 1): Macro compares unsigned to 0, no problem*/
...
/******added by gxd***/
DEFUN (show_ip_ospf_virtual_link,
		 show_ip_ospf_virtual_link_cmd,
		...
		"virtual-link information\n")
{
	...
	struct interface *ifp;
	 if ((ospf = ospf_lookup ()) == NULL){
     		 vty_out (vty, " OSPF Routing Process : enabled%s", VTY_NEWLINE);
     		 return CMD_SUCCESS;
    	 }
	 for (ALL_LIST_ELEMENTS_RO (iflist, listnode, ifp)){
	 	if(NULL==listnode->data)
			continue;
		else{
	   		if(!memcmp("VLINK",ifp->name,5))
	   			show_ip_ospf_interface_sub (vty, ospf, ifp);
		}
	 }
	 return CMD_SUCCESS;
...
ospf_vty_show_init (void)
{
  ...
  install_element (ENABLE_NODE, &show_ip_ospf_virtual_link_cmd);
  /****************************************************/
}
...
ospf_vty_if_init (void)
{
  ...
  install_node (&interface_node, config_write_interface, "INTERFACE_NODE");
...
ospf_vty_init (void)
{
  ...
  install_node (&ospf_node, ospf_config_write, "OSPF_NODE");
