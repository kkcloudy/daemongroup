		    ...
		    struct in_addr *addr, int exact)
{
  ...
  if (exact)
	...
	return NULL;
      ...
      area = ospf_area_lookup_by_area_id (ospf, *addr);
			/* CID 11926 (#1 of 1): Dereference null return value */
			if(!area)
				return NULL;
			/* CID 11926 end */
      if (area->external_routing == OSPF_AREA_STUB)
...
#define OSPF_LSDB_ENTRY_OFFSET \
  ...
  if (exact)
	...
	return NULL;
      ...
      if (! area)
	return NULL;
			/* CID 13950 (#1 of 6): Extra sizeof expression , no problem */
      offset += IN_ADDR_SIZE;
      ...
      *type = *offset;
      ...
      oid2in_addr (offset, IN_ADDR_SIZE, ls_id);
			/* CID 13950 (#2 of 6): Extra sizeof expression , no problem */
      offset += IN_ADDR_SIZE;
	...
	{
	  /* Next we lookup type. */
		/* CID 13950 (#3 of 6): Extra sizeof expression , no problem */
	  offset += IN_ADDR_SIZE;
	  ...
	  /* Router ID. */
		/* CID 13950 (#4 of 6): Extra sizeof expression , no problem */
	  offset += IN_ADDR_SIZE;
	  offsetlen -= IN_ADDR_SIZE;
	  len = offsetlen;
	  ...
	  if (lsa)
	    {
	      ...
	      *length = v->namelen + OSPF_LSDB_ENTRY_OFFSET;
	      ...
	      oid_copy_addr (offset, area_id, IN_ADDR_SIZE);
				/* CID 13950 (#5 of 6): Extra sizeof expression , no problem */
	      offset += IN_ADDR_SIZE;
	      *offset = lsa->data->type;
	      offset++;
	      oid_copy_addr (offset, &lsa->data->id, IN_ADDR_SIZE);
				/* CID 13950 (#6 of 6): Extra sizeof expression , no problem */
	      offset += IN_ADDR_SIZE;
		     ...
		     int exact)
{
  ...
  if (exact) 
	...
	return NULL;
			...
			/*CID 13946 (#1 of 3): Extra sizeof expression	, no problem*/
      offset += IN_ADDR_SIZE;
	...
	return NULL;
      ...
      do 
	{
		/*CID 13946 (#2 of 3): Extra sizeof expression  , no problem*/
	  offset += IN_ADDR_SIZE;
	  offsetlen -= IN_ADDR_SIZE;
	  len = offsetlen;

		/* CID 11834 (#1 of 1): Unsigned compared against 0, no problem */
	  if (len < 0)
	    len = 0;
	  if (len > IN_ADDR_SIZE)
	    len = IN_ADDR_SIZE;
	  ...
	  if (range)
	    {
	      ...
	      *length = v->namelen + IN_ADDR_SIZE + IN_ADDR_SIZE;
	      ...
	      oid_copy_addr (offset, area_id, IN_ADDR_SIZE);
				...
				/*CID 13946 (#3 of 3): Extra sizeof expression  , no problem*/
	      offset += IN_ADDR_SIZE;
      ...
      if (oi)
	{
	  *length = v->namelen + IN_ADDR_SIZE + 1;
	  ...
	  oid_copy_addr (offset, ifaddr, IN_ADDR_SIZE);
		/*CID 13948 (#1 of 1): Extra sizeof expression, no problem*/
	  offset += IN_ADDR_SIZE;
	  *offset = *ifindex;
	  return oi;
	}
	...
	*ifindex = name[v->namelen + IN_ADDR_SIZE];
      ...
      if (oi)
	{
	  *length = v->namelen + IN_ADDR_SIZE + 1 + 1;
	  offset = name + v->namelen;
	  oid_copy_addr (offset, ifaddr, IN_ADDR_SIZE);
		/* CID 13949 (#1 of 1): Extra sizeof expression , no problem*/
	  offset += IN_ADDR_SIZE;
	  *offset = *ifindex;
	  offset++;
  ...
  if (exact)
	...
	return NULL;
      ...
      oid2in_addr (name + v->namelen, IN_ADDR_SIZE, area_id);
			/* CID 13951 (#1 of 3): Extra sizeof expression, no problem */
	...
	len = IN_ADDR_SIZE;
      ...
      if (len > IN_ADDR_SIZE)
	len = IN_ADDR_SIZE;
			/* CID 13951 (#2 of 3): Extra sizeof expression, no problem */
	...
	{
	  *length = v->namelen + IN_ADDR_SIZE + IN_ADDR_SIZE;
		/* CID 13951 (#3 of 3): Extra sizeof expression, no problem */
		   ...
		   struct in_addr *ls_id, struct in_addr *router_id, int exact)
{
  ...
  if (exact)
	...
	return NULL;
      ...
      /* LS ID. */
      oid2in_addr (offset, IN_ADDR_SIZE, ls_id);
			/*CID 13947 (#1 of 3): Extra sizeof expression , no problem*/
      offset += IN_ADDR_SIZE;
	...
	len = IN_ADDR_SIZE;
      ...
      oid2in_addr (offset, len, ls_id);
			...
			/*CID 13947 (#2 of 3): Extra sizeof expression , no problem*/
      offset += IN_ADDR_SIZE;
      offsetlen -= IN_ADDR_SIZE;
      ...
      if (lsa)
	{
	  ...
	  *offset = OSPF_AS_EXTERNAL_LSA;
	  offset++;
	  oid_copy_addr (offset, &lsa->data->id, IN_ADDR_SIZE);
		/*CID 13947 (#3 of 3): Extra sizeof expression , no problem*/
	  offset += IN_ADDR_SIZE;
	  oid_copy_addr (offset, &lsa->data->adv_router, IN_ADDR_SIZE);
