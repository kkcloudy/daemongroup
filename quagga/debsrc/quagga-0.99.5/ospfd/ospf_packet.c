  ...
  for (size -= OSPF_DB_DESC_MIN_SIZE;
	...
	default:
      ...
      switch (ospf_lsa_more_recent (find, new))
        {
					/*CID 11165 (#1 of 1): Missing break in switch, no problem*/
          case -1:
          ...
          case 0:
             ...
             */
//there is some errors	noticed by scx						
            if (find)
...
//	zlog_debug("func ospf_db_desc nbr->state is %s",LOOKUP (ospf_nsm_state_msg, nbr->state));
      ...
      if (IS_DEBUG_OSPF_NSSA)
			{
			  ...
			  zlog_debug("IN FUNC ospf_ls_upd LSA Type-%d from %s, ID: %s, ADV: %s",
				  lsa->data->type,
	 ...
	 translate them to a separate Type-5 packet.  */
      ...
      if (lsa->data->type == OSPF_AS_EXTERNAL_LSA)
        ...
        if (nbr->oi->area->external_routing != OSPF_AREA_DEFAULT) 
			  {
			    DISCARD_LSA (lsa, 1);
/* CID 14457 (#1 of 2): Structurally dead code , no problem */
	    if (IS_DEBUG_OSPF_NSSA)
...
ospf_db_desc_send (struct ospf_neighbor *nbr)
{
  ...
  if(0==nbr->db_sum.total&&nbr->state>=NSM_Exchange){
        if (IS_DEBUG_OSPF_PACKET (0, SEND))
  	    zlog_debug("ospf_db_desc_send():unset M bit of dd_flag");
  	UNSET_FLAG (nbr->dd_flags,OSPF_DD_FLAG_M);
  }
