/*
    $Id: watchquagga.c,v 1.3 2011/05/19 06:55:28 houxiaoxian Exp $
...
static const char *phase_str[] =
{
  ...
  "Rtm restart job running",
  "Waiting for rtm to come up",
  "Start jobs running",
};
...
1. Mode: %s.\n\
  ...
  them all.  Example:\n\
    %s -dz \\\n\
      -R '/sbin/service rtm restart; /sbin/service ospfd restart' \\\n\
      rtm ospfd\n\n\
2. Mode: %s.\n\
  ...
  in trouble using the supplied restart command.  Example:\n\
    %s -dz -r '/sbin/service %%s restart' rtm ospfd bgpd\n\n\
3. Mode: %s.\n\
  The same as the previous mode, except that there is special treatment when\n\
  the rtm daemon is in trouble.  In that case, a phased restart approach\n\
  is used: 1. stop all other daemons; 2. restart rtm; 3. start the other\n\
  daemons.  Example:\n\
    %s -adz -r '/sbin/service %%s restart' \\\n\
      -s '/sbin/service %%s start' \\\n\
      -k '/sbin/service %%s stop' rtm ospfd bgpd\n\n\
4. Mode: %s.\n\
...
-z, --unresponsive-restart\n\
		...
		restart purposes.\n\
-a, --all-restart\n\
		When rtm hangs or crashes, restart all daemons using\n\
		this phased approach: 1. stop all other daemons; 2. restart\n\
		rtm; 3. start other daemons.  Requires -r, -s, and -k.\n\
-A, --always-all-restart\n\
		When any daemon (not just rtm) hangs or crashes, use the\n\
