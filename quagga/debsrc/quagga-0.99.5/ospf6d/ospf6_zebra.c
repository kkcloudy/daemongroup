...
ospf6_zebra_if_add (int command, struct zclient *zclient, zebra_size_t length)
{
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug ("Rtm Interface add: %s index %d mtu %d",
...
ospf6_zebra_if_del (int command, struct zclient *zclient, zebra_size_t length)
{
  ...
  if (if_is_up (ifp))
    zlog_warn ("Rtm: got delete of %s, but interface is still up", ifp->name);

  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug ("Rtm Interface delete: %s index %d mtu %d",
		ifp->name, ifp->ifindex, ifp->mtu6);
                             ...
                             zebra_size_t length)
{
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug ("Rtm Interface state change: "
                                   ...
                                   zebra_size_t length)
{
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug ("Rtm Interface address add: %s %5s %s/%d",
                               ...
                               zebra_size_t length)
{
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    zlog_debug ("Rtm Interface address delete: %s %5s %s/%d",
		c->ifp->name, prefix_family_str (c->address),
		inet_ntop (c->address->family, &c->address->u.prefix,
			   buf, sizeof (buf)), c->address->prefixlen);
  ...
  if (IS_OSPF6_DEBUG_ZEBRA (RECV))
    {
      ...
      zlog_debug ("Rtm Receive route %s: %s %s nexthop %s ifindex %ld",
       ...
       "Rtm information\n")
{
  ...
  if (zclient == NULL)
    {
      vty_out (vty, "Not connected to rtm%s", VNL);
      return CMD_SUCCESS;
    }

  vty_out (vty, "Rtm Infomation%s", VNL);
...
DEFUN (no_router_zebra,
       ...
       "Disable connection to rtm daemon\n")
{
  zclient->enable = 0;
...
config_write_ospf6_zebra (struct vty *vty)
{
  if (! zclient->enable)
    {
      vty_out (vty, "no router rtm%s", VNL);
    }
  else if (! zclient->redist[ZEBRA_ROUTE_OSPF6])
    {
      vty_out (vty, "router rtm%s", VNL);
...
ospf6_zebra_route_update (int type, struct ospf6_route *request)
{
  ...
  if (zclient->sock < 0)
    {
      if (IS_OSPF6_DEBUG_ZEBRA (SEND))
        zlog_debug ("  Not connected to Rtm");
      return;
    }
  ...
  if (nexthops == NULL)
    {
      zlog_warn ("Can't send route to rtm: malloc failed");
      return;
    }
...
ospf6_zebra_init ()
{
  ...
  /* Install zebra node. */
  install_node (&zebra_node, config_write_ospf6_zebra, "ZEBRA_NODE");
...
DEFUN (debug_ospf6_zebra_sendrecv,
       ...
       "debug ospf6 rtm (send|recv)",
       DEBUG_STR
       OSPF6_STR
       "Debug connection between rtm\n"
       "Debug Sending rtm\n"
       "Debug Receiving rtm\n"
      )
{
...
config_write_ospf6_debug_zebra (struct vty *vty)
{
  if (IS_OSPF6_DEBUG_ZEBRA (SEND) && IS_OSPF6_DEBUG_ZEBRA (RECV))
    vty_out (vty, "debug ospf6 rtm%s", VNL);
  else
    {
      if (IS_OSPF6_DEBUG_ZEBRA (SEND))
        vty_out (vty, "debug ospf6 rtm send%s", VNL);
      if (IS_OSPF6_DEBUG_ZEBRA (RECV))
        vty_out (vty, "debug ospf6 rtm recv%s", VNL);
    }
  return 0;
}
