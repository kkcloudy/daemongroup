...
#else
	  ifp = if_get_by_vty_index((char*)vty->index);
#endif
  ...
  for (ALL_LIST_ELEMENTS_RO (iflist, i, ifp))
    {
        ...
        /*vty_out (vty, " description %s%s", ifp->desc, VNL);*/
	  {
		if(CHECK_FLAG(ifp->desc_scope,INTERFACE_DESCIPTION_LOCAL))
			vty_out (vty, " local description %s%s", ifp->desc,VNL);
		else
			vty_out (vty, " description %s%s", ifp->desc,VNL);
		}
...
ospf6_interface_init ()
{
  ...
  install_node (&interface_node, config_write_ospf6_interface, "INTERFACE_NODE");
