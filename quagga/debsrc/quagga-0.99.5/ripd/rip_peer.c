...
#define RIP_UPTIME_LEN 25
  ...
  for (ALL_LIST_ELEMENTS (peer_list, node, nnode, peer)){
      vty_out (vty, "  Gateway prefix : %s%s" 
	  				"   BadPackets : %d%s" 
	  				"   BadRoutes : %d%s" 
	  				"   Distance : %d%s"   
	  				"   Last Update : %s%s", 
	  	inet_ntoa (peer->addr),VTY_NEWLINE,
	       peer->recv_badpackets,VTY_NEWLINE,
	       	peer->recv_badroutes,VTY_NEWLINE,
	        	ZEBRA_RIP_DISTANCE_DEFAULT,VTY_NEWLINE,
	       			rip_peer_uptime (peer, timebuf, RIP_UPTIME_LEN), VTY_NEWLINE);
 }
}
