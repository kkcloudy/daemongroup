       ...
       RIP_STR)
{
  ...
  if (IS_RIP_DEBUG_ZEBRA)
    vty_out (vty, "  RIP rtm debugging is on%s", VTY_NEWLINE);
...
config_write_debug (struct vty *vty)
{
  ...
  if (IS_RIP_DEBUG_PACKET)
    {
      if (IS_RIP_DEBUG_SEND && IS_RIP_DEBUG_RECV && !IS_RIP_DEBUG_DETAIL)
		{
  ...
  if (IS_RIP_DEBUG_ZEBRA)
    {
      vty_out (vty, "debug rip rtm%s", VTY_NEWLINE);
      write++;
    }
  return write;
}
