...
config_write_zebra (struct vty *vty)
{
  if (! zclient->enable)
    {
      vty_out (vty, "no router rtm%s", VTY_NEWLINE);
      return 1;
    }
  else if (! zclient->redist[ZEBRA_ROUTE_RIP])
    {
      vty_out (vty, "router rtm%s", VTY_NEWLINE);
...
config_write_rip_redistribute (struct vty *vty, int config_mode)
{
  ...
  for (i = 0; i < ZEBRA_ROUTE_MAX; i++)
	...
	/*added by gxd*/
			    ...
			    if (rip->route_map[i].metric_config){
					if (rip->route_map[i].name)
					  vty_out (vty, "  Redistribute type : %s" 
					  			 	"  Metric : %d" 
					  			 	"  Route-map %s%s",
						zebra_route_string(i),
						    rip->route_map[i].metric,
						   		rip->route_map[i].name,VTY_NEWLINE);
					else
					  vty_out (vty, "  Redistribute type : %s"
					  				"  Metric : %d%s",
						zebra_route_string(i),
						   		rip->route_map[i].metric,VTY_NEWLINE);
			    }
			    else{
					if (rip->route_map[i].name)
					  vty_out (vty, "  Redistribute type : %s" 
					  ...
					  vty_out (vty, "  Redistribute type : %s%s", 
...
rip_zclient_init ()
{
  ...
  /* Install zebra node. */
  install_node (&zebra_node, config_write_zebra, "ZEBRA_NODE");
